...
**洛谷题目**：P1059（明明的随机数——set 的去重排序），P1093（奖学金——vector + sort + Lambda），P1923（第 k 小数——nth_element），P3378（堆——priority_queue），P5788（单调栈——stack 容器适配器），P3367（并查集——unordered_map）。
