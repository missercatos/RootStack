...
>
> 每个章节配有底层原理剖析、实际案例, 以及力扣题目链接用于自检。
...
| 16 | [[cpp深化教程/16_CMake深入]] | CMake 进阶/目标/测试/打包 |
| 17 | [[cpp深化教程/17_包管理器Conan与vcpkg]] | Conan/vcpkg 包管理 |
...
| CPP 章节 | 建议同步阅读的 C 内容 |
|----------|---------------------|
| `03_指针与引用` | [[../c语言教程/2深化/01_指针深度剖析|C指针深度剖析]] |
| `04_动态内存` | [[../c语言教程/2深化/02_动态内存管理|C动态内存管理]] |
| `05_类与对象基础` | [[../c语言教程/2深化/08_函数指针与回调|C实现OOP]] |
| `13_多线程` | [[../c语言教程/2深化/09_多线程与同步|C多线程]] |

---

## 第二部分 — STL 容器类

> 每个容器章节: 底层结构 → 用法大全 → 实际案例 → 力扣题目

| 章节 | 内容 |
|------|------|
| [[容器库/其他/pair\|pair]] | std::pair 键值对 |
| [[容器库/序列容器/vector\|vector]] | 动态数组, 随机访问 O(1) |
| [[容器库/序列容器/string\|string]] | std::string 字符序列 |
| [[容器库/适配器/stack\|stack]] | 栈适配器 LIFO |
| [[容器库/适配器/queue\|queue]] | 队列适配器 FIFO |
| [[容器库/序列容器/deque\|deque]] | 双端队列 |
| [[容器库/适配器/priority_queue\|priority_queue]] | 优先队列(堆) |
| [[容器库/序列容器/list\|list]] | 双向/单向链表 |
| [[容器库/关联容器/set\|set]] | 有序集合(红黑树) |
| [[容器库/关联容器/map\|map]] | 有序映射(红黑树) |
| [[容器库/无序容器/unordered_set\|unordered_set]] | 无序集合(哈希) |
| [[容器库/无序容器/unordered_map\|unordered_map]] | 无序映射(哈希) |
| [[容器库/其他/bitset\|bitset]] | std::bitset 位集 |
...
| 类型 | 索引文件 | 说明 |
|------|---------|------|
| 容器库 | [[容器库/容器库索引]] | 序列容器/关联容器/无序容器/适配器/其他 |
| 功能库 | [[cpp功能库/功能库索引]] | 字符串/IO/算法/数值/并发/内存/函数式/工具 |
| 第三方库 | [[cpp第三方库/第三方库索引]] | 网络/GUI/游戏/数据库/序列化/测试/日志/加密/数学/并发/综合 |
