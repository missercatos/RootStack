...
> **重要说明**: 渗透测试是**职业方向路径**，C/C++/Java/Rust/Python 路径是计算机科学基础，所有方向的学习者都可以学习；渗透测试是基础完成后的**职业分化**——选择网络安全方向后，本路径指引如何搭建通用 Linux 工作环境、获取工具，并进入渗透测试、红队攻防和 CTF 竞赛领域。
>
...
| 阶段 | 模块 | 时间 | 内容 |
|------|------|------|------|
| 前置准备 | Linux 工作环境与工具获取 | 1天 | [[red_team/工具获取与环境搭建\|工具获取与环境搭建]] |
...
数据库安全是渗透测试的核心技能——SQL 注入、权限提升、数据窃取都离不开数据库知识。

| 数据库 | 安全关联 | 教程链接 |
|--------|----------|----------|
| MySQL 基础 | SQL 注入的前提：理解数据库结构、用户权限、数据类型 | [[数据库/mysql/01-安装与配置\|MySQL 安装]] → [[数据库/mysql/02-SQL基础语法\|SQL 基础]] |
| MySQL 深入 | 注入绕过技巧：理解查询优化、字符串处理、编码问题 | [[数据库/mysql/03-查询进阶\|查询进阶]] → [[数据库/mysql/04-索引事务与优化\|索引与事务]] |
| MySQL 权限 | 提权攻击：理解 GRANT/REVOKE、用户管理、密码策略 | [[数据库/mysql/05-用户权限与备份恢复\|权限与备份]] |
| Redis | Redis 渗透：未授权访问、内存马、RCE | [[数据库/redis/Redis教程\|Redis 教程]] |
| MongoDB | MongoDB 渗透：NoSQL 注入、未授权访问 | [[数据库/mongodb/MongoDB教程\|MongoDB 教程]] |
| Elasticsearch | ES 清透：未授权访问、数据泄露 | [[数据库/elasticsearch/Elasticsearch入门教程\|Elasticsearch 入门]] |
| SQL 注入攻防 | 红队 SQL 注入实战：手工注入、自动化工具、绕过 WAF | [[red_team/数据库安全/02-MySQL渗透\|MySQL 渗透]] → [[red_team/数据库安全/06-Redis渗透\|Redis 渗透]] |

> **建议**：在学第一阶段（网安基础）时同步学习 MySQL 01-02（SQL 基础）；在学第三阶段（Web 安全）时同步学习 MySQL 03-05（查询进阶 + 权限）和红队数据库安全章节。

---

## 第二阶段 -- 渗透测试基础 (建议 2-3 周)

> 入口: [[red_team/总目录与快速查询#2. 渗透测试基础\|总目录 → 2.渗透测试基础]]

渗透测试基础教程，覆盖红队基本操作流程：信息收集 → 扫描枚举 → 漏洞评估 → 密码攻击 → 嗅探 → 后渗透。

| 序号 | 文件 | 核心工具 | 关联RootStack |
|------|------|---------|--------------|
| 01 | [[red_team/渗透基础/01-基础命令与Linux安全操作]] | bash/chmod/SSH | [[|]] (Linux基础) |
| 02 | [[red_team/渗透基础/02-信息收集与侦察技术]] | theharvester/recon-ng | - |
| 03 | [[red_team/渗透基础/03-网络扫描与枚举技术]] | nmap/masscan | - |
| 04 | [[red_team/渗透基础/04-漏洞评估与利用基础]] | Metasploit/searchsploit | - |
| 05 | [[red_team/渗透基础/05-密码攻击与破解技术]] | hydra/john | [[|]] (哈希与数论) |
| 06 | [[red_team/渗透基础/06-网络嗅探与中间人攻击]] | Wireshark/bettercap | [[汇编基础/1基础/01_寄存器与指令基础]] (网络协议底层) |
| 07 | [[red_team/渗透基础/07-后渗透与权限维持]] | Meterpreter | [[|]] (权限模型) |
| 08 | [[red_team/渗透基础/08-痕迹清除与渗透报告]] | shred/logs | - |

---

## 第三阶段 -- Web 安全 (建议 3-4 周)

> 入口: [[red_team/总目录与快速查询#6. Web 安全\|总目录 → 6.Web安全]]

Web 渗透是红队最常见的攻击面。从 HTTP 协议开始，逐一掌握 SQL 注入、XSS/CSRF、文件包含、命令注入、认证攻击、WAF 绕过。

| 序号 | 文件 | 攻击类型 | 关联RootStack |
|------|------|---------|--------------|
| 01 | [[red_team/Web渗透/01-Web基础与HTTP协议]] | HTTP基础 | - |
| 02 | [[red_team/Web渗透/02-Web信息收集与侦察]] | Web指纹/扫描 | - |
| 03 | [[red_team/Web渗透/03-Web漏洞扫描与检测]] | 自动化扫描 | - |
| 04 | [[red_team/Web渗透/04-SQL注入攻击]] | SQLi | [[ISSUES|数据结构]] (数据库结构理解) |
| 05 | [[red_team/Web渗透/05-XSS与CSRF攻击]] | XSS/CSRF | - |
| 06 | [[red_team/Web渗透/06-文件包含与命令注入]] | LFI/RFI/CMD | - |
| 07 | [[red_team/Web渗透/07-认证与会话攻击]] | Auth/Session | - |
| 08 | [[red_team/Web渗透/08-Web渗透综合实战]] | 综合演练 | - |
| 09 | [[red_team/Web渗透/09-WAF绕过与高级技巧]] | WAF bypass | - |
...
进阶模块覆盖 12 个安全领域，每个领域 2-9 篇教程。

| 工具组 | 入口 | 内容 | 关联RootStack |
|--------|------|------|--------------|
| 侦察 (recon) | [[red_team/信息收集/01-高级子域名与资产发现]] | amass/subfinder/Shodan/OSINT | - |
| 扫描 (scanner) | [[red_team/扫描与枚举/01-高速大规模扫描技术]] | masscan/zmap | - |
| 漏洞利用 (exploit) | [[red_team/漏洞利用/01-漏洞利用框架精通]] | Metasploit高级/msfvenom | - |
| 密码破解 (cracking) | [[red_team/密码攻击/01-在线密码攻击大师课]] | Hydra/Hashcat/John | - |
| 提权 (privesc) | [[red_team/权限提升/01-Linux权限提升完整指南]] | SUID/kernel exploits | [[ISSUES|系统内核]] |
| 隧道代理 (proxy) | [[red_team/代理与隧道/01-代理与隐蔽通信]] | SSH tunnels/chisel/Tor | - |
| 无线 (wireless) | [[red_team/无线安全/01-无线网络基础]] | WiFi/802.11/WPA破解 | - |
| 取证 (forensics) | [[red_team/数字取证/01-磁盘取证与反取证]] | Autopsy/Volatility | - |
| 模糊测试 (fuzz) | [[red_team/模糊测试/01-模糊测试入门与AFL实战]] | AFL++/ffuf/wfuzz | - |
| 恶意软件 (malware) | [[red_team/恶意软件分析/01-恶意软件分析入门]] | YARA/radare2 | [[ISSUES|汇编基础]] (逆向必备) |
...
| 3 | [[red_team/实战教程/02-渗透测试工作环境搭建]] | 工具获取与工作环境 | - |
