...
建议先阅读: [[A_数组_Array|A 数组]], [[L_树_Tree_BST_AVL|K 树 BST AVL]], [[P_哈希表_HashTable|O 哈希表]]
...
顺序查找太慢、二分查找要求全局有序——分块查找是二者的折中：把表分成若干块，**块间有序、块内可以无序**（前一块的最大值小于后一块的最小值）。

### 原理

维护一张索引表，每项记录该块的最大值。查找分两步：先在索引表中确定 key 可能所在的块，再在该块内顺序查找。
...
设 $n$ 个元素分为 $b$ 块、每块 $s$ 个（$n = b \cdot s$），索引表与块内都用顺序查找：

- 索引查找平均比较 $\frac{b+1}{2}$ 次，块内平均 $\frac{s+1}{2}$ 次
- 总平均查找长度 $ASL = \frac{b+1}{2} + \frac{s+1}{2}$
- 当 $s = \sqrt{n}$ 时取最小：$ASL \approx \sqrt{n} + 1$

分块查找适合**动态变化**的场景：块内插入删除只影响本块，不必像二分查找那样维护全局有序。若索引表本身改用二分查找，ASL 可进一步降为 $\log_2 b + \frac{s+1}{2}$。
...
| 分块查找 | 块间有序 | $\approx \sqrt{n}$ | — | $O(\sqrt{n})$ | 兼顾动态与效率 |
