...
建议先阅读: [[E_容器_Container|容器概览]], [[G_栈_Stack|栈]]
...
BFS 用队列的 FIFO 顺序保证了"先发现先处理"。在无权重图中，节点在 BFS 树中的深度 = 从起点到该节点的最短路径长度。Dijkstra 算法是 BFS 的加权推广——将队列替换为优先队列（堆）。详见 [[T_图_Graph|图的 BFS]]。
