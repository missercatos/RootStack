...
> **考研 408 导引**：B 树在 408 中是**选择题+偶尔简答题考点**。核心是 m 阶 B 树的定义（五个条件）、B 树 vs B+ 树的对比（数据存储位置/范围查询/叶子链表）、B 树插入的分裂过程（给序列逐步画树）。B+ 树的 InnoDB 应用为增值内容。正文备有 1 组闭卷手算自测。
...
**第 5 步**：插入 50 → 右子 `[30,40]` 变 `[30,40,50]`（满）
...
| [449](https://leetcode.cn/problems/serialize-and-deserialize-bst/) | 序列化和反序列化 BST | 树的序列化（B树同理） |
