...
> **考研 408 导引**：本章覆盖考纲全部考点。应试重点依次为——字符串的三种存储方式（含存储密度计算）、BF 与 KMP 的手算过程、next / nextval 数组的定义与求解（均有专门小节与手算示范表）；DFA 视角、BM/RK、后缀结构等超出考纲的内容可作为理解加深，考场上不作要求。
...
国内数据结构教材（408 考纲体系）把字符串的存储归纳为三类。它们与上一节的国际视角并不冲突——只是按"内存从哪来、长度放哪、怎么链接"三个维度重新分类：
...
考试要点：给定块大小与指针宽度能手算存储密度；能说出三种方式的适用场景——定长适合协议字段、固定配置项；堆分配是通用默认；块链几乎只活在教材里，现代实现都走"胖指针 + 连续缓冲区"路线（上一节）。
...
**408 手算示范**：以 $T$ = `"ababcabcab"`、$P$ = `"abcab"` 为例（下标从 0 开始），BF 的完整比较轨迹：
...
失败后的行为是考点核心：$i$ 只回退到 $i+1$，$j$ 归零从头再比——第 0 步已经比对出的信息（T[0..1]="ab"）被完全丢弃。KMP 的全部价值就在于回收这份信息：失配在 P[2] 时其实无需移动 i，直接让模式串"滑"到正确位置继续。

**408 自测：BF 轨迹书写**

合上前例，自己完成：$T$ = `"aababaab"`、$P$ = `"baab"`，列出 BF 从每个起点开始的比较过程与次数，并给出总比较次数。
...
>
> 总计 $1+1+3+1+4=10$ 次比较。自查要点：每次失败后是否记得 j 归零、i 只前进一位；第 i=2 步那种"匹配了前缀又断掉"的情形是失分重灾区——比较次数要按实际比对过的字符数写。
...
**408 视角：1-indexed 的 next 数组**

考研教材（严蔚敏体系）对同一个数学对象使用不同记号：模式串下标从 1 开始，数组名叫 next：
...
```c
// 1-indexed 版本：T、P 的下标都从 1 开始（408 教材原貌）
...
再练一个周期性强的：$P$ = `"aaaab"` → next = [0, 1, 2, 3, 4]，每个位置的 Border 都取到最大。**手算要领：盯住 P[1..j-1] 这一段，看首尾最多能重叠多长，加 1 即答案。**

**408 自测：next 数组求解**

求下列三个模式串的 next 数组，写完再对照答案（难度递进，第 ③ 题专门考"Border 回落"）：
...
408 偶尔考"给定两串，写出 DP 表格（或其前几行）"。合书完成下面两题：
...
| 自测 | 位置 | 考什么 |
|------|------|--------|
| BF 轨迹书写 | BF 小节末 | 每个起点的比较过程与次数 |
| next 数组求解 ×3 | next 手算表后 | Border 回落、真前后缀陷阱 |
| KMP 轨迹书写 ×2 | 匹配示范后 | j 连续回跳时 i 不动 |
| nextval 求解 ×2 | nextval 对照表后 | 从左往右的折叠方向 |
| DP 网格填表 ×2 | LCS 小节末 | 编辑距离 vs LCS 的转移差异 |
