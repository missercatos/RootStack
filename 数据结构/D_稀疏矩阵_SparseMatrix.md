...
建议先阅读：[[A_数组_Array|数组]] — 稀疏矩阵的所有存储格式，最终都是把矩阵元素存在一维数组里，然后通过额外的索引数据重建二维索引。[[E_容器_Container|容器]] — 连续 vs 节点存储的性能模型直接影响三种格式的选择。
...
> 这里的"顺序表"指用连续数组顺序存储的表结构（线性表的顺序实现），概念见 [[C_线性表_LinearList|线性表与顺序表]]。
