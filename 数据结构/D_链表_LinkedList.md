...
创建链表的时候，头指针必须初始化为空，如果不初始化会被分配随机垃圾值，在链表的结构里，头指针为空意味着这是一个空表，如果不为空，代表链表里至少有一个节点，如果不初始化，那么程序会误认链表不为空，去访问那个垃圾值，导致程序崩溃

### 带头节点
有些链表会额外增加一个头节点，也叫虚拟节点，
这个时候头指针指向头节点，它不为空
空链表的时候 head->[dummy|NULL]
这个时候头指针也不为空，所以头节点的next是NULL
当带头节点的时候，判断空链表看的是head->next == NULL
而不是head == NULL

### 一些关于链表的表示还有名词解释
比如： head->[10|NULL]  这里head表示头指针，里面存的是节点地址，->是指向的意思，
[10|NULL]表示一个节点，[]方框内表示一块节点占据内存（也可以直接说是节点空间）
|中间的竖线，左边是数据域，内容是10,右边是指针域，指针是NULL
**数据域**存储实际的数据值，**指针域**存储下一个节点在内存中的地址
![[../assets/images/链表.png]]
实际上画图可以如上图所示，head->[data|next]是对上图的抽象化演示
在设置多个节点的时候，可以这样表示：head -> [10|* ] -> [20|* ] -> [30|NULL]
头指针入口放在第一位，最终节点的下一位指针必须是NULL
...
链接存储的存储空间分为两部分：**数据域**存放结点的值，**指针域**存放结点之间的关系。单链表每个结点额外带一个指针，存储密度小于 1；顺序表只存数据，存储密度为 1。
...
**核心版本**只改两条指针，前提是"`p` 一定存在，而且插入点不是表头"：
...
它处理不了"插到表头之前"——表头没有前驱结点，没法只靠 `p->next` 改到 `head` 本身。正规实现用二级指针接收头指针，把这种情况单独处理：

```c
Status insert_at(Node **head, int i, int x) {
    if (i < 1) return ERROR;
    if (i == 1) {                 /* 插到表头：改的是头指针本身 */
        Node *s = newNode(x);
        s->next = *head;
        *head = s;
        return OK;
...
补充的部分解决了两个边界：`i == 1` 时改的是头指针，只能用 `**head`；`p == NULL` 说明位置超出表长，直接报错，避免解引用空指针。
...
**核心版本**假设"待删结点一定存在，且删的不是表头"：

```c
void delete_core(Node *p) {
    Node *q = p->next;
    p->next = q->next;
    free(q);
}
...
它删的是 `p` 的后继；碰到"要删表头"就无能为力——表头没有前驱。正规实现改成按值删除，把表头和找不到两种情况都处理掉：

```c
Status delete_value(Node **head, int x) {
    Node *prev = NULL, *cur = *head;
    while (cur != NULL && cur->data != x) {
        prev = cur;
        cur = cur->next;
    }
    if (cur == NULL) return ERROR;        /* 目标不存在 */
    if (prev == NULL) *head = cur->next;  /* 删的是首元结点 */
...
补充的部分：用 `prev` 记住前驱，才能让前驱"跳过"被删结点；用 `**head` 处理删表头；找不到目标时返回 `ERROR`，而不是继续往下走导致崩溃。
...
**核心版本**就是一层 `while`，把"访问动作"直接写在循环里：

```c
void traverse_core(SNode *head) {
    for (SNode *p = head; p != NULL; p = p->next)
        printf("%d ", p->data);
}
```

它的问题和顺序表一样：访问方式和遍历过程绑死，换成求和、统计、写文件就要重抄一遍。正规实现把"做什么"抽成函数指针：

```c
void traverse(SNode *head, void (*visit)(int)) {
    for (SNode *p = head; p != NULL; p = p->next)
        visit(p->data);
}
...
traverse(head, print_elem);
```

![[list_traverse.gif]]

补充的函数指针让同一份遍历代码可以打印、求和、找最大值；遍历是 $O(n)$，但不能在 `visit` 里删除结点，否则当前指针立刻失效。
...
为什么这样写：循环条件是"没走到头且还没数到 k"，返回的 `cur` 要么是第 k 个结点，要么是 `NULL`（表长不足）。时间复杂度 $O(k)$，最坏 $O(n)$——这是链表相对顺序表最大的劣势。
...
**正规实现**还可以把"怎么判断相等"交给调用者，这样结构体元素也能用同一份查找代码：

```c
SNode* locate(SNode *head, int x, int (*eq)(int, int)) {
    for (SNode *p = head; p != NULL; p = p->next)
        if (eq(p->data, x)) return p;
    return NULL;
}
```
...
![[list_push_front_back.gif]]

图中上半部分是头插、下半部分是尾插，依次插入 1、2、3。区别不只是"插在哪边"：

| 维度 | 头插 | 尾插 |
|------|------|------|
| 插入代价 | $O(1)$ | 无尾指针 $O(n)$，有尾指针 $O(1)$ |
| 最终顺序 | 与输入**相反**（3→2→1） | 与输入**相同**（1→2→3） |
| 代码关键 | `s->next = head; head = s;` | 走到 `p->next == NULL` 再挂上 |
| 典型用途 | 造逆序、尾插不便时 | 按输入顺序建表 |
...
所以**创建一个含 $n$ 个结点的有序单链表，时间复杂度是 $O(n^2)$**。对比：元素已有序时，用头插或尾插创建只需 $O(n)$。
...
- **典型用途**：不支持指针的语言、嵌入式等需要预分配内存的场景；插入/删除只改游标。
...
**游标插入怎么推导**：数组里结点 `i` 的物理后一个位置是 `i+1`，但它的**逻辑后继**由游标 `next[i]` 决定——这两者没有任何关系。所以插入时要改的是游标，不是物理位置。在 `i` 之后插入新结点 `s`：

```c
next[s] = next[i];   /* ① 让新结点接住 i 原来的逻辑后继 */
next[i] = s;         /* ② 再让 i 指向新结点 */
```

它和链式写法 `s->next = p->next; p->next = s;` 一模一样，只是把"指针"换成了"数组下标"。删除同理：`next[i] = next[next[i]];`，然后把被删结点的下标挂回**备用链表**（用 `next[0]` 串起空闲结点，供后续插入复用）。考题常给一张"数据 + 游标"的表，让你写出插入/删除后游标字段的变化序列——抓住"逻辑后继看游标、不看相邻下标"这一句即可。
...
/* 删除 p */
p->next->prev = p->prev;    /* ① 后继先接上 p 的前驱 */
p->prev->next = p->next;    /* ② 前驱再接上 p 的后继 */
free(p);
```

![[doubly_insert_delete.gif]]

**四个指针的改动顺序**，记一句口诀：**先在新位置把"后继"关系接好，再回头改"前驱"**。

- 插入 `s`：① `s->next = p->next`（新结点接住后继）→ ② `s->prev = p`（指回前驱）→ ③ `p->next->prev = s`（原后继回指新结点）→ ④ `p->next = s`（最后才断开旧指针）。关键约束是 ① 和 ③ 都必须在 ④ 之前——④ 一旦执行，`p` 原来的后继地址就找不到了；
- 删除 `p`：① `p->next->prev = p->prev`（后继接前驱）→ ② `p->prev->next = p->next`（前驱接后继）→ ③ `free(p)`。删除时两步都只读 `p->prev / p->next`，先后其实都安全，但统一成"先接后继"不易记混，还能保证 `free` 一定在最后。
...
判空条件随形态不同，而且"只有一个元素结点"的判断常常紧跟着一起考：

| 形态 | 判空 | 只有一个元素结点 | 特点 |
|------|------|------------------|------|
| 循环单链表（不带头结点） | `head == NULL` | `head->next == head` | 尾节点 next 指回 head |
| 循环单链表（带头结点） | `head->next == head` | `head->next->next == head` | 从任意结点出发可达全表 |
| 循环双链表（带头结点） | `head->next == head` | `head->next != head && head->next->next == head` | `head->prev` 即尾结点，头尾操作全 $O(1)$ |

注意"单元素"的判断必须带上前半句 `head->next != head`：只写 `head->next->next == head` 的话，空表（`head->next == head`，此时 `head->next->next` 也等于 `head`）会被误判成"有一个元素"。
...
进程就绪队列、时间轮这类场景常用**带头结点 + 尾指针 `rear`** 的循环单链表：`rear` 指向尾结点，于是 `rear->next` 是头结点，而**首元结点是 `rear->next->next`**。

```mermaid
graph LR
    H["头结点 head"] --> A["a1 首元"] --> B["a2"] --> C["a3 尾"]
    C -->|"next 指回头结点，成环"| H
    R["rear 尾指针"] -.-> C
```

有了尾指针，表尾操作和表头操作都变成了"一个指针的活"：
...
/* 判空：只剩头结点 */
int empty(Node *head) { return head->next == head; }

/* 取首元结点：注意是 rear->next（头结点）的下一个 */
Node* first(Node *rear) { return rear->next->next; }

/* 尾插：新结点接回头结点，再把原尾结点指向它 */
void push_back(Node *head, Node **rear, int x) {
    Node *s = (Node *)malloc(sizeof(Node));
    s->data = x;
    s->next = head;             /* 新结点接回头结点，保持成环 */
    (*rear)->next = s;          /* 原尾结点指向新结点 */
    *rear = s;                  /* 更新尾指针 */
}
```

真正容易错的是**删除首元结点**：删掉之后链表变短，一般只要 `head->next = q->next;`；但如果链表**只剩一个元素**，删完就变空，尾指针还指着那个已经被释放的结点，必须把它收回头结点：

```c
/* 删除首元结点：q 是首元结点，rear 是尾指针 */
int pop_front(Node *head, Node **rear) {
    Node *q = head->next;             /* 首元结点 */
    if (q == head) return -1;         /* 空表 */
    head->next = q->next;             /* 头结点跳过 q */
    if (q == *rear) *rear = head;     /* ① 只有一个元素：尾指针收回 */
    free(q);
    return 0;
}
```

`if (q == *rear) *rear = head;` 就是这一结构的经典边界。若把 `rear` 写成局部指针、头指针写成 `h`，它就是教材里更紧凑的那句 `if (p == q) p = h;`——`p` 是尾指针、`q` 是首元结点、`h` 是头结点，含义相同：**删到只剩一个元素时，尾指针必须重新指向头结点**。

有了尾指针，尾插 $O(1)$、取尾结点 $O(1)$；但**删除尾结点**仍然要 $O(n)$——循环单链表只靠单指针找不到尾结点的前驱。这也正是"带头结点双循环链表"存在的理由：它用两个方向换取尾删也能 $O(1)$。
...
如果要合并成**降序**，把每次选出的较小结点用**头插法**挂到结果表头即可（不必再维护 `tail`）：

```c
ListNode* merge_desc(ListNode* a, ListNode* b) {
    ListNode* head = NULL;
    while (a && b) {
        ListNode* s;
        if (a->data <= b->data) { s = a; a = a->next; }
        else                    { s = b; b = b->next; }
        s->next = head;          /* 头插：逆序，因此得到降序 */
        head = s;
    }
    while (a) { ListNode* s = a; a = a->next; s->next = head; head = s; }
    while (b) { ListNode* s = b; b = b->next; s->next = head; head = s; }
    return head;
}
```

比较指针最多走 $m+n$ 步，时间仍是 $O(m+n)$；大 $O$ 记号下 $O(m+n)$ 与 $O(\max(m,n))$ 等价（因为 $\max \le m+n \le 2\max$）。**尾插保值、头插逆序**——这道题把两条结论一次考全。
...
算法设计题之外，选择题和填空题更爱考下面这些结论，集中放在这里；正文里不再穿插考点提示，以免打断原理的学习。

**1. 存储密度**：$\text{存储密度} = \dfrac{\text{数据本身占用的空间}}{\text{结点占用的总空间}}$。顺序表必为 1；单链表小于 1（`int` + 8 字节指针时约 $1/3$）；结点带其他辅助字段时会更低。

> 考察内容：比较两种存储结构空间效率的选择题。

**2. 两种"插入"的复杂度**：在**已知结点 `*p` 之后**插入是 $O(1)$（只改两条指针）；在**给定值为 `x` 的结点之后**插入是 $O(n)$（先按值查找）。题目问插入复杂度时，先判断位置是已知还是查找得到的。

> 考察内容：选择题的经典陷阱——把"已知位置插删 $O(1)$"套到"按值插删"上。
...
只做逻辑删除（不释放）时可简写为 `p->next = p->next->next;`；顺序不能颠倒。

> 考察内容：填空题写指针操作序列，或判断"先 free 后改指针"的错误写法。

**4. 头插法得到逆序**：用头插法建表，得到的序列与输入顺序相反；这个性质常与就地逆置、链表重排一起考。

> 考察内容：选择题给插入序列问最终链表，或判断逆序关系。
...
频繁"末尾插入 + 删除尾结点"选**带头结点双循环链表**；只做表头操作时，**带头结点单链表**也能 $O(1)$——"带头结点"的意义就在于简化首元结点的插删与统一空表处理。

> 考察内容：结构选型选择题，或综合题里的方案比较。

**6. 双向链表插删的指针顺序（填空）**：删除 `p`：`p->next->prior = p->prior;` 然后 `p->prior->next = p->next;`；在 `p` 之后插入 `q`：`q->prior = p; q->next = p->next; p->next->prior = q; p->next = q;`。先接后继、再改前驱，顺序颠倒会丢失原后继地址。

> 考察内容：填空题写指针操作顺序，或找出错误顺序。

**7. 创建有序单链表是 $O(n^2)$**：无序数据建成有序单链表，每插一个都要找位置，共 $O(n^2)$；数据已有序时用头插/尾插创建只需 $O(n)$。
...
> 考察内容：填空题与选择题的常客，特别是"带头结点单链表"这一行。

**9. 带头结点双循环链表"只有一个元素结点"（高频填空）**：

```c
L->next != L && L->next->next == L
```

等价写法：`L->next->next == L && L->prior == L->next`。前半句排除空表（`L->next == L`），后半句说明首元结点的后继回到头结点。

> 考察内容：填空题；常与判空条件一起考。

**10. 带尾指针的循环单链表**：`rear` 指向尾结点，则 `rear->next` 是头结点，**首元结点是 `rear->next->next`**。已知 `rear` 时尾插、取尾结点都是 $O(1)$；但删除尾结点仍 $O(n)$（找不到前驱）。删除首元结点若链表只剩一个元素，必须更新尾指针：`if (q == *rear) *rear = head;`，紧凑写法就是 `if (p == q) p = h;`（`p` 尾指针、`q` 首元结点、`h` 头结点）。

> 考察内容：填空题问首元结点的表示、删除首元结点的边界判断。

**11. 循环链表的指针链推导**：循环单链表中，沿 `next` 走几层回到起点，就说明链上有几个结点。例如 `L->next->next->next == L`，从 `L`（头结点）出发走 3 步回到 `L`，说明链上共 3 个结点（不含头结点）；若 `L->next == L` 则为空表。**数 `->next` 的层数 = 数结点个数**。

> 考察内容：选择题给一串 `->next` 表达式，问表长或判断是否成环。

**12. 有序单链表单次插入是 $O(n)$**：保持有序的插入，时间几乎都花在"从头找第一个不小于 `x` 的结点"上——查找 $O(n)$、改指针 $O(1)$，总计 $O(n)$。$n$ 个无序数据建成有序单链表是 $O(n^2)$。注意与"已知位置插入 $O(1)$"区分开。

> 考察内容：复杂度选择/填空，经典干扰项是把两者混为一谈。

**13. 升序合并为降序是 $O(m+n)$**：合并两个长度 $m$、$n$ 的升序链表，若把每次选出的较小结点用**头插法**挂到结果表头，得到的就是降序表。比较指针最多走 $m+n$ 步，故 $O(m+n)$；大 $O$ 记号下它与 $O(\max(m,n))$ 等价（$\max \le m+n \le 2\max$）。**尾插保序、头插逆序**，这是这道题的全部考点。

> 考察内容：算法设计/复杂度分析，问"升序合并为降序"的实现与复杂度。

**14. 静态链表的游标修改**：静态链表用数组下标（游标）代替指针，逻辑后继看游标、不看相邻下标。在结点 `i` 之后插入 `s`，做 `next[s] = next[i]; next[i] = s;`；删除做 `next[i] = next[next[i]];`，被删结点下标挂回备用链表。

> 考察内容：填空题给"数据 + 游标"表，问插入/删除后游标字段的变化序列。

**15. 单链表的遍历局限**：单链表只有指向后继的指针，**从任一结点出发只能向后走，无法向前**；想从中间结点反向遍历全表，只能重新从头开始。要支持双向遍历就得用双向链表（或循环双链表）。顺序表则两边都能随机访问。

> 考察内容：概念判断题，常与"双向链表的优势"一起考。

--- 

## 扩展：隐藏节点与更多实现

链表的核心就是"结点 + 指针"，围绕它还有不少工程变体与技巧值得了解。

### 隐藏节点：哑结点、头结点、哨兵

它们指的是同一类东西——**不存放数据的辅助结点**，作用是让边界情况消失：

| 名称 | 位置 | 作用 |
|------|------|------|
| 哑结点（dummy） | 临时放在表头之前 | 让"插入/删除首元结点"和中间结点用同一段代码 |
| 头结点 | 永久挂在表头之前 | 头指针永远不空，统一空表与非空表的处理 |
| 哨兵（sentinel） | 表头或表尾 | 消除 `NULL` 判断或循环边界判断 |

没有头结点时，删除首元结点要单独写一段"改 head"的代码；有了头结点，`prev->next = cur->next` 对首元结点同样成立。代价是多一个结点、多一次间接访问。判断"要不要带头结点"，就看边界处理是否已经复杂到值得多花这个结点。

### 带尾指针的单链表

普通单链表尾插要走到表尾（$O(n)$）。如果额外维护一个 `tail` 指针，尾插就是 `tail->next = node; tail = node;`，$O(1)$。这就是"用空间换时间"的一行注脚——代价是多存一个指针，好处是尾部操作从 $O(n)$ 降到 $O(1)$。

### 内存池与空闲链表

频繁 `newNode` / `free` 会让 malloc 反复分配与回收小块内存，开销和碎片都不小。工程做法是一次申请一大块结点，自己维护一条**空闲链表**：要结点时从空闲链摘一个，删除时还回去。Linux 内核、游戏引擎、高性能服务器里都很常见。

### 更多链表形态

单向、双向、循环只是最基础的三种。链表其实可以从三个方向继续变：**一个结点挂几条链**（多重链表）、**链上放的是数据还是整张表**（广义表）、**一段连续空间还是单个结点**（块状链表、展开链表）。前面"深入底层"已经讲过展开链表、静态链表、XOR 链表、侵入式链表，这里再补几种常见变体和它们的核心写法。

### 广义表：链表的递归形态

广义表是线性表的推广，元素既可以是原子，也可以是**子表**：`(a, (b, c), d)`。实现的关键是一个 `tag` 区分两种结点，再用 `union` 复用同一片空间：

```c
typedef enum { ATOM, LIST } ElemTag;

typedef struct GLNode {
    ElemTag tag;                 /* 标记这是原子还是子表 */
    union {
        int atom;                /* tag == ATOM：存数据 */
...
`hp` 和 `tp` 就是 Lisp 里的 `head` 与 `tail`：取表头 `GetHead` 是"拿 `hp` 指向的那个元素"，取表尾 `GetTail` 是"拿去掉表头后剩下的表"。广义表的**长度**是顶层元素个数，**深度**是括号嵌套的层数，两个都要递归地数。考点基本都围绕这两者展开。

### 多重链表：一个结点挂多条链

普通链表的结点只有一个 `next`；多重链表的结点带**多个指针域**，同一个结点同时串在多条链上：

```c
typedef struct OLNode {
    int row, col, val;
    struct OLNode *right, *down;   /* 同行、同列各串一条链 */
} OLNode;
```

上面这个结构就是**十字链表**——每个非零元素既在第 `row` 行的链上（`right`），也在第 `col` 列的链上（`down`）。还有：

- **邻接多重表**：无向图的一条边只存一份，两个端点各用一根指针串进自己的边链；
- **孩子兄弟表示法**：树的结点用 `firstChild`（第一条孩子链）和 `nextSibling`（同层兄弟链）表示，一棵多叉树就被改造成了二叉树。

共同点是用**多个指针域换取"同时属于多条链"的能力**。具体应用见 [[H_稀疏矩阵_SparseMatrix|稀疏矩阵]]、[[U_图_Graph|图]]、[[L_树_Tree_BST_AVL|树]] 各章。

### 舞蹈链（DLX）：十字交叉双向循环链表

**舞蹈链**（Dancing Links, DLX）把每个结点上下左右都接起来，形成十字交叉的双向循环链表，用来高效求解精确覆盖问题（数独、N 皇后等）——回溯时"删一列/恢复一列"只动指针，不搬数据。核心就两个函数：

```c
typedef struct Node {
    struct Node *L, *R, *U, *D;    /* 左、右、上、下四个方向 */
} Node;

void remove(Node *c) {             /* 删除第 c 列 */
    c->L->R = c->R;
    c->R->L = c->L;
...
void restore(Node *c) {            /* 恢复第 c 列（与删除顺序相反） */
...
能"删了还能原样长回来"的关键，是 `restore` 严格按 `remove` 的**逆序**操作。这是链表结构里少见的、被算法竞赛反复使用的高级技巧。

### 自组织链表：会"自我调整"的链表

链表的查找是 $O(n)$，但"常查的元素如果能往前挪"，平均代价就会下降。三种经典策略：

| 策略 | 做法 | 特点 |
|------|------|------|
| 前移法 | 找到后直接移到表头 | 最常用，实现简单 |
| 转置法 | 找到后与前驱交换 | 每次只前进一步，更稳 |
| 频度法 | 按访问次数排序 | 需要额外计数域 |

前移法的核心代码——找到 `x` 后把它摘下来、挂到表头：

```c
Node* find_mtf(Node **head, int x) {
    Node *prev = NULL, *cur = *head;
    while (cur && cur->data != x) { prev = cur; cur = cur->next; }
    if (!cur || prev == NULL) return cur;   /* 没找到，或已经在表头 */
    prev->next = cur->next;                 /* ① 从原位置摘下来 */
    cur->next = *head;                      /* ② 挂到表头 */
    *head = cur;
    return cur;
}
...
工程里的 **LRU 缓存**就是前移法的加强版：哈希表负责 $O(1)$ 定位，双向链表负责 $O(1)$ 把它挪到"最近使用"的一端。

### 块状链表：数组与链表的杂交

把长序列切成若干块，**块内用数组、块间用链表**串起来，每块约 $\sqrt{n}$ 个元素：
...
- 按下标访问：沿链找到所在块，再在块内随机访问，$O(\sqrt n)$；
- 插入/删除：块内搬移 $O(\sqrt n)$；块写满就**分裂**成两块，太短就与邻居**合并**。

它和"深入底层"讲过的展开链表长得像，区别在于支持块的分裂与合并，能稳定维护"第 $k$ 个元素"的位置，常用于文本编辑器（rope）和大型序列的中间层。

### 有序链表与循环有序链表

如果建表时始终让结点有序，查找一旦遇到更大的值就能提前停下，合并、去重、求交并差都变得顺手。插入的核心是"找到第一个不小于 `x` 的结点，插到它前面"：

```c
void insert_sorted(Node **head, int x) {
    Node *s = newNode(x);
    while (*head && (*head)->data < x)
        head = &(*head)->next;   /* 二级指针一路递推，省掉 prev */
    s->next = *head;
    *head = s;
}
```

这里的技巧是用 `**head` 沿链递推，不用单独维护 `prev`，插表头和插中间就自然统一了。注意单次插入的时间几乎都花在"找位置"上，是 $O(n)$；把 $n$ 个无序数据建成有序表则是 $O(n^2)$。**循环有序链表**（两个有序循环链表合并后仍成环）常见于操作系统的时间轮与轮询调度。

### 还能用链表做什么

- **LRU 缓存**：哈希表定位 + 双向链表调整顺序，get/put 均 $O(1)$（见 [[P_哈希表_HashTable|哈希表]]）
- **跳表**：给有序链表加多级索引，把查找从 $O(n)$ 降到 $O(\log n)$（见 [[R_跳表_SkipList|跳表]]）
- **邻接表**：图的存储方式之一，每个顶点挂一条边链表（见 [[U_图_Graph|图]]）
- **侵入式链表**：把 `next/prev` 嵌进业务结构体，结点零额外分配（见本章"深入底层"）
