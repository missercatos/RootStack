...
建议先阅读: [[E_容器_Container|容器概览]] — 理解栈的底层存储（数组 vs 链表）和扩容机制。
...
- 扩容策略见 [[E_容器_Container|容器章节]] 的均摊分析
...
flowchart TB
  M["main 帧"]
  F4["factorial(4)  [n=4 | 返回地址 | 等待 4 × f(3)]"]
  F3["factorial(3)  [n=3 | ...]"]
  F2["factorial(2)  [n=2 | ...]"]
  F1["factorial(1) ← 基准情形：直接返回 1"]
  M --> F4 --> F3 --> F2 --> F1
```
...
- **DFS 非递归**：手动用显式栈替代递归，避免系统调用栈的深度限制。详见 [[T_图_Graph|图的 DFS]]
