...
建议先阅读: [[E_容器_Container|容器概览]], [[../算法/算法技巧/递推递归|递推递归]]
...
把一叠无序的考试成绩按从低到高排列——这就是排序。看似简单，但当数据量从 8 个变成 80 亿个时，$O(n^2)$ 的冒泡排序需要 640 亿次操作（约 2 小时），而 $O(n \log n)$ 的快速排序只需约 270 亿次（约 50 分钟）——差距在大数据量下变得致命。排序是计算机科学的基础设施：数据库的 `ORDER BY`、搜索引擎的结果排名、操作系统的进程调度优先级——底层都是排序算法。
