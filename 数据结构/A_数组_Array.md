...
> **本章定位**：数组是顺序存储的载体，以数组实现的线性表称为**顺序表**。线性表 ADT、顺序表插入/删除的平均移动次数推导与 408 算法设计题见 [[C_顺序表_SequentialList|线性表与顺序表]]。
...
动态数组扩容的核心机制在 [[G_容器_Container#vector 扩容机制|容器章节]] 有完整的均摊数学分析，本章 [[#动态数组扩容的均摊分析|深入底层]] 一节从聚合方法和势能法两个角度给出了严格证明。
...
*证明*：对 $i$ 做数学归纳。当 $i=0$ 时，$\text{addr}(arr, 0) = \text{base} + 0 = \text{base}$，成立。假设对 $i=k$ 成立，即 $\text{addr}(arr, k) = \text{base} + k \times \text{sizeof}(T)$。由于数组连续存放，$\text{addr}(arr, k+1) = \text{addr}(arr, k) + \text{sizeof}(T) = \text{base} + (k+1) \times \text{sizeof}(T)$。由归纳法，对所有 $i \ge 0$ 成立。$\square$

**定理（二维行优先寻址公式）**：对于 $m \times n$ 的二维数组 `T arr[m][n]`（行优先存储），元素 `arr[i][j]` 的地址为：
$$\text{addr}(i, j) = \text{base} + (i \times n + j) \times \text{sizeof}(T)$$

*证明*：二维数组 `arr[m][n]` 在内存中被解释为"m 个一维数组，每个长度为 n"——即 `arr` 是长度为 m 的数组，其每个元素 `arr[i]` 是长度为 n 的一维数组。根据 C 标准（§6.5.2.1），`arr[i]` 等价于 `*(arr + i)`，即从 `arr` 起始地址偏移 $i$ 个"长度为 n 的一维数组"。

由引理，每个一维数组占用 $n \times \text{sizeof}(T)$ 字节，因此：
$$\text{arr}[i]\text{的起始地址} = \text{base} + i \times n \times \text{sizeof}(T)$$
...
**推广到 $k$ 维**：对于形状为 $n_1 \times n_2 \times \cdots \times n_k$ 的 $k$ 维数组（最右侧维度变化最快），元素 $(i_1, i_2, \ldots, i_k)$ 的步幅为：

$$\text{stride}_p = \prod_{q=p+1}^{k} n_q$$

即维度 $p$ 的步幅等于其右侧所有维度大小的乘积。这可以归纳证明：假设 $p+1$ 维以上的步幅已正确，则 $i_p$ 每增 1，需要跳过 $n_{p+1} \times n_{p+2} \times \cdots \times n_k$ 个元素，即 $\text{stride}_p = n_{p+1} \cdot \text{stride}_{p+1}$。这是一个递推关系，从最内层 $\text{stride}_k = 1$ 出发逐步展开即可得到乘积公式。
...
三列名字一一对应——C++ `vector`、Python `list`、Java `ArrayList` 本质上是同一个 ADT 的三种方言，实现差异在 [[G_容器_Container|容器章节]] 展开。
...
当矩阵本身带有结构（对称、三角、带状）时，可以把 $O(n^2)$ 个元素压进一维数组存 $O(n)$ 个——而压缩后"矩阵坐标 $(i,j)$ → 一维下标 $k$"的映射，正是本节开头寻址公式的逆向运用。
...
以上三种是"结构规律已知、公式可直接写出"的压缩。当非零元位置完全无规律时（零元占绝大多数），改用三元组顺序表或十字链表记录"（行，列，值）"，见 [[H_稀疏矩阵_SparseMatrix|稀疏矩阵章节]]。
...
动态数组（`std::vector` / `ArrayList`）尾插的均摊 $O(1)$ 不是直觉——它有一个严格的数学证明。

**问题模型**：初始容量为 1 的动态数组，执行 $n$ 次 `push_back`。每次空间不足时，分配 2 倍大小的新数组，将旧元素全部复制过去。

**关键观察**：扩容只在容量为 $1, 2, 4, 8, \ldots, 2^k$（$2^k \le n$）时发生，每次扩容复制的元素数分别为 $1, 2, 4, \ldots, 2^k$ 个。

**均摊分析（聚合方法 / Aggregate Method）**：

$n$ 次插入的总复制次数为等比级数：
$$T(n) = 1 + 2 + 4 + 8 + \cdots + 2^{\lfloor \log_2 n \rfloor} \le 1 + 2 + 4 + \cdots + n \le 2n$$

这是因为 $2^k \le n$，所以 $\sum_{i=0}^{k} 2^i = 2^{k+1} - 1 < 2n$。

加上 $n$ 次常数写入，总代价为 $O(n)$。因此**均摊每次插入**：
$$\frac{T(n)}{n} = \frac{O(n)}{n} = O(1)$$

**更精细的计算**：设 $n = 2^k + r$（$0 \le r < 2^k$），总复制次数为：
$$1 + 2 + 4 + \cdots + 2^{k-1} = 2^k - 1$$

总操作成本 = $n$ 次写入 + $(2^k - 1)$ 次复制 = $n + 2^k - 1 < 2n$。均摊每次操作的实际代价 $< 2$。

**势能法（Potential Method）证明**：

定义势能函数 $\Phi(D_i) = 2 \times \text{size}_i - \text{cap}_i$，其中 $D_i$ 是第 $i$ 次操作后的状态。

- **非扩容操作**（size 未触及 cap）：实际代价 = 1，$\Phi$ 增加 2（size +1，cap 不变），均摊代价 = $1 + 2 = 3$。
- **扩容操作**（cap 翻倍）：实际代价 = $\text{size} + 1$（复制旧元素 + 写入新元素），扩容后 $\text{size}$ 不变而 $\text{cap}$ 翻倍，$\Phi$ 减少 $\text{size}$。均摊代价 = $(\text{size} + 1) + (-\text{size}) = 1$。

每次操作的均摊代价 $\le 3$，与 $n$ 无关，即均摊 $O(1)$。$\square$

**为什么是 2 倍而非其他倍数？** 倍增因子 $\alpha$ 的选择影响均摊常数：
- 扩容为 $\alpha$ 倍时，均摊代价 = $\frac{\alpha}{\alpha - 1}$（复制成本分摊到后续 $\alpha - 1$ 次插入中）
- $\alpha = 2$ 时，均摊代价 = $2$
- $\alpha = 1.5$ 时，均摊代价 = $3$，但内存利用率更高（最多浪费 33% 空间）
- Java `ArrayList` 用 $\alpha = 1.5$，`std::vector` 用 $\alpha = 2$

$\alpha = 2$ 是复制次数和空间浪费的最优折中：均摊常数最小，同时空间浪费不超过 50%。
...
**顺序访问模型**：对于长度为 $N$ 的一维数组，顺序遍历（步长 $s=1$）时，每条 cache line（$L$ 字节）容纳 $\frac{L}{\text{sizeof}(T)}$ 个元素。首次访问该 line 时发生一次 miss，后续 $\frac{L}{\text{sizeof}(T)} - 1$ 次访问全部命中。因此：

$$\text{miss rate}_{\text{sequential}} = \frac{\text{sizeof}(T)}{L}$$

对于 int（4 字节）、64 字节 cache line：$\text{miss rate} = \frac{4}{64} = 6.25\%$，即命中率 $93.75\%$——与上表实验数据吻合。

**随机访问模型**：当访问位置完全随机时，每次访问独立地落在 cache 中 $C$ 个 cache line 的某一条上。若数组占用 $M$ 条 cache line（$M = \frac{N \times \text{sizeof}(T)}{L}$），且 $M \gg C$（数组远大于 cache），则每次访问命中 cache 的概率为 $\frac{C}{M}$，miss rate 为：

$$\text{miss rate}_{\text{random}} = 1 - \frac{C}{M} \approx 1 \quad (M \gg C)$$

即几乎每次访问都是 miss。这就是为什么随机遍历比顺序遍历慢数十倍。

**跨步访问模型**：步长为 $s$ 时，每 $\frac{L}{s \times \text{sizeof}(T)}$ 次访问命中一条 cache line（向下取整）。因此：

$$\text{miss rate}_{\text{stride}} = \frac{s \times \text{sizeof}(T)}{L} \quad (\text{当 } s \times \text{sizeof}(T) \le L)$$

当 $s \times \text{sizeof}(T) > L$ 时，miss rate = 100%。

**二维数组行/列优先对比模型**：对于 $m \times n$ 的 int 数组（4 字节），cache line = 64 字节（16 个 int）：

| 遍历方式 | 步长（元素数） | 步长（字节） | miss rate | 理论加速比 |
|----------|:-----------:|:---------:|:---------:|:--------:|
| 行优先 $\times$ 行优先 | 1 | 4 | 6.25% | 1× |
| 行优先 $\times$ 列优先 | $n$ | $4n$ | $\min(100\%, \frac{4n}{64})$ | — |

当 $n = 1000$ 时，列优先步长 $= 4000$ 字节 $\gg 64$ 字节，miss rate $\approx 100\%$。行优先 miss rate = 6.25%。性能比 $\approx \frac{100\%}{6.25\%} = 16\times$，与实验数据的 20 倍量级一致（差异来自 L2/L3 缓存的缓冲效应）。

**多级缓存下的等效延迟模型**：设 L1 hit rate = $h_1$，L2 hit rate = $h_2$，L3 hit rate = $h_3$，各层延迟为 $t_1, t_2, t_3, t_m$（主存）：

$$t_{\text{eff}} = t_1 + (1 - h_1) \cdot t_2 + (1 - h_1)(1 - h_2) \cdot t_3 + (1 - h_1)(1 - h_2)(1 - h_3) \cdot t_m$$

行优先遍历时 $h_1 \approx 0.94$，$t_{\text{eff}} \approx 5 + 0.06 \times 12 + 0.06 \times 0.1 \times 40 \approx 7.4$ ns。
列优先遍历时 $h_1 \approx 0$，$t_{\text{eff}} \approx 5 + 1 \times 12 + 1 \times 1 \times 40 \approx 57$ ns。
加速比 $\approx \frac{57}{7.4} \approx 7.7\times$（仅 L1-L3，不含主存带宽瓶颈的额外惩罚）。
...
**理想加速比**：设 SIMD 寄存器宽度为 $W$（字节），元素大小为 $S$（字节），则单条 SIMD 指令可并行处理 $P = \frac{W}{S}$ 个元素。对于 $N$ 个元素的数组操作：

$$T_{\text{scalar}} = N \times C_{\text{op}}$$

$$T_{\text{SIMD}} = \frac{N}{P} \times C_{\text{op}} + \text{remainder}$$

其中 $C_{\text{op}}$ 是单个元素操作的时钟周期数，remainder 处理不足 $P$ 个的尾部元素。理想加速比：

$$S = \frac{T_{\text{scalar}}}{T_{\text{SIMD}}} = \frac{N \cdot C_{\text{op}}}{\frac{N}{P} \cdot C_{\text{op}}} = P = \frac{W}{S}$$

| SIMD 指令集 | 寄存器宽度 $W$ | int 加速比 $P$ | double 加速比 $P$ |
|:----------:|:------------:|:-------------:|:---------------:|
...
1. **带宽瓶颈**：当数组操作是内存受限型（如简单拷贝）时，加速比受内存带宽限制而非计算能力。若内存带宽为 $B$ GB/s，数据大小为 $D$ 字节，理论最小时间为 $D / B$，SIMD 无法突破这个下限。

2. **尾部处理开销**：$N$ 不是 $P$ 的整数倍时，尾部 $\text{rem} = N \bmod P$ 个元素需标量处理，开销占比 $\frac{\text{rem}}{N} \approx \frac{1}{N}$（可忽略）。

3. **指令吞吐限制**：现代 CPU 每周期可发射 1-2 条 SIMD 指令（端口限制）。若循环体有 $k$ 条 SIMD 指令，实际加速比为 $\min(P, \frac{\text{ports}}{k})$。

4. **循环展开（Unrolling）**：将循环展开 $U$ 次可减少分支预测开销和指令发射瓶颈：

$$\text{Effective speedup} = \min\left(\frac{W}{S},\ \frac{U \cdot W}{S \cdot k}\right)$$

例如 AVX2（$W=32$）处理 int（$S=4$），展开 4 次、3 条指令：$\min(8, \frac{4 \times 32}{4 \times 3}) = \min(8, 10.7) = 8$——此时计算能力足够，加速比受寄存器宽度限制。
...
> 前面两节回答了"数组是什么、为什么快"。本节回答另一半要求：**在这块连续内存上，有哪些被反复验证过的索引操纵套路**。双指针、滑动窗口、二分、前缀和与差分——它们都不改变存储结构，而是把 $O(1)$ 随机访问这个硬件特性兑换成算法层面的效率跃升。
...
![[difference_core.gif]]
$m$ 次区间修改的成本从 $O(nm)$ 降到 $O(n + m)$。前缀和向高维推广是二维积分图，向动态化推广就是 [[T_树状数组_BIT|树状数组]]与 [[S_线段树_SegmentTree|线段树]]——它们让"边修改边查区间信息"维持在 $O(\log n)$。
...
| 数据结构 | 底层用数组的方式 | 对应章节 |
|---------|-----------------|---------|
| 堆（Heap） | 完全二叉树映射到一维数组，`parent(i) = (i-1)/2`，`left(i) = 2i+1` | [[K_堆_Heap]] |
| 循环队列 | 数组 + 模运算，用 `(head+1) % cap` 绕回 | [[F_队列_Queue]] |
| 哈希表（开放寻址） | 数组存储键值对，探测时线性/二次扫描 | [[P_哈希表_HashTable]] |
| 线段树 | 大小为 4n 的数组，`tree[i]` 保存区间信息 | [[S_线段树_SegmentTree]] |
| 树状数组 | 长度为 n+1 的数组，利用 `lowbit(i) = i & -i` 定位管辖区间 | [[T_树状数组_BIT]] |

堆是最经典的例子——逻辑上是一棵完全二叉树，但在存储层面只是一个数组。父子关系不靠指针而靠下标公式，每一步跳转就是一条 `lea` 指令。详见 [[K_堆_Heap#数组存储|堆 — 数组存储]]。
...
动态数组的扩容实现（含均摊 O(1) 的数学证明）见 [[G_容器_Container|容器章节]]。
...
> 行优先则是 $(4 \times 6 + 3) \times 4 = 108$ → $2108$。两种布局都算一遍正是本题的要点——**行列数不同时，两种优先级的偏移量没有对称关系**。
