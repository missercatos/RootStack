...
动态数组扩容的核心机制在 [[D_容器_Container#vector 扩容机制|容器章节]] 有完整的均摊数学分析，本章 [[#动态数组扩容的均摊分析|深入底层]] 一节从聚合方法和势能法两个角度给出了严格证明。
...
以上三种是"结构规律已知、公式可直接写出"的压缩。当非零元位置完全无规律时（零元占绝大多数），改用三元组顺序表或十字链表记录"（行，列，值）"，见 [[C_稀疏矩阵_SparseMatrix|稀疏矩阵章节]]。
