...
> **本章定位**：数组是顺序存储的载体，以数组实现的线性表称为**顺序表**。线性表 ADT、顺序表插入/删除的平均移动次数推导与 408 算法设计题见 [[C_线性表_LinearList|线性表与顺序表]]。
...
动态数组扩容的核心机制在 [[E_容器_Container#vector 扩容机制|容器章节]] 有完整的均摊数学分析，本章 [[#动态数组扩容的均摊分析|深入底层]] 一节从聚合方法和势能法两个角度给出了严格证明。
...
三列名字一一对应——C++ `vector`、Python `list`、Java `ArrayList` 本质上是同一个 ADT 的三种方言，实现差异在 [[E_容器_Container|容器章节]] 展开。
...
以上三种是"结构规律已知、公式可直接写出"的压缩。当非零元位置完全无规律时（零元占绝大多数），改用三元组顺序表或十字链表记录"（行，列，值）"，见 [[D_稀疏矩阵_SparseMatrix|稀疏矩阵章节]]。
...
![[difference_core.gif]]
$m$ 次区间修改的成本从 $O(nm)$ 降到 $O(n + m)$。前缀和向高维推广是二维积分图，向动态化推广就是 [[S_树状数组_BIT|树状数组]]与 [[R_线段树_SegmentTree|线段树]]——它们让"边修改边查区间信息"维持在 $O(\log n)$。
...
堆是最经典的例子——逻辑上是一棵完全二叉树，但在存储层面只是一个数组。父子关系不靠指针而靠下标公式，每一步跳转就是一条 `lea` 指令。详见 [[J_堆_Heap#数组存储|堆 — 数组存储]]。
...
动态数组的扩容实现（含均摊 O(1) 的数学证明）见 [[E_容器_Container|容器章节]]。
