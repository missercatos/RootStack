...
> 本章的链表、栈、队列都属于**线性结构**；线性表 ADT、顺序表与链表的系统对比、408 考点见 [[C_线性表_LinearList|线性表与顺序表]]。
...
序列式容器的次序由使用者决定；关联式容器的次序由键值关系决定；适配器则干脆把底层容器的部分接口藏起来，只留一种访问姿势。本章覆盖序列式与适配器的原理与实现；关联式的红黑树见 [[K_树_Tree_BST_AVL|树章节]]，哈希表见后续章节。
