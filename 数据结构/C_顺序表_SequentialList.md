...
**6. 具体位置的移动次数（必背公式）**：长度为 $n$ 的顺序表，在第 $i$ 个位置（$1 \le i \le n+1$）**插入**需移动 $n-i+1$ 个元素；**删除**第 $i$ 个元素（$1 \le i \le n$）需移动 $n-i$ 个元素。

| 操作 | 表头 | 表尾 | 一般位置 $i$ |
|------|:----:|:----:|:------------:|
| 插入（合法位置 $1\sim n+1$） | $i=1$：$n$ 个 | $i=n+1$：$0$ 个 | $n-i+1$ |
| 删除（合法位置 $1\sim n$） | $i=1$：$n-1$ 个 | $i=n$：$0$ 个 | $n-i$ |

> 考察内容：填空题直接代入公式；注意"插入能到 $n+1$，删除只能到 $n$"。

**7. 地址计算的正反两用**：正向用 $LOC(a_i) = LOC(a_1) + (i-1)L$ 求任意元素地址；反过来也能用——已知某元素地址反求首地址 $LOC(a_1) = LOC(a_i) - (i-1)L$；已知两个元素的地址差反求元素大小 $L = \dfrac{LOC(a_j) - LOC(a_i)}{j - i}$。三步走：先写下正向公式，再把未知量当未知数解出来，最后代入数值。

> 考察内容：选择题/填空题给首地址与元素大小求某元素地址，或给两元素地址反求首地址、元素大小。

**8. 表尾操作是 $O(1)$（易被"顺序表插删是 $O(n)$"带偏）**：在表尾插入（$i=n+1$）不需要移动任何元素，只写入并让 `length` 加一；删除表尾元素（$i=n$）同理，只需 `length` 减一。只要容量足够，二者都是 $O(1)$。所谓"顺序表插入删除是 $O(n)$"，指的是**平均或一般位置**，表尾是例外。

> 考察内容：判断题/选择题，专门考"顺序表在表尾插入的时间复杂度"。
