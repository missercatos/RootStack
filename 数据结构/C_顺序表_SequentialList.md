...
> **本章定位**：线性表是最基础、最常用的数据结构，也是 408 的第一考点。数组章讲的是"连续内存怎么用"，链表章讲的是"节点怎么串"，而本章回答一个更上位的问题：**线性表这套 ADT 是什么，它的两种实现（顺序表/链表）如何取舍**。读完本章再进入 [[D_链表_LinkedList|链表]]，你会清楚链表到底在"替代"什么。
