...
> 本章的链表、栈、队列都属于**线性结构**；线性表 ADT、顺序表与链表的系统对比、408 考点见 [[C_顺序表_SequentialList|线性表与顺序表]]。
...
序列式容器的次序由使用者决定；关联式容器的次序由键值关系决定；适配器则干脆把底层容器的部分接口藏起来，只留一种访问姿势。本章覆盖序列式与适配器的原理与实现；关联式的红黑树见 [[L_树_Tree_BST_AVL|树章节]]，哈希表见后续章节。
...
本节的完整底层原理见 [[../计算机原理/B_缓存层级|计算机原理 — 缓存层级]] 和 [[../计算机原理/D_内存层次结构|内存层次结构]]。
...
本节的完整底层原理见 [[../操作系统/F_内存管理|操作系统 — 内存管理]] 和 [[../操作系统/G_内存分配器|内存分配器]]。

CPU 缓存之上还有一层**虚拟内存**。进程看到的地址是虚拟地址，OS（操作系统内核）通过页表将其映射到物理内存。

**关键事实**：
- `malloc` 是 C 标准库函数（在 `stdlib.h` 中声明），运行在**用户态**。它内部通过系统调用（`sbrk` / `mmap`）向 OS 内核请求内存。当 `malloc` "分配大内存"时，内核只记录了虚拟地址→空的映射，并未分配物理页框
- 首次访问某个虚拟页时，CPU 触发**缺页中断（page fault）**，内核才从物理内存中分配一个页框（通常 4KB）并建立映射
...
本节底层原理见 [[../计算机原理/A_数据表示#内存对齐|计算机原理 — 数据表示]] 和 [[../计算机原理/B_缓存层级|缓存层级]]。
...
本节的完整展开见 [[../操作系统/G_内存分配器|操作系统 — 内存分配器]]，包括 ptmalloc 架构、sbrk vs mmap、碎片模式。
...
前面讲清了节点存储的缓存劣势，但链表依然是不可替代的结构——任意位置 $O(1)$ 插删、节点地址终生稳定（迭代器不失效）、无需预估容量。它也是手写代码题的第一主角。
...
注意：教材存在两种 top 约定——**指向栈顶元素**（初值 -1，入栈先加后存）与**指向下一个空位**（初值 0，入栈先存后加）。两种约定的 push/pop 语句不同，做题第一步先确认题目采用哪种。
...
- **建堆为何是 O(n)** 而不是直觉上的 $O(n \log n)$：一半节点是叶子根本不用动；倒数第二层的 $\frac{n}{4}$ 个节点最多下沉 1 层，再上层节点数减半而下沉上限加一，求和 $\sum \frac{n}{2^{k}} \cdot O(k) = O(n)$

堆排序、Top-K 问题、多路归并的败者树都建立在这套操作之上，见 [[I_排序_八大排序_Sorting|排序章节]]。
