...
上面三种表达不是孤立存在的。拿到一个具体操作（比如"在第 $i$ 个位置插入一个元素"），规范的做法是按四步走，每一步换一种表达：

| 步骤 | 表达方式 | 要回答的问题 |
|:----:|---------|-------------|
| 1. 语义 | 自然语言 | 这个操作要做什么？输入输出是什么？边界在哪？ |
| 2. 草图 | 图示 | 手工演算一遍：元素怎么动？哪个位置会被覆盖？ |
| 3. 抽象 | 数学公式 | 工作量是多少（比较/移动次数）？复杂度多高？ |
| 4. 代码 | 具体语言 | 翻译成代码，处理边界、内存与返回约定 |

这里的"草图"不一定是真的画出来，也可以是一段文字规划或一张流程图，目的是把思路理清；选哪种形式取决于问题的复杂度。

用一个最小例子完整走一遍：**把数组中的元素循环右移一位**。

**第一步：语义（文字）**

每个元素都向右移动一格，最后一个元素搬到第一个位置，数组长度不变。要注意两点：数组为空或只有一个元素时不用移动；移动过程中不能把还没搬走的元素覆盖掉。
...
从图上能直接看出覆盖风险：如果先把 `a[1]` 写成 `a[0]`，原来的 20 就没了，而它后面还要用。所以要么先把最后一个元素存起来，要么从后往前搬。

**第三步：抽象（公式）**

除首元素外，其余 $n-1$ 个元素各搬一次，最后一个元素额外保存一次：

$$
\text{移动次数} = n - 1, \qquad T(n) = O(n), \qquad S(n) = O(1)
$$
...
void rotate_right(int *a, int n) {
    if (n <= 1) return;          /* 空表或单元素，不用搬 */
    int last = a[n - 1];         /* 先保存最后一个元素 */
    for (int i = n - 1; i > 0; i--)
        a[i] = a[i - 1];         /* 从后往前搬，避免覆盖 */
    a[0] = last;
}
```

写的时候还要注意：`n <= 1` 的提前返回不是可有可无的——不检查的话，`a[n - 1]` 在空数组上就是越界访问。

> 四步不是死流程：简单操作可以只做"语义 + 代码"，复杂算法才需要完整的草图与公式。本教程后面的每个操作都按这条线展开，但不一定每一步都单独写出来。
