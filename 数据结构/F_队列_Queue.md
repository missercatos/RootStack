...
- **缓存友好性**：遍历 deque 时同一 block 内连续命中缓存，跨 block 时大概率 cache miss，整体介于 vector 和 list 之间。详见 [[../计算机原理/B_缓存层级|计算机原理 — 缓存层级]]
...
- **CPU 任务调度**: 就绪队列按 FIFO 分配时间片。详见 [[../操作系统/D_CPU调度|操作系统 — CPU调度]]
