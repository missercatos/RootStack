...
> **考研 408 导引**：本章应试核心是顺序栈/链栈的操作语句、**两栈共享空间**的初始值与栈满条件、**括号匹配**的手算过程、**中缀转后缀**的逐步轨迹与后缀求值；出入栈序列合法性判断（卡塔兰数）为选择题常客。x86-64 调用栈、栈溢出利用、单调栈为超出考纲的增值内容。正文备有 3 组闭卷手算自测。
...
```
main 帧
└─ factorial(4)   [n=4 | 返回地址 | 等待 4 * f(3)]
   └─ factorial(3)   [n=3 | ...]
      └─ factorial(2)   [n=2 | ...]
         └─ factorial(1)  ← 基准情形：直接返回 1
```
...
固定大小的内存区域里，两个栈从两端向中间生长，共享同一段空间——408 选择题的高频模型：
...
**408 手算轨迹**：把 `9+(3-1)*3+10/2` 转为后缀——逐步列出运算符栈与输出区：
...
| 题号 | 题目 | 说明 |
|------|------|------|
| [20](https://leetcode.cn/problems/valid-parentheses/) | 有效的括号 | 栈匹配（见正文括号匹配节） |
| [150](https://leetcode.cn/problems/evaluate-reverse-polish-notation/) | 逆波兰表达式求值 | 后缀表达式 |
| [155](https://leetcode.cn/problems/min-stack/) | 最小栈 | 辅助栈（正文有差值法变体） |
| [739](https://leetcode.cn/problems/daily-temperatures/) | 每日温度 | 单调栈 |
| [921](https://leetcode.cn/problems/minimum-add-to-make-parentheses-valid/) | 括号的有效化 | 计数器退化（呼应自测③） |
| [1047](https://leetcode.cn/problems/remove-all-adjacent-duplicates-in-string/) | 删除字符串中的所有相邻重复项 | 栈消消乐 |
