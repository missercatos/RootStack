...
建议先阅读: [[G_容器_Container|容器概览]], [[E_栈_Stack|栈]]（递归与栈的关系），[[F_队列_Queue|队列]]（层序遍历依赖队列）
...
- **数据库索引**：BST → AVL → 红黑树 → B+树。自平衡 BST 是 B+树的元实现基础。详见 [[M_红黑树_RedBlackTree|红黑树]] 和 [[N_B树_BTree|B树]]
