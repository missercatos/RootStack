...
建议先阅读: [[G_容器_Container|容器概览]], [[L_树_Tree_BST_AVL|树 BST AVL]]（二叉树概念 + 完全二叉树 + 数组表示）
...
- **优先队列**：操作系统任务调度器中按优先级排序的任务列表。Dijkstra 中每次取最近顶点，时间复杂度从 $O(V^2)$ 降至 $O(E \log V)$。详见 [[../操作系统/D_CPU调度|CPU 调度]] 和 [[U_图_Graph|图的最短路径]]
