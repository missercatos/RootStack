...
建议先阅读: [[E_容器_Container|容器概览]], [[T_图_Graph|图]] — 并查集是图连通性判断的基础工具。
...
**与图算法的关系**：并查集是 Kruskal 最小生成树的核心——每加一条边前用 `Find` 检查两端是否已在同一集合，避免成环。详见 [[T_图_Graph#最小生成树|图 — MST]]。
...
- **图的连通性**：Kruskal 最小生成树——检查加入一条边是否会产生环（两端是否已在同一集合）。详见 [[T_图_Graph#最小生成树|图 — MST]]
