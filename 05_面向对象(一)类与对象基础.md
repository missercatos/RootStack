...
>  4. 另外，如果成员变量的内存空间在进入构造函数体之前已经分配好了，初始化列表直接在已分配的内存上构造；而构造函数体智能对已经构造完成的对象做赋值。同时，const/引用成员，在进行函数体内赋值会因为造成语义冲突（const不允许改变值）。
...
> - `this` 指针的链式调用与 const 变体 → [[#2.5 this指针的更多用法]]
> - 浅拷贝 vs 深拷贝的区别与实现 → [[#浅拷贝 vs 深拷贝]]
> - `static` 成员变量和函数的类内/类外规则 → [[#2.6 static成员变量和静态成员函数]]
> - `const` 成员函数中修改 mutable 成员 → [[#2.7 const成员函数、mutable关键字]]
