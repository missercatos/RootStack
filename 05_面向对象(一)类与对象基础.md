...
this指针的使用场景总结：
  ...
  6. 在模板中获取派生类的类型（CRTP 模式，参见 [[08_面向对象(四)运算符重载与友元]]）
...
> 🔗 **友元用于运算符重载的完整讲解见** → [[08_面向对象(四)运算符重载与友元]]，涵盖 `operator<<`、`operator>>`、对称运算符的友元实现、以及流运算符为什么必须用友元而不能用成员函数。
