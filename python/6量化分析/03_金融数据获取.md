# 金融数据获取 (Financial Data APIs)
---

## 📖 章节概述

没有数据就没有量化分析。本章介绍两个核心金融数据获取库：akshare（中国 A 股/基金/期货）和 yfinance（国际市场）。你将学会获取历史 OHLCV 行情、实时行情、基本面数据，以及如何将不同来源的数据标准化为统一的 DataFrame 格式。数据的"形状一致性"是所有量化策略的基石。

> **核心理念**：量化分析的第一步永远是"数据工程"——获取、清洗、对齐、存储。用 C 程序员的思维来理解：数据就位（struct 对齐）后，才能开始计算。Python 的生态优势在于有现成的 API 库帮你完成数据获取，你只需要调用几个函数，就能得到结构化的 DataFrame。但要注意：每个数据源的格式都略有不同，标准化是必须做的工作。

---

### 📚 第一节：akshare —— A 股数据的一站式入口

#### 1.1 安装与基础架构

```bash
pip install akshare --upgrade
```

akshare 提供 500+ 个数据接口，覆盖股票、基金、期货、债券、宏观数据等。所有接口返回 Pandas DataFrame。

#### 1.2 获取 A 股历史行情（OHLCV）

```bash
python -c "
import akshare as ak

# 获取平安银行(000001)的历史日线数据
df = ak.stock_zh_a_hist(
    symbol='000001',
    period='daily',
    start_date='20240101',
    end_date='20241231',
    adjust='qfq'  # 前复权
)
print('Columns:', list(df.columns))
print('Shape:', df.shape)
print(df.head())
print()
print('dtypes:')
print(df.dtypes)
"
```

关键参数说明：

| 参数 | 含义 | 可选值 |
|------|------|--------|
| `symbol` | 股票代码 | 6 位数字字符串，如 `'000001'` |
| `period` | 数据周期 | `daily`, `weekly`, `monthly` |
| `start_date` | 起始日期 | `'YYYYMMDD'` 格式 |
| `end_date` | 结束日期 | `'YYYYMMDD'` 格式 |
| `adjust` | 复权方式 | `''`（不复权）、`'qfq'`（前复权）、`'hfq'`（后复权） |

#### 1.3 获取实时行情

```bash
python -c "
import akshare as ak

# A 股实时行情（全部股票）
# df = ak.stock_zh_a_spot_em()  # 东方财富源
# print(df.columns)
# print(df[['代码', '名称', '最新价', '涨跌幅', '成交量', '成交额']].head())

# 特定股票的实时数据
df = ak.stock_zh_a_hist(symbol='000001', period='daily', start_date='20260801', end_date='20260807')
print(df.tail())
"
```

#### 1.4 获取基金与指数数据

```bash
python -c "
import akshare as ak

# 获取沪深300指数成分股
# df = ak.index_stock_cons(symbol='000300')
# print(df.head())

# 获取指数历史数据
# df = ak.stock_zh_index_daily(symbol='sh000300')
# print(df.tail())

# 获取基金历史净值
# df = ak.fund_open_fund_info_em(symbol='000001', indicator='单位净值走势')
# print(df.head())
print('akshare provides 500+ data interfaces for Chinese markets')
"
```

#### 1.5 财报数据获取

```bash
python -c "
import akshare as ak

# 获取某股票的资产负债表
# df = ak.stock_financial_balance_sheet_by_report_em(symbol='000001')
# print(df.columns)

# 获取利润表
# df = ak.stock_financial_profit_sheet_by_report_em(symbol='000001')
# print(df.columns)
print('Financial statements available via akshare EM interfaces')
"
```

---

### 📚 第二节：yfinance —— 国际市场数据

#### 2.1 安装与基础用法

```bash
pip install yfinance --upgrade
```

yfinance 通过 Yahoo Finance API 获取全球市场的股票、ETF、指数、外汇、加密货币数据。

```bash
python -c "
import yfinance as yf

# 下载苹果公司历史数据
aapl = yf.download('AAPL', start='2024-01-01', end='2024-12-31')
print('Columns:', list(aapl.columns))
print('Shape:', aapl.shape)
print(aapl.tail())
"
```

#### 2.2 多股票同时下载

```bash
python -c "
import yfinance as yf

# 同时下载多只股票
tickers = ['AAPL', 'GOOGL', 'MSFT', 'TSLA']
data = yf.download(tickers, start='2024-01-01', end='2024-06-30')
print('Shape:', data.shape)  # Multi-level columns
print('Column levels:', data.columns.names)
print()
# 只看收盘价
print('Close prices:')
print(data['Close'].tail())
"
```

#### 2.3 获取基本面数据

```bash
python -c "
import yfinance as yf

msft = yf.Ticker('MSFT')

# 基本信息
info = msft.info
print('Market Cap:', info.get('marketCap'))
print('PE Ratio:', info.get('trailingPE'))
print('Sector:', info.get('sector'))

# 获取财务报表
# balance_sheet = msft.balance_sheet
# income_stmt = msft.financials
# cashflow = msft.cashflow
"
```

#### 2.4 获取分红和拆股数据

```bash
python -c "
import yfinance as yf

aapl = yf.Ticker('AAPL')
dividends = aapl.dividends
splits = aapl.splits

print('Recent dividends:')
print(dividends.tail())
print()
print('Stock splits:')
print(splits)
"
```

---

### 📚 第三节：数据标准化 —— 统一不同数据源的格式

#### 3.1 为什么需要标准化

不同数据源的列名、日期格式、复权方式各有不同。写策略之前必须将所有输入标准化为统一的格式：

```bash
python -c "
import pandas as pd
import numpy as np

# 定义标准 OHLCV 列名映射
COLUMN_MAP = {
    'open':         ['open', 'Open', 'OPEN', '开盘'],
    'high':         ['high', 'High', 'HIGH', '最高'],
    'low':          ['low', 'Low', 'LOW', '最低'],
    'close':        ['close', 'Close', 'CLOSE', '收盘'],
    'volume':       ['volume', 'Volume', 'VOLUME', '成交量'],
    'date':         ['date', 'Date', '日期', 'trade_date'],
    'symbol':       ['symbol', '代码', '股票代码'],
    'adj_factor':   ['adj_factor', '复权因子'],
}

def standardize_columns(df, symbol=None):
    # 列名小写化
    df.columns = [c.lower().strip() for c in df.columns]
    # 映射到标准列名
    rename = {}
    for std_name, aliases in COLUMN_MAP.items():
        for alias in aliases:
            if alias.lower() in df.columns:
                rename[alias.lower()] = std_name
    df = df.rename(columns=rename)
    if 'symbol' not in df.columns and symbol:
        df['symbol'] = symbol
    return df

print('Column standardization function defined')
"
```

#### 3.2 日期格式统一化

```bash
python -c "
import pandas as pd

# 不同数据源的日期格式
samples = ['2024-01-15', '20240115', '2024/01/15', '2024年01月15日']

for s in samples:
    try:
        dt = pd.to_datetime(s)
        print(f'{s:20s} -> {dt.strftime(\"%Y-%m-%d\")}')
    except Exception as e:
        print(f'{s:20s} -> ERROR: {e}')
"
```

#### 3.3 复权处理标准化

```bash
python -c "
import numpy as np
import pandas as pd

# 如果数据源只提供不复权价格，手动计算复权
# 复权因子通常是累计的
def apply_adjustment(df):
    '''
    使用复权因子将不复权价格转换为前复权价格
    '''
    if 'adj_factor' not in df.columns:
        # 若无复权因子，以后复权因子为 1.0
        df['adj_factor'] = 1.0
    
    # 前复权：当前价格不变，历史价格按累计因子调整
    last_factor = df['adj_factor'].iloc[-1]
    for col in ['open', 'high', 'low', 'close']:
        if col in df.columns:
            df[col] = df[col] * df['adj_factor'] / last_factor
    return df

print('Adjustment function defined')
print('Forward adjustment: historical prices adjusted down, latest unchanged')
"
```

#### 3.4 数据完整性检查

```bash
python -c "
import pandas as pd
import numpy as np

def validate_ohlcv(df):
    checks = {}
    
    # 基本检查
    checks['has_columns'] = all(
        c in df.columns for c in ['open', 'high', 'low', 'close', 'volume']
    )
    checks['no_nan'] = df[['open', 'high', 'low', 'close', 'volume']].notna().all().all()
    checks['high_ge_low'] = (df['high'] >= df['low']).all()
    checks['high_ge_open'] = (df['high'] >= df['open']).all()
    checks['high_ge_close'] = (df['high'] >= df['close']).all()
    checks['low_le_open'] = (df['low'] <= df['open']).all()
    checks['low_le_close'] = (df['low'] <= df['close']).all()
    checks['non_negative'] = (df[['open', 'high', 'low', 'close', 'volume']] >= 0).all().all()
    checks['sorted_by_date'] = df['date'].is_monotonic_increasing
    
    all_passed = all(checks.values())
    for check, result in checks.items():
        status = 'OK' if result else 'FAIL'
        print(f'  [{status}] {check}')
    
    return all_passed

print('Validation function defined')
"
```

---

### 📚 第四节：数据本地存储 —— HDF5 与 Parquet

#### 4.1 为什么本地存储

量化回测会反复读取历史数据。每次都从网络获取不仅慢，而且不稳定。把数据缓存在本地才是正道。

#### 4.2 HDF5 存储（Pandas 原生支持）

```bash
python -c "
import pandas as pd
import numpy as np
import os

# 创建示例数据
np.random.seed(42)
dates = pd.date_range('2024-01-01', periods=100, freq='B')
df = pd.DataFrame({
    'open':  100 + np.cumsum(np.random.randn(100) * 2),
    'high':  100 + np.cumsum(np.random.randn(100) * 2),
    'low':   100 + np.cumsum(np.random.randn(100) * 2),
    'close': 100 + np.cumsum(np.random.randn(100) * 2),
    'volume': np.random.randint(1e6, 5e6, 100),
}, index=dates)

# 写入 HDF5
h5_path = '/tmp/market_data.h5'
df.to_hdf(h5_path, key='AAPL', mode='w')
print(f'Data written to {h5_path}')

# 读取
df_loaded = pd.read_hdf(h5_path, key='AAPL')
print(f'Loaded {len(df_loaded)} rows')

# 文件大小
print(f'File size: {os.path.getsize(h5_path)} bytes')
"
```

#### 4.3 Parquet 格式（列式存储，更快）

```bash
python -c "
import pandas as pd
import numpy as np
import os

np.random.seed(42)
dates = pd.date_range('2024-01-01', periods=1000, freq='B')
df = pd.DataFrame({
    'symbol': ['AAPL'] * 1000,
    'date': dates,
    'close': 150 + np.cumsum(np.random.randn(1000) * 2),
    'volume': np.random.randint(1e6, 1e7, 1000),
})

parquet_path = '/tmp/market_data.parquet'
df.to_parquet(parquet_path, index=False)
loaded = pd.read_parquet(parquet_path)
print(f'Parquet: {os.path.getsize(parquet_path)} bytes, {len(loaded)} rows')
"
```

存储格式对比：

| 格式 | 压缩率 | 读取速度 | Pandas 支持 | 跨语言支持 |
|------|--------|---------|------------|-----------|
| CSV | 差 | 慢 | ✅ | ✅✅ |
| HDF5 | 好 | 快 | ✅ | 一般 |
| Parquet | 极好 | 极快 | ✅ (pyarrow) | ✅✅ |
| Feather | 中 | 极快 | ✅ (pyarrow) | ✅ |

---

### 📝 小节练习

> [!question] 选择题 1
> akshare 的 `adjust='qfq'` 参数表示什么？
> - [ ] A. 不进行复权
> - [ ] B. 前复权
> - [ ] C. 后复权
> - [ ] D. 除权除息日调整
>
> > [!success]- 点击查看答案
> > 正确答案: B
> > **解析**: `qfq` = "前复权"，使得最新价格不变，历史价格按照累计的送股/分红因子向下调整。这样历史价格和最新价格在比较时具有连续性。`hfq` = 后复权，保持历史价格不变。

> [!question] 选择题 2
> yfinance 下载多只股票数据时，返回的 DataFrame 的 columns 是什么结构？
> - [ ] A. 单层列名
> - [ ] B. 多层列名（MultiIndex），第一层是 OHLCV，第二层是股票代码
> - [ ] C. 扁平列名如 'AAPL_Close'
> - [ ] D. 随机结构
>
> > [!success]- 点击查看答案
> > 正确答案: B
> > **解析**: yfinance 使用 MultiIndex 列结构。例如 `data['Close']['AAPL']` 需要两级索引。访问方法是 `data[('Close', 'AAPL')]` 或 `data.xs('AAPL', level=1, axis=1)`。

---

## 📋 章节测试

### 一、判断题

> [!question] 判断题 1
> akshare 所有接口返回的都是 Pandas DataFrame 格式。 （ ）
> - [ ] ✅ 正确
> - [ ] ❌ 错误
>
> > [!success]- 点击查看答案
> > 答案: 正确
> > **解析**: akshare 的设计原则之一就是所有接口返回 DataFrame，保持一致的 API 风格。

> [!question] 判断题 2
> yfinance 的数据来源于雅虎财经，可以免费用于个人量化分析。 （ ）
> - [ ] ✅ 正确
> - [ ] ❌ 错误
>
> > [!success]- 点击查看答案
> > 答案: 正确
> > **解析**: Yahoo Finance 提供免费的历史数据（有一定速率限制），适合个人学习和研究使用。商业用途需要确认许可条款。

> [!question] 判断题 3
> 股票复权是为了修正除权除息（送股、分红）对价格造成的跳空影响。 （ ）
> - [ ] ✅ 正确
> - [ ] ❌ 错误
>
> > [!success]- 点击查看答案
> > 答案: 正确
> > **解析**: 除权除息会导致股价出现"断层"（如 10 送 10 后股价腰斩），使用复权价格可以还原股价的真实涨跌，确保收益率计算的连续性。

> [!question] 判断题 4
> Parquet 格式是一种行式存储格式，适合随机访问单行数据。 （ ）
> - [ ] ✅ 正确
> - [ ] ❌ 错误
>
> > [!success]- 点击查看答案
> > 答案: 错误
> > **解析**: Parquet 是列式存储格式，将同一列的所有数据连续存储在一起。这使得列扫描（如计算某列均值）极快，且列级别的压缩率更高。

> [!question] 判断题 5
> 获取实时行情数据时，akshare 使用的是 WebSocket 流式推送。（ ）
> - [ ] ✅ 正确
> - [ ] ❌ 错误
>
> > [!success]- 点击查看答案
> > 答案: 错误
> > **解析**: akshare 通过 HTTP 请求抓取网页数据（如东方财富、新浪财经），返回的是请求时刻的快照，不是持续的 WebSocket 推送。

> [!question] 判断题 6
> `pd.to_datetime()` 可以自动解析多种日期格式。 （ ）
> - [ ] ✅ 正确
> - [ ] ❌ 错误
>
> > [!success]- 点击查看答案
> > 答案: 正确
> > **解析**: `pd.to_datetime()` 非常智能，可以处理 `'2024-01-15'`, `'20240115'`, `'2024/01/15'` 等多种常见格式。遇到特殊格式时，可以用 `format='%Y%m%d'` 显式指定。

---

### 二、选择题

> [!question] 选择题 1
> akshare 获取股票历史数据时，以下哪个参数控制数据频率？
> - [ ] A. `freq`
> - [ ] B. `interval`
> - [ ] C. `period`
> - [ ] D. `timeframe`
>
> > [!success]- 点击查看答案
> > 正确答案: C
> > **解析**: akshare 使用 `period` 参数控制数据频率，可选值包括 `'daily'`, `'weekly'`, `'monthly'`。

> [!question] 选择题 2
> 以下哪种文件格式对量化回测的数据缓存在速度和压缩率之间平衡最好？
> - [ ] A. JSON
> - [ ] B. CSV
> - [ ] C. Parquet
> - [ ] D. XML
>
> > [!success]- 点击查看答案
> > 正确答案: C
> > **解析**: Parquet 提供列式存储 + 内置压缩，对量化常见的列扫描操作极快。CSV 是所有格式中最慢的，JSON/XML 是文本格式，不适合大量数值数据。

> [!question] 选择题 3
> `yf.Ticker('AAPL').history(period='1mo')` 获取的数据时间范围是？
> - [ ] A. 最近一天
> - [ ] B. 最近一个月
> - [ ] C. 最近一年
> - [ ] D. 全部历史
>
> > [!success]- 点击查看答案
> > 正确答案: B
> > **解析**: `period='1mo'` 获取最近一个月的数据。可选值：`'1d'`, `'5d'`, `'1mo'`, `'3mo'`, `'6mo'`, `'1y'`, `'2y'`, `'5y'`, `'10y'`, `'ytd'`, `'max'`。

> [!question] 选择题 4
> 以下哪种操作是数据标准化流程的正确步骤？
> - [ ] A. 直接使用各数据源的原始列名，让策略代码适配每个数据源
> - [ ] B. 在获取数据后，统一映射为标准列名和格式
> - [ ] C. 修改数据源的输出格式
> - [ ] D. 不使用第三方库，自己写爬虫获取数据
>
> > [!success]- 点击查看答案
> > 正确答案: B
> > **解析**: 在获取数据后立即标准化是最佳实践。这样策略代码只依赖一种格式，切换数据源时只需修改适配层，不需要改动策略逻辑。

> [!question] 选择题 5
> 股票代码 `'000001'` 在 akshare 中属于哪个板块？
> - [ ] A. 上证主板
> - [ ] B. 深证主板
> - [ ] C. 创业板
> - [ ] D. 科创板
>
> > [!success]- 点击查看答案
> > 正确答案: B
> > **解析**: A 股代码规则：`000xxx` 和 `001xxx` 是深证主板，`002xxx` 是中小板，`300xxx` 是创业板，`600xxx`/`601xxx`/`603xxx` 是上证主板，`688xxx` 是科创板。

> [!question] 选择题 6
> HDF5 文件中的 `key` 参数用于什么？
> - [ ] A. 加密密钥
> - [ ] B. 数据集标识符（类似文件系统中的路径）
> - [ ] C. 二级制编码格式
> - [ ] D. 版本号
>
> > [!success]- 点击查看答案
> > 正确答案: B
> > **解析**: HDF5 的组织结构类似文件系统，`key` 是数据集在 HDF5 文件中的路径。例如 `key='stocks/AAPL'` 将数据存储在 `/stocks/AAPL` 路径下。一个 HDF5 文件可以存储多个数据集。

---

### 🛠️ 动手练习题

> [!example] 练习题 1：构建本地股票数据库
> **难度**: ⭐⭐
>
> 用 akshare 下载沪深 300 指数全部成分股的历史日线数据（至少 2024 全年），逐只写入 HDF5 文件。编写一个函数 `load_stock(h5_path, symbol)` 从 HDF5 读取指定股票的行情数据并返回标准化格式。

> [!example] 练习题 2：多市场数据对比
> **难度**: ⭐⭐
>
> 用 yfinance 下载 `AAPL` 的历史数据，用 akshare 下载任意一只港股或 A 股的数据。将两者标准化为相同的列名格式，计算它们的 60 日滚动波动率，用 Matplotlib 绘制对比图。

> [!example] 练习题 3：数据质量检查脚本
> **难度**: ⭐⭐⭐
>
> 编写一个完整的数据完整性检查函数，对 OHLCV 数据检查：
> 1. 日期是否连续（没有缺失交易日）
> 2. high >= max(open, close)，low <= min(open, close)
> 3. 价格和成交量没有负数
> 4. 没有 NaN
> 5. 异常值检测（涨跌超过 20% 标记为警告）
>
> 对有问题的数据行输出详细的错误报告。
