...
> [!question] 选择题 1
> `fig, axes = plt.subplots(2, 3)` 返回的 `axes` 类型是？
> - [ ] A. Python list
> - [ ] B. Python tuple
> - [ ] C. NumPy ndarray (2×3)
> - [ ] D. 单个 Axes 对象
...
> > [!success]- 点击查看答案
> > > 正确答案: C
> > > **解析**: 当 `nrows > 1` 且 `ncols > 1` 时，`axes` 是一个 `(nrows, ncols)` 的 NumPy ndarray，每个元素是对应的 `Axes` 对象。一行或一列时，`axes` 是一维数组。
...
> [!question] 选择题 1
> `ax2 = ax1.twinx()` 做了什么？
> - [ ] A. 复制 `ax1` 的所有内容到一个新坐标系
> - [ ] B. 创建一个与 `ax1` 共享 X 轴的新坐标系（独立 Y 轴）
> - [ ] C. 创建一个与 `ax1` 共享 Y 轴的新坐标系（独立 X 轴）
> - [ ] D. 将两个坐标系合并为一个
>
> > [!success]- 点击查看答案
> > > 正确答案: B
> > > **解析**: `twinx()` 创建一个新的 `Axes` 对象，它与原 `Axes` 共享 X 轴，但拥有独立的 Y 轴（显示在右侧）。对应的 `twiny()` 共享 Y 轴，独立 X 轴。
...
> [!question] 选择题 1
> 以下哪个 colormap 最适合展示有正负对称的数据（如相关系数矩阵）？
> - [ ] A. `viridis`
> - [ ] B. `plasma`
> - [ ] C. `coolwarm`
> - [ ] D. `binary`
...
> > [!success]- 点击查看答案
> > > 正确答案: C
> > > **解析**: `coolwarm`（蓝-白-红）和 `RdBu` 是发散型（diverging）colormap，以中间值为中心向两端渐变，适合展示有正负两极的数据。`viridis` 和 `plasma` 是序数型，适合展示从低到高的连续值。

> [!question] 选择题 2
> `ax.annotate()` 和 `ax.text()` 的主要区别是？
> - [ ] A. 没有区别
> - [ ] B. `annotate` 支持箭头指向特定数据点，`text` 仅放置纯文本
> - [ ] C. `annotate` 支持数学公式，`text` 不支持
> - [ ] D. `annotate` 自动换行，`text` 不能换行
>
> > [!success]- 点击查看答案
> > > 正确答案: B
> > > **解析**: `annotate()` 的核心功能是**带箭头的注释**：`xy` 是被指向的数据点，`xytext` 是文本位置，`arrowprops` 定义箭头样式。`text()` 只在指定坐标放置纯文本，不包含指向功能。
...
> [!question] 选择题 1
> `dtype=[('algo', 'U10'), ('n', int), ('time', float)]` 中的 `'U10'` 表示？
> - [ ] A. 无符号整数，最大 10
> - [ ] B. Unicode 字符串，最大 10 个字符
> - [ ] C. 未知类型
> - [ ] D. URL 类型
>
> > [!success]- 点击查看答案
> > > 正确答案: B
> > > **解析**: `U10` 是 NumPy 中 Unicode 字符串类型，`10` 表示最大字符数。相当于 C 中的 `char algo[11]`（多一个 `\0` 的空间）。结构化数组的每一列有独立的名称和类型。
...
> [!question] 选择题 1
> `plt.subplots(1, 4)` 返回的 `axes` 如何访问第三个子图？
> - [ ] A. `axes[2, 0]`
> - [ ] B. `axes[0, 2]`
> - [ ] C. `axes[2]`
> - [ ] D. `axes[3]`
...
> > [!success]- 点击查看答案
> > > 正确答案: C
> > > **解析**: 当只有一行时，`axes` 是一维数组（长度为列数），所以第三个子图（索引从 0 开始）是 `axes[2]`。双层索引 `axes[row, col]` 仅在二维数组时可用。

> [!question] 选择题 2
> 以下哪种 colormap 是 Matplotlib 3.x 的默认 colormap？
> - [ ] A. `jet`
> - [ ] B. `gray`
> - [ ] C. `viridis`
> - [ ] D. `hot`
...
> > [!success]- 点击查看答案
> > > 正确答案: C
> > > **解析**: Matplotlib 2.0+ 将默认 colormap 从有缺陷的 `jet` 改为感知均匀的 `viridis`。`viridis` 在灰度打印和色盲友好方面都优于 `jet`。推荐用于大多数科学图表。

> [!question] 选择题 3
> 要在图中同时展示"响应时间（毫秒级）"和"吞吐量（万次/秒）"，最佳方案是？
> - [ ] A. 两个独立的子图，分别绑制
> - [ ] B. 使用 `twinx()` 双 Y 轴
> - [ ] C. 将两组数据归一化到同一尺度
> - [ ] D. 只展示其中一个
...
> > [!success]- 点击查看答案
> > > 正确答案: B
> > > **解析**: 当两组数据量级不同但共享 X 轴时，`twinx()` 是最佳方案。它在同一张图上提供两个 Y 轴（左轴和右轴），可以用不同颜色区分两组数据，节省空间且便于对比。

> [!question] 选择题 4
> `ax.grid(True, which='major', linestyle='--', alpha=0.5)` 中 `which='major'` 的含义是？
> - [ ] A. 只显示主刻度位置的网格线
> - [ ] B. 只显示最重要的网格线
> - [ ] C. 网格线比通常更粗
> - [ ] D. 显示所有可能位置的网格线
...
> > [!success]- 点击查看答案
> > > 正确答案: A
> > > **解析**: Matplotlib 的刻度分为 `major`（主刻度）和 `minor`（次刻度）。`which='major'` 表示只在主刻度位置画网格线。可以分别设置两者的样式：`ax.grid(True, which='major', ...)` 和 `ax.grid(True, which='minor', ...)`。

> [!question] 选择题 5
> `fig.tight_layout()` 和 `fig.subplots_adjust()` 的关系是？
> - [ ] A. 完全相同
> - [ ] B. `tight_layout` 是自动调整，`subplots_adjust` 是手动指定间距
> - [ ] C. `subplots_adjust` 内部调用 `tight_layout`
> - [ ] D. 互斥，不能同时使用
>
> > [!success]- 点击查看答案
> > > 正确答案: B
> > > **解析**: `tight_layout()` 是**自动**计算最优间距以避免标签重叠。`subplots_adjust(left=0.1, right=0.9, wspace=0.3)` 是**手动**指定精确的间距值。当自动计算不够理想时，可以回退到手动调整。

> [!question] 选择题 6
> 哪种文件格式最适合出版级图表（需要嵌入 LaTeX 论文）？
> - [ ] A. PNG (150 dpi)
> - [ ] B. JPG (quality=95)
> - [ ] C. PDF 或 EPS
> - [ ] D. SVG (interactive)
...
> > [!success]- 点击查看答案
> > > 正确答案: C
> > > **解析**: PDF 和 EPS（Encapsulated PostScript）是科学出版的标准矢量格式。LaTeX 可以直接嵌入 PDF/EPS，图表文字与论文正文使用相同字体，且无限缩放不失真。PNG/JPG 是位图，放大会模糊。
