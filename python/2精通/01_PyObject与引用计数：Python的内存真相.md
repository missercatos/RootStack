...
> [!question] 选择题 1
> CPython 中每个对象都包含哪两个必须字段？
> - [ ] A. ob_value 和 ob_name
> - [ ] B. ob_refcnt 和 ob_type
> - [ ] C. ob_size 和 ob_data
> - [ ] D. ob_malloc 和 ob_free
>
> > [!success]- 点击查看答案
> > 正确答案: B
> > **解析**: `PyObject` 结构体包含 `ob_refcnt`（引用计数）和 `ob_type`（类型指针）。这是所有 Python 对象的"头部"，无论何种类型都继承自它。

> [!question] 选择题 2
> Python 的 `int` 大小为 28 字节而 C 的 `int` 为 4 字节，多出的空间主要用于？
> - [ ] A. 随机填充
> - [ ] B. 类型信息和引用计数等对象元数据
> - [ ] C. 保存英文注释
> - [ ] D. 防止缓冲区溢出
...
> > [!success]- 点击查看答案
> > 正确答案: B
> > **解析**: Python int 是堆上的 `PyLongObject` 结构体，包含 PyObject 头部（引用计数+类型指针）以及支持任意精度整数的数据结构。C int 只是裸值。
...
> [!question] 选择题 1
> 以下代码执行后，列表 `[1,2,3]` 的引用计数是多少（不计算 getrefcount 的临时引用）？
> ```python
> import sys
> a = [1, 2, 3]
> b = a
...
> > [!success]- 点击查看答案
> > 正确答案: B
> > **解析**: `a` 和 `c` 各持有一次引用（`b` 已删除），所以引用计数为 2。
...
> [!question] 选择题 1
> CPython 的垃圾回收器采用什么策略来降低性能开销？
> - [ ] A. 每次都扫描所有对象
> - [ ] B. 分代回收 — 高频回收年轻对象，低频回收老对象
> - [ ] C. 使用独立线程在后台回收
> - [ ] D. 向操作系统请求回收通知
>
> > [!success]- 点击查看答案
> > 正确答案: B
> > **解析**: 分代回收基于"大多数对象很快死亡"的经验假设。gen0 高频回收（新对象），gen2 低频回收（长久存在的对象），减少扫描开销。
...
> [!question] 选择题 1
> 以下哪个操作在 Python 中最接近 C 数组的连续内存访问效率？
> - [ ] A. 普通 list 遍历
> - [ ] B. 使用 `array.array('i', ...)`
> - [ ] C. 使用 dict 遍历
> - [ ] D. 使用 set 遍历
...
> > [!success]- 点击查看答案
> > 正确答案: B
> > **解析**: `array.array` 模块存储的是 C 类型的连续裸值（而非 PyObject*），内存布局接近 C 数组，访问效率明显高于 list。
...
> [!question] 选择题 1
> `sys.getrefcount(obj)` 返回 3，实际引用计数是多少？
> - [ ] A. 1
> - [ ] B. 2
> - [ ] C. 3
> - [ ] D. 4
...
> > [!success]- 点击查看答案
> > 正确答案: B
> > **解析**: `getrefcount()` 的参数是一个临时引用，使结果比实际多 1。返回 3 意味着实际有 2 个引用。

> [!question] 选择题 2
> 以下哪种情况会导致循环引用？
> - [ ] A. `a = []; b = a`
> - [ ] B. `a = []; b = []; a.append(b)`
> - [ ] C. `a = []; b = []; a.append(b); b.append(a)`
> - [ ] D. `a = []; b = []; del a; del b`
>
> > [!success]- 点击查看答案
> > 正确答案: C
> > **解析**: 需要双向引用才能形成循环。A 是别名（不循环），B 只有 a→b 单向引用，C 中 a↔b 形成闭环。

> [!question] 选择题 3
> CPython 分代 GC 的 gen0 阈值默认为多少？
> - [ ] A. 100
> - [ ] B. 500
> - [ ] C. 700
> - [ ] D. 1000
...
> > [!success]- 点击查看答案
> > 正确答案: C
> > **解析**: 默认阈值为 `(700, 10, 10)`，表示 gen0 对象数超过 700 时触发 gen0 回收；gen0 回收 10 次后触发 gen1 回收；gen1 回收 10 次后触发 gen2 回收。

> [!question] 选择题 4
> 以下哪个操作会直接导致 `ob_refcnt` 减 1？
> - [ ] A. `a = 1`
> - [ ] B. `print(a)`
> - [ ] C. `del a`
> - [ ] D. `type(a)`
...
> > [!success]- 点击查看答案
> > 正确答案: C
> > **解析**: `del a` 移除名字 `a` 的绑定，使对应对象的引用计数减 1。`a = 1` 创建新引用（refcnt +1），`print(a)` 和 `type(a)` 是只读访问。

> [!question] 选择题 5
> Python 的 int 大小（28 bytes）与 C 的 int 大小（4 bytes）相比，主要原因是？
> - [ ] A. Python 使用 UTF-8 编码存储数字
> - [ ] B. Python int 包含对象元数据和任意精度支持
> - [ ] C. Python int 以大端序存储
> - [ ] D. Python int 包含调试信息
>
> > [!success]- 点击查看答案
> > 正确答案: B
> > **解析**: Python int 是堆上的 `PyLongObject` 结构体，包含 PyObject 头部（refcnt + type*）、数字位数信息和实际数字数组。它还支持任意精度（可表示远超 64 位的整数）。

> [!question] 选择题 6
> 以下哪种方式可以安全地引用一个对象而不阻止其被回收？
> - [ ] A. 赋值给全局变量
> - [ ] B. 存入 list 的最后一个元素
> - [ ] C. 使用 `weakref.ref()`
> - [ ] D. 使用 `ctypes.c_void_p()`
>
> > [!success]- 点击查看答案
> > 正确答案: C
> > **解析**: `weakref.ref()` 创建弱引用，不增加引用计数。全局变量和 list 中的引用都是强引用，会阻止回收。

> [!question] 选择题 7
> `a is b` 返回 True 的含义是？
> - [ ] A. a 和 b 的值相等
> - [ ] B. a 和 b 的类型相同
> - [ ] C. a 和 b 是同一个对象（内存地址相同）
> - [ ] D. a 和 b 的哈希值相同
>
> > [!success]- 点击查看答案
> > 正确答案: C
> > **解析**: `is` 运算符比较对象的身份（内存地址），即 `id(a) == id(b)`。`==` 比较值是否相等。
...
> > [!success]- 点击查看答案
> > 正确答案: B
> > **解析**: `del obj` 后强引用消失，对象被回收。弱引用 `ref()` 此时返回 `None`，表示目标已不存在。
