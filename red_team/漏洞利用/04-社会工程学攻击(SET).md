...
```mermaid
sequenceDiagram
 participant A as 攻击机 (SET)
 participant M as 邮件服务器
 participant V as 受害者
 participant C2 as C2 (Metasploit)

 Note over A: Phase 1: 准备攻击
 A->>A: 1. Spear-Phishing Attack Vectors
 A->>A: 2. 创建钓鱼邮件模板
 A->>A: 3. 克隆目标网站(Gmail/OWA/企业VPN)

 Note over A,V: Phase 2: 投递
 A->>M: 发送钓鱼邮件 (含恶意链接)
 M->>V: 投递邮件到收件箱

 Note over V: Phase 3: 交互
 V->>V: 打开邮件，点击链接
 V->>A: 访问克隆网站并输入凭据

 Note over A: Phase 4: 收集与利用
 A->>A: 记录用户名+密码 (harvester)
 A->>V: (可选的) 重定向到真实网站
 A->>A: 凭据验证，整理报告

 Note over A,C2: 进阶: Meterpreter
 V->>A: 访问含 exploit 的页面
 A-->>V: 投递 payload (Java/Browser Exploit)
 V-->>C2: Meterpreter reverse_tcp 回连
```
...
> 相关模块: [[01-漏洞利用框架精通|Metasploit 框架]] | [[03-路由器与IoT漏洞利用|路由器/IoT 利用]] | [[../渗透基础/04-漏洞评估与利用基础|漏洞利用基础]]
...
```bash
# 安装 setoolkit：用包管理器或从 GitHub 获取（或以 root 账号启动）
sudo setoolkit
