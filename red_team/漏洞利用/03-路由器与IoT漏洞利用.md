...
```mermaid
flowchart TB
 subgraph Phase1[Phase 1: 侦察与发现]
 A1[Nmap 端口扫描<br/>80,443,8080,23,22] --> A2[HTTP 指纹<br/>管理界面识别]
 A2 --> A3[确定厂商和型号]
 end

 subgraph Phase2[Phase 2: 漏洞利用 — rsf]
 B1[use scanners/autopwn<br/>综合漏洞扫描] --> B2{发现漏洞?}
 B2 -->|是| B3[use exploits/routers/厂商/型号<br/>执行对应的 exploit]
 B2 -->|否| B4[use creds/routers/厂商/ssh_default<br/>尝试默认凭证]
 end

 subgraph Phase3[Phase 3: 后利用]
 C1[获取 shell 访问] --> C2[dump 配置文件]
 C1 --> C3[修改路由表/DNS]
 C1 --> C4[固件提取与逆向]
 end

 subgraph Phase4[Phase 4: 固件分析（无真实设备时）]
 D1[下载固件 .bin] --> D2[binwalk -Me 解包]
 D2 --> D3[提取文件系统]
 D3 --> D4[分析启动脚本/后门账户]
 D4 --> D5[模拟运行 FAT/Firmadyne]
 end
 ...
 style B3 fill:#f56c6c,color:#fff
...
> 相关模块: [[01-漏洞利用框架精通|Metasploit 框架]] | [[02-Exploit-DB与SearchSploit使用|Exploit-DB 搜索]] | [[../渗透基础/04-漏洞评估与利用基础|漏洞利用基础]]
...
```bash
# 安装 routersploit：用包管理器或从 GitHub 获取（启动交互式控制台）
rsf
...
```bash
# 安装 binwalk：用包管理器或从 GitHub 获取（下载固件后解包）
...
```bash
# 搜索相关工具：用包管理器搜索，或从 GitHub 获取 FAT / Firmadyne
