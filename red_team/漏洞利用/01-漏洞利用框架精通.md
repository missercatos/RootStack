...
```mermaid
sequenceDiagram
 participant A as 攻击机 (MSF)
 participant V as 受害机 (Windows)

 Note over A: Phase 1: 侦察
 A->>V: nmap -sV -O (端口/OS/服务)
 V-->>A: 端口列表、服务版本

 Note over A: Phase 2: 漏洞匹配
 A->>A: searchsploit + search exploits
 A->>A: msfvenom 生成 payload

 Note over A,V: Phase 3: 投递与利用
 A->>V: exploit 执行 (EternalBlue/其他)
 V-->>A: Meterpreter reverse_tcp 回连

 Note over V: Phase 4: 后渗透
 A->>V: getsystem (提权)
 A->>V: load kiwi (加载 mimikatz)
 V-->>A: creds_all (凭证导出)
 A->>V: hashdump (哈希转储)
 A->>V: run persistence (持久化)

 Note over A: Phase 5: 清理
 A->>V: clearev (清除日志)
 A->>V: resource cleanup.rc
```
...
> 相关模块: [[02-Exploit-DB与SearchSploit使用|Exploit-DB 搜索]] | [[03-路由器与IoT漏洞利用|路由器/IoT 利用]] | [[04-社会工程学攻击(SET)|SET 社会工程学]] | [[../渗透基础/04-漏洞评估与利用基础|漏洞利用基础]]
...
```bash
# 安装 metasploit：用包管理器或从 GitHub 获取（初始化数据库（首次使用））
sudo msfdb init
