...
### 第六部分：PHP后端基础
- [[php/PHP基础总目录|PHP 总目录与学习路线]]
- [[php/01-PHP核心语法与超全局变量|01-PHP核心语法与超全局变量]]
- [[php/02-弱类型与认证绕过|02-弱类型与认证绕过]]
- [[php/03-危险函数与命令执行|03-危险函数与命令执行]]
- [[php/04-终端PHP与curl封装器实战|04-终端PHP与curl封装器实战]]
- [[php/05-文件包含LFI_RFI|05-文件包含 LFI/RFI]]
- [[php/06-文件上传与WebShell|06-文件上传与WebShell]]
- [[php/07-反序列化与phar|07-反序列化与phar]]
- [[php/08-代码审计基础|08-代码审计基础]]
...
**阶段七：PHP后端（第10-12周）**
23. PHP核心语法与超全局变量 → Web 后端主力语言，读源码基础
24. 弱类型与认证绕过 → ==/0e/数组，CTF 与实战高频
25. 危险函数与命令执行 → eval/system 家族，RCE 根源
26. 终端 PHP 与 curl 封装器 → php://filter 读源码、php://input 打码、日志投毒
27. 文件包含 LFI/RFI → 包含到 RCE 的完整路径
28. 文件上传与 WebShell → 绕过检测、解析漏洞、上马
29. 反序列化与 phar → POP 链、PHPGGC、原生类
30. 代码审计基础 → 漏洞模式速查、审计流程、工具链
...
| PHP 超全局变量 | 输入面 | 参数注入, 上传, 头伪造 |
| PHP 弱类型 | 比较语义 | 认证绕过, 0e哈希, 数组绕过 |
| PHP 危险函数 | 执行入口 | eval/system → RCE |
| PHP 封装器 | 文件包含 | php://filter读源码, 日志投毒 |
| PHP 文件包含 | 包含语义 | LFI → RCE, RFI, Session包含 |
| PHP 文件上传 | 文件落地 | WebShell, 解析漏洞, .user.ini |
| PHP 反序列化 | 对象重构 | POP链 → RCE, phar反序列化 |
