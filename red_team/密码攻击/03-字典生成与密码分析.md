...
涉及工具：crunch, cupp, cewl, mentalist, changeme, pipal
...
```bash
# 安装
# 安装 cewl：用包管理器或从 GitHub 获取（基本爬取）
...
```bash
# 安装
# 安装 cupp：用包管理器或从 GitHub 获取（交互式模式）
cupp -i
...
```bash
# 安装
# 安装 crunch：用包管理器或从 GitHub 获取（基本语法: crunch [min] [max] [charset] -o [output]）
...
```bash
# 安装
# 安装 mentalist：用包管理器或从 GitHub 获取（启动GUI）
mentalist
```
...
```bash
# 安装
# 安装 pipal：用包管理器或从 GitHub 获取（分析rockyou泄露库）
...
```bash
# 安装
# 安装 changeme：用包管理器或从 GitHub 获取（扫描默认凭证）
