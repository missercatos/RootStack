...
OWASP ZAP是世界使用最广泛的免费Web安全工具之一，结合了拦截代理、主动扫描器、被动扫描器、爬虫、Fuzzer。通过 `zaproxy` 命令启动。
...
Nuclei是基于YAML模板的高速漏洞扫描器，社区广泛使用，替代已停止维护的arachni和skipfish。
...
Nuclei优势：模板丰富（CVE、misconfig、exposures、default-logins）、速度快、可自定义YAML模板、社区活跃。**工具更替说明：arachni（已停止维护，请使用ZAP/Nuclei替代），skipfish（已停止维护，请使用ffuf/gobuster替代）。**
