...
```bash
# 安装 testdisk：用包管理器或从 GitHub 获取（包含testdisk + photorec）
sudo testdisk
```
...
```bash
# 安装 dc3dd：用包管理器或从 GitHub 获取（创建带哈希验证的镜像）
...
```bash
# 安装 ddrescue：用包管理器或从 GitHub 获取（第一轮: 快速复制好区域，跳过坏区）
...
```bash
# 安装 bulk_extractor：用包管理器或从 GitHub 获取（从磁盘镜像中提取所有类型信息）
