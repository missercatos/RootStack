...
>
> 这不是一个"教程"，这是一次真实的渗透测试模拟。你是一个被雇佣的红队队员，目标是 AcmeCorp。你手头只有一台 Linux 物理机和域名 `acmecorp.com`。剩下的一切，全靠你自己去挖。
>
> 环境：Linux 物理机（任意发行版）。你需要安装的所有工具都会在每一步列出。全程手动操作，不依赖商业 C2 框架。
...
```bash
# 安装 hashcat（如果还没装）
# 安装 hashcat 与 hashcat-utils：用包管理器或从 GitHub 获取
