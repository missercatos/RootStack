...
- [[#一、Web技术概述|一、Web技术概述]]
- [[#二、HTTP协议深度解析|二、HTTP协议深度解析]]
- [[#三、HTTPS/TLS握手过程|三、HTTPS/TLS握手过程]]
- [[#四、Cookie机制|四、Cookie机制]]
- [[#五、Session机制|五、Session机制]]
- [[#六、同源策略 (Same-Origin Policy)|六、同源策略]]
- [[#七、CORS跨域资源共享|七、CORS跨域资源共享]]
- [[#八、URL编码/解码|八、URL编码/解码]]
- [[#九、Base64编码原理|九、Base64编码原理]]
- [[#十、前端基础|十、前端基础]]
- [[#十一、红队视角总结|十一、红队视角总结]]

## 一、Web技术概述

Web技术是互联网的核心，也是渗透测试中最常见的目标。
绝大多数的漏洞利用和攻击都发生在Web应用层。
...
HTTP (HyperText Transfer Protocol) 是应用层协议，基于TCP（默认端口80）。
...
| 方法 | 说明 | 红队关注点 |
|------|------|-----------|
| GET | 获取资源。参数在URL中(有长度限制)。可被缓存/收藏 | URL参数注入(如 `id=1' OR '1'='1`) |
| POST | 提交数据(如表单)。参数在请求体中 | 修改POST数据绕过客户端验证 |
| PUT | 上传/替换资源。可能导致任意文件上传 | PUT方法写WebShell |
| DELETE | 删除资源。可能被利用进行拒绝服务 | 资源删除攻击 |
| HEAD | 同GET但只返回响应头(无响应体) | 快速检测端点，减少日志 |
| OPTIONS | 查询服务器支持的方法。常用于CORS预检 | 信息收集，了解服务端能力 |
| PATCH | 部分修改资源。与PUT的区别:PUT是全量替换 | 可能绕过PUT的完整性校验 |
| TRACE | 回显请求内容。用于调试，可被利用进行XST攻击 | TRACE方法可能泄露Cookie(HttpOnly也无法保护) |
| CONNECT | 建立隧道连接（用于HTTPS代理） | 可能绕过HTTP代理限制 |

红队技巧：使用OPTIONS探测服务器，然后尝试危险的HTTP方法(PUT/DELETE/TRACE)
...
| 请求头 | 说明 | 红队关注点 |
|--------|------|-----------|
| Host | 目标主机名（HTTP/1.1必需） | Host头注入、虚拟主机枚举 |
| User-Agent | 客户端标识（浏览器/操作系统） | UA欺骗绕过检测 |
| Cookie | 发送存储的Cookie | Cookie注入、会话劫持 |
| Referer | 来源页面URL | Referer泄露、CSRF检测 |
| Authorization | 认证凭证（Basic, Bearer, Digest） | 凭证嗅探、JWT分析 |
| Content-Type | 请求体MIME类型（form/json/xml等） | 修改Content-Type绕过WAF |
| X-Forwarded-For | 原始客户端IP（代理添加） | IP欺骗、SSRF绕过 |
| X-Real-IP | 同X-Forwarded-For | IP限制绕过、访问控制绕过 |
| Accept/Accept-Language/Accept-Encoding | 客户端偏好设置 | 指纹收集 |
| Connection | 连接管理 | Keep-Alive滥用 |
| Cache-Control | 缓存策略 | 缓存投毒 |
| X-Requested-With | Ajax请求标识（XMLHttpRequest） | CSRF检测绕过 |

【HTTP响应头详解】

| 响应头 | 说明 | 红队关注点 |
|--------|------|-----------|
| Server | 服务器软件及版本 | 信息泄露，便于版本漏洞匹配 |
| Set-Cookie | 设置Cookie | Cookie属性缺失(无Secure/HttpOnly) |
| Location | 重定向目标URL | 开放重定向检测 |
| Content-Type | 响应体MIME类型 | MIME类型混淆攻击 |
| Content-Length | 响应体大小（字节） | 请求走私辅助 |
| X-Powered-By | 后端技术栈（如 PHP/7.4） | 信息泄露 |
| Content-Security-Policy (CSP) | 内容安全策略 | CSP bypass技术 |
| X-Frame-Options | 防点击劫持 (DENY/SAMEORIGIN) | 缺失时可进行Clickjacking |
| X-Content-Type-Options | 禁止MIME类型嗅探 (nosniff) | 浏览器行为控制 |
| Strict-Transport-Security | HSTS强制HTTPS | HTTPS降级防护 |
| Access-Control-\* | CORS跨域头 | 跨域配置检查 |
| X-XSS-Protection | XSS过滤器（已废弃） | 历史兼容 |
...
Cookie是存储在客户端的少量文本数据，由服务器通过Set-Cookie头发送，
浏览器自动在后续请求中通过Cookie头附带。
...
| 属性 | 说明 |
|------|------|
| Domain | Cookie生效的域名范围。`.example.com` → 所有子域名，不设置 → 仅当前域名。红队: 宽松Domain可被子域名XSS影响 |
| Path | Cookie生效的路径。`Path=/` → 全站，`Path=/admin` → 仅/admin路径 |
| Expires/Max-Age | 过期时间(GMT)。均不设置 → 会话Cookie(浏览器关闭即删除)。Max-Age优先级高于Expires |
| HttpOnly | 禁止JavaScript通过document.cookie读取。红队: 无HttpOnly时XSS可直接窃取Cookie。注意: TRACE方法可绕过HttpOnly(XST攻击) |
| Secure | 仅通过HTTPS传输Cookie。红队: 无Secure时MITM可截获Cookie |
...
由于HTTP是无状态协议，Session机制用于在多个请求之间维持用户状态。

 三种常见身份维持方式对比：

| 方案 | 存储位置 | 优点 | 缺点 |
|------|---------|------|------|
| Cookie | 客户端 | 简单，自动发送，可设HttpOnly防XSS | CSRF风险，体积限制(4KB) |
| Session | 服务端 | 更安全，可存储大量数据 | 服务端内存开销，分布式需Session共享 |
...
CORS (Cross-Origin Resource Sharing) 允许服务器声明哪些外部源可以访问自己的资源。
本质是放宽同源策略的机制。
