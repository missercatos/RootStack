...
- [[#四、非对称加密|四、非对称加密]]
- [[#五、数字签名|五、数字签名]]
- [[#六、PKI/CA证书体系|六、PKI/CA证书体系]]
- [[#七、SSL/TLS详解|七、SSL/TLS详解]]
- [[#八、Base64/Hex编码|八、Base64/Hex编码]]
- [[#九、常见编码方式|九、常见编码方式]]
- [[#十、Kerberos认证原理|十、Kerberos认证原理]]
- [[#十一、NTLM认证流程|十一、NTLM认证流程]]
- [[#十二、红队视角总结|十二、红队视角总结]]
...
而是为了：
 1. 破解弱加密的密码(Hash破解)
 2. 理解认证协议的弱点(Kerberos/NTLM攻击)
 3. 识别不安全的加密配置(TLS弱密码套件)
 4. 绕过基于密码学的防御
...
【SHA家族详解】
 ...
 Linux密码存储：$6$ 表示 SHA-512 (crypt)
 hashcat模式: 1800 = sha512crypt ($6$)

【bcrypt/Argon2 - 密码专用哈希】
 特点：故意设计为慢速，对抗暴力破解
 包含：盐值(salt) + 成本因子(cost factor)
 bcrypt: $2a$10$salt+hash，10是成本因子(迭代2^10次)
 Argon2: 2015年密码哈希竞赛冠军，抗GPU/ASIC

【加盐(Salt)】
 在密码后附加随机字符串再进行哈希，防止：
 1. 彩虹表攻击(预计算的哈希->明文映射表)
 2. 相同密码产生不同哈希
 3. 批量破解(不同用户的相同密码需要逐个破解)
...
【红队哈希破解实践】
 ...
 hashcat -m 1800 hashes.txt rockyou.txt # SHA512 crypt ($6$)
...
实际通信中非对称加密不直接加密大块数据：
 ...
 场景：Alice向Bob发送加密消息
 1. Bob生成RSA密钥对，将公钥发给Alice
 2. Alice生成随机AES对称密钥
 3. Alice用Bob的公钥加密AES密钥 (非对称)
 4. Alice用AES密钥加密消息 (对称)
 5. Alice发送(加密的AES密钥 + 加密的消息)给Bob
 6. Bob用自己的私钥解密得到AES密钥
 7. Bob用AES密钥解密消息
...
flowchart TB
 ROOT["Root CA (自签名)"]
 INT1["Intermediate CA 1 (由Root CA签发)"]
 INT2["Intermediate CA 2 (由Int CA 1签发)"]
 SERVER["Server Certificate (由Int CA 2签发)\nCN=example.com"]
 ROOT --> INT1 --> INT2 --> SERVER
```
...
【TLS攻击面(红队)】
 ...
 6. ROBOT (Return Of Bleichenbacher's Oracle Threat)：
...
【HTML实体编码】
 ...
 XSS绕过: <img src=x onerror=alert(1)> ->
...
【Kerberos红队攻击技术】
 ...
 1. Kerberoasting (Kerberos服务票据爆破):
