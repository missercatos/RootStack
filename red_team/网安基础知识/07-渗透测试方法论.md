...
- [[#一、渗透测试概述|一、渗透测试概述]]
- [[#二、PTES七阶段详解|二、PTES七阶段详解]]
- [[#三、Cyber Kill Chain详解|三、Cyber Kill Chain详解]]
- [[#四、MITRE ATT&CK框架|四、MITRE ATT&CK框架]]
- [[#五、OWASP测试指南概述|五、OWASP测试指南概述]]
- [[#六、红队 vs 渗透测试的区别|六、红队 vs 渗透测试的区别]]
- [[#七、综合实战视角|七、综合实战视角]]
...
【常用Technique举例 (每项的重要子技术)】
  ...
  Lateral Movement (TA0008):
    T1021.001 RDP (远程桌面)
    T1021.002 SMB/Windows管理共享(C$ ADMIN$)
    T1021.006 WinRM
...
【OWASP Testing Guide v4 核心测试模块】
  ...
  2. 配置和部署管理测试 (Configuration Management)
     - 测试网络/基础设施配置
     - 测试应用平台配置
     - 测试文件扩展处理
     - 测试备份文件和未引用文件
  ...
  3. 身份管理测试 (Identity Management)
     - 测试角色定义和权限
     - 测试用户注册过程
     - 测试账户配置过程
     - 测试账户枚举和用户可猜用户名

  4. 认证测试 (Authentication)
     - 测试密码策略(弱口令, 默认凭证)
     - 测试登录机制绕过
     - 测试多因素认证
     - 测试密码重置/找回功能
  ...
  5. 授权测试 (Authorization)
     - 测试目录遍历/路径穿越
     - 测试越权(IDOR)
     - 测试权限提升

  6. 会话管理测试 (Session Management)
  ...
  9. 弱加密测试 (Weak Cryptography)
     - 测试SSL/TLS配置和弱密码套件
     - 测试明文/弱哈希密码存储

  10. 业务逻辑测试 (Business Logic Testing)
     - 测试工作流缺陷(跳过步骤)
     - 测试价格操纵
     - 测试输入限制绕过
...
【攻击流程图解】
   ...
   2. 漏洞分析 (PTES 4, KC 2)
      - 识别漏洞/弱配置
   3. 初始入侵 (KC 3-4, ATT&CK TA0001-2)
      - Exploit利用 / Phishing钓鱼
      - 获得初始Shell
   4. 本地信息收集 (PTES 6, ATT&CK TA0007)
      - 提权 (ATT&CK TA0004)
   5. 持久化 (ATT&CK TA0003)
      - 安装后门/计划任务
   6. 横向移动 (ATT&CK TA0008)
      - 凭证窃取 (ATT&CK TA0006)
      - 跳板到其他系统
      - 重复3-6直到到达目标
   7. 目标行动 (KC 7, ATT&CK TA0009-42)
      - 数据收集
      - 数据窃取 (TA0010)
   8. 报告/评分 (PTES 7)
