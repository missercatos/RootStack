...
### 第一部分：Linux系统深度
- [[#一、Linux系统概述|一、Linux系统概述]]
- [[#二、文件系统结构 (FHS标准)|二、文件系统结构]]
- [[#三、文件权限体系|三、文件权限体系]]
- [[#四、进程管理|四、进程管理]]
- [[#五、用户与组|五、用户与组]]
- [[#六、systemd服务管理|六、systemd服务管理]]
- [[#七、日志系统|七、日志系统]]
- [[#八、常用命令速查（红队信息收集专用）|八、常用命令速查]]
- [[#九、Linux核心安全机制|九、Linux核心安全机制]]

### 第二部分：Windows系统
- [[#十、Windows NT架构|十、Windows NT架构]]
- [[#十一、注册表基础|十一、注册表基础]]
- [[#十二、服务与进程|十二、服务与进程]]
- [[#十三、用户与组|十三、用户与组]]
- [[#十四、事件日志|十四、事件日志]]
- [[#十五、权限模型 (UAC + Integrity Level)|十五、权限模型]]
...
Linux是红队最常用的操作系统，原因：
 1. 开源免费，高度可定制
 2. 命令行强大，适合自动化
 3. 大量安全工具原生支持Linux（Kali Linux）
 4. 服务器市场占有率极高
 5. 了解Linux = 了解90%的目标服务器
...
2. SGID (Set Group ID) — 数值 2000
 可执行文件上: 以文件所属组的权限运行
 目录上: 目录中新创建的文件继承目录的组(而非创建者的组)
 显示: -rwxr-sr-x
 红队：特定场景可用于权限提升和横向移动

3. Sticky Bit (粘滞位) — 数值 1000
 仅用于目录，限制删除权限
...
密码哈希前缀识别：
 $1$ -> MD5 (弱! 易碰撞)
 $2a$ -> Blowfish (bcrypt)
 $2y$ -> Blowfish (bcrypt, PHP风格)
 $5$ -> SHA-256 (crypt)
 $6$ -> SHA-512 (crypt, Linux默认)
 $y$ -> yescrypt (较新的算法)
 !! -> 账户已锁定/无密码
 ! -> 密码已锁定
...
破解方法：
 john /etc/shadow
 hashcat -m 1800 -a 0 hash.txt wordlist.txt # $6$ 是模式1800
 hashcat -m 500 -a 3 hash.txt ?l?l?l?l?l?l # $1$ 是模式500
...
红队持久化方式（利用systemd）：
 1. 创建自己的恶意服务文件，enable后开机自启
 2. 修改现有服务(如sshd)的ExecStart，附加恶意命令
 3. 利用systemd Timer创建定期任务(替代cron)
 4. systemd user service (~/.config/systemd/user/)——不需要root
...
```mermaid
flowchart TB
 subgraph USER[用户模式 User Mode]
 direction TB
 PROC["用户进程 (EXE)"]
 SUBSYS["子系统DLL (kernel32)"]
 ENV["环境子系统 (Win32/POSIX)"]
 NTDLL["ntdll.dll\n系统调用接口/Stub"]
 end
 PROC --> NTDLL
 SUBSYS --> NTDLL
 ENV --> NTDLL
 NTDLL -->|syscall/sysenter| KERNEL

 subgraph KERNEL[内核模式 Kernel Mode]
 NTOS["NTOSKRNL.EXE\n执行体 + 内核 + 驱动层"]
 DRV["设备驱动"]
 HAL["HAL (硬件抽象层)"]
 end
...
UAC绕过技术(红队核心):
 1. DLL劫持: 利用需提升权限的系统程序，在搜索路径中放置恶意DLL
 2. COM劫持: 替换COM对象的注册，让高权限程序加载恶意DLL
 3. 注册表劫持: 修改系统进程的依赖项
 4. Elevated COM接口(如ICMLuaUtil): 利用系统的高权限COM对象直接执行代码
 5. SilentCleanup: 利用Windows任务清理程序的自动提权
 6. fodhelper: 利用Windows FOD helper绕过UAC
 7. 常见UAC Bypass工具: UACMe (GitHub开源)
...
Linux方面:
 1. 熟悉FHS，能快速找到配置、日志、凭证
 2. 理解文件权限和SUID/SGID提权
 3. 掌握进程管理和信号操作(杀进程、暂停进程)
 4. 能熟练使用systemctl管理服务(持久化)
 5. 理解日志系统，知道如何清除痕迹
 6. 了解SELinux/AppArmor的限制和绕过方法
 7. 能手工写systemd服务用于持久化

Windows方面:
 1. 理解NT架构(用户态/内核态分界线)
 2. 掌握注册表关键位置(启动项、服务、凭证)
 3. 理解进程注入技术原理
 4. 熟悉令牌体系(Token)和提权方式
 5. 了解事件日志结构和清除方法
 6. 掌握UAC绕过和完整性级别概念
 7. 会使用PowerShell进行系统管理

推荐学习路径:
 1. 日常使用Linux命令行(不用GUI，全部命令操作)
 2. 用Linux做所有渗透测试工作
 3. 在虚拟机中搭建各种Windows版本(2008/2012/2016/2019/10/11)
 4. 学习PowerShell基础
 5. 实验进程注入和令牌窃取
 6. 练习日志清除和痕迹擦除
