...
**前置要求**：
- 已学完 [[../2深化/05_位运算与硬件操作|位运算与硬件操作]]（理解原子位操作）
- 已学完 [[../2深化/04_函数指针与回调|函数指针与回调]]（理解 RCU 回调）
- 已学完 [[05_中断与系统调用|中断与系统调用]]（理解中断上下文）
- 建议阅读 [[../Rust教程/6内核/02-内核Rust抽象层|Rust: 内核Rust抽象层]]（对比 Rust 的并发安全模型）
...
// ============================================
//
// 见 [[../Rust教程/6内核/02-内核Rust抽象层|Rust: 内核Rust抽象层]]
//
// Rust 的所有权系统保证:
...
> > 答案: 错误
> >
> > **解析**: volatile 只告诉编译器不要优化对此变量的读写（每次都访问内存），但它不提供原子性，不阻止 CPU 重排序，不解决并发访问的多步复合操作（read-modify-write）。`volatile` 用于硬件寄存器访问，不适用于多线程同步。详见 [[../2深化/05_位运算与硬件操作|位运算与硬件操作]]。
...
> > 正确答案: B
> >
> > **解析**: Rust 通过所有权（ownership）系统和借用检查器（borrow checker）在编译时保证：要么有多个不可变引用（只读），要么有一个可变引用（可写），不能同时有读和写。这消除了数据竞争的根本可能性。Send/Sync trait 确保类型可以安全地在线程间传递。详见 [[../Rust教程/6内核/02-内核Rust抽象层|Rust: 内核Rust抽象层]]。
...
- **返回**: [[../目录|目录]]
- **相关**: [[../2深化/05_位运算与硬件操作|位运算与硬件操作]] | [[../2深化/04_函数指针与回调|函数指针与回调]] | [[../2深化/02_内存模型与布局|内存模型与布局]]
- **Rust 延伸**: [[../../Rust教程/6内核/02-内核Rust抽象层|Rust: 内核Rust抽象层]] | [[../../Rust教程/6内核/04-内核驱动：Rust vs C对比|Rust: Rust vs C驱动对比]]
