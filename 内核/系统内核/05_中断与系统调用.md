...
- 已学完 [[../2深化/06_编译链接与ELF|编译链接与ELF]]（理解符号和链接）
- 已学完 [[../ASM教程/04_控制流与系统调用|ASM: 系统调用]]（汇编层理解）
- 建议阅读 [[../ASM教程/01_体系结构与寄存器|ASM: 体系结构与寄存器]]（理解 CPU 寄存器）
...
- **上一章**: [[04_设备驱动|设备驱动]]
- **返回**: [[../目录|目录]]
- **相关**: [[../2深化/06_编译链接与ELF|编译链接与ELF]] | [[../2深化/05_位运算与硬件操作|位运算与硬件操作]]
