...
- 已学完 [[../2深化/05_位运算与硬件操作|位运算与硬件操作]]（volatile、内存映射 IO、位操作）
- 已学完 [[../2深化/07_面向对象C编程|面向对象C编程]]（理解驱动中的"对象"——每个设备实例的结构体）
- 建议阅读 [[../cpp深化教程/05_面向对象类与对象基础|CPP: 类与对象]]（对比 C 和 C++ 的封装方式）
...
如 [[../Rust教程/6内核/04-内核驱动：Rust vs C对比|Rust: Rust vs C驱动对比]] 中详细讨论的，Rust 正在进入 Linux 内核驱动程序开发。以下是一个精简的对比：
...
// ============================================
// Rust 驱动对应实现 (关键优势对比)
// 实际代码参考: [[../Rust教程/6内核/04-内核驱动：Rust vs C对比]]
// ============================================
...
> > 答案: 正确
> >
> > **解析**: Rust 的所有权系统（ownership）、借用检查器（borrow checker）和生命周期（lifetimes）在编译时保证了内存安全——不存在 use-after-free、double-free、buffer overflow（在 safe Rust 中）。这些是 C 驱动中最常见的严重漏洞类别（约 70%）。详见 [[../Rust教程/6内核/01-Linux内核Rust支持概述|Rust: 内核Rust支持概述]]。
...
> > 正确答案: C
> >
> > **解析**: `volatile` 告诉编译器不要优化对指定地址的读写——每次访问都必须生成实际的内存访问指令，不能缓存到寄存器中。硬件寄存器的值可能在任何时刻变化（由硬件修改），编译器不知道这一点。详见 [[../2深化/05_位运算与硬件操作|位运算与硬件操作]]。
...
- **上一章**: [[03_文件系统|文件系统]]
- **返回**: [[../目录|目录]]
- **相关**: [[../2深化/05_位运算与硬件操作|位运算与硬件操作]] | [[../2深化/07_面向对象C编程|面向对象C编程]] | [[../2深化/04_函数指针与回调|函数指针与回调]]
- **CPP 对比**: [[../../cpp深化教程/05_面向对象类与对象基础|CPP: 类与对象]]
- **汇编参考**: [[../../ASM教程/04_控制流与系统调用|ASM: 系统调用]] | [[../../ASM教程/06_内存与IO|ASM: 内存与IO]]
- **Rust 对比**: [[../../Rust教程/6内核/04-内核驱动：Rust vs C对比|Rust: Rust vs C 驱动对比]]
