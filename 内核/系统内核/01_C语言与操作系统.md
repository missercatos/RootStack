...
>  **本章是第四部分"内核"的入口**。前三部分（[[../1入门/01_环境配置|1入门]] → [[../2深化/01_指针深度剖析|2深化]] → [[../3数据结构/01_动态数组|3数据结构]]）为你打下了坚实的 C 语言基础。现在，我们将进入 C 语言最辉煌的战场——Linux 内核。在写第一行内核代码之前，你需要先理解：为什么是 C？内核是什么？内核态和用户态的区别是什么？
...
**前置要求**：
- 已学完 [[../2深化/02_内存模型与布局|内存模型与布局]]（理解虚拟地址空间）
- 已学完 [[../2深化/05_位运算与硬件操作|位运算与硬件操作]]（理解位操作和 volatile）
- 已学完 [[../2深化/06_编译链接与ELF|编译链接与ELF]]（理解编译和链接过程）
- 建议同时阅读 [[../ASM教程/04_控制流与系统调用|ASM: 系统调用]]（理解用户态到内核态的切换）
...
- 但 Rust 不取代 C，而是与 C 共存：详见 [[../Rust教程/6内核/01-Linux内核Rust支持概述|Rust: 内核Rust支持概述]]
...
>  关于系统调用的汇编层细节，详见 [[../ASM教程/04_控制流与系统调用|ASM: 系统调用]]。本章第三部分将有更深入的追踪。
...
>  关于 ELF 格式的深入讲解，参见 [[../2深化/06_编译链接与ELF|编译链接与ELF]]。
...
//    详见: [[../3数据结构/02_链表|内核链表]] 和 [[07_Linux内核源码导读|内核源码导读]]
...
> > 正确答案: B
> >
> > **解析**: `container_of` 是 Linux 内核中最重要的宏之一。给定一个成员的指针、结构体类型和成员名，它可以计算出包含该成员的结构体的地址。这是内核链表实现的基础。详见 [[../2深化/07_面向对象C编程|面向对象C编程]] 和 [[07_Linux内核源码导读|内核源码导读]]。
...
> > 正确答案: C
> >
> > **解析**: Linux 6.1（2022年12月）正式合入了 Rust 基础设施，允许用 Rust 编写内核模块。Rust 是 C 的补充而非替代——它主要用于编写新的设备驱动，利用其编译时内存安全检查减少 UAF 和缓冲区溢出漏洞。详见 [[../Rust教程/6内核/01-Linux内核Rust支持概述|Rust: 内核Rust支持概述]]。
...
- **返回**: [[../目录|目录]]
- **相关**: [[../2深化/02_内存模型与布局|内存模型与布局]] | [[../2深化/05_位运算与硬件操作|位运算与硬件操作]] | [[../2深化/06_编译链接与ELF|编译链接与ELF]]
- **汇编参考**: [[../../ASM教程/04_控制流与系统调用|ASM: 系统调用]] | [[../../ASM教程/01_体系结构与寄存器|ASM: 体系结构与寄存器]]
- **Rust 延伸**: [[../../Rust教程/6内核/01-Linux内核Rust支持概述|Rust: 内核Rust支持概述]]
