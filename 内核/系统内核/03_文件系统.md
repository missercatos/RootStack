...
>  **本章探讨"一切皆文件"的基石**。Linux 最著名的设计哲学——"一切皆文件"——由虚拟文件系统（VFS）实现。VFS 是一个纯 C 语言实现的面向对象框架，它是 Linux 内核中最优雅的设计之一，完美展示了 [[../2深化/07_面向对象C编程|面向对象C编程]] 中我们学到的"用 C 实现多态"思想。
...
**前置要求**：
- 已学完 [[../2深化/07_面向对象C编程|面向对象C编程]]（理解函数指针表 = 虚函数表）
- 已学完 [[../2深化/04_函数指针与回调|函数指针与回调]]（理解函数指针语法）
- 已学完 [[01_C语言与操作系统|C语言与操作系统]]（理解内核模块基础）
- 建议阅读 [[../ASM教程/04_控制流与系统调用|ASM: 系统调用]]（理解 open/read/write 的用户态入口）
...
>  这与 [[../2深化/07_面向对象C编程|面向对象C编程]] 中手动实现虚函数表的方法完全一致！如果你还没读过那一章，现在是一个很好的交叉阅读时机。
...
- **返回**: [[../目录|目录]]
- **相关**: [[../2深化/07_面向对象C编程|面向对象C编程]] | [[../2深化/04_函数指针与回调|函数指针与回调]]
- **汇编参考**: [[../../ASM教程/04_控制流与系统调用|ASM: 系统调用]]
- **Rust 延伸**: [[../../Rust教程/6内核/04-内核驱动：Rust vs C对比|Rust: Rust vs C驱动对比]]
