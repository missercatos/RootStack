...
>  **本章是内核编程的核心**。进程和内存是操作系统最重要的两个抽象。理解 `task_struct`、页表、伙伴系统和 slab 分配器，是阅读和编写任何内核代码的基础。本章将把你在 [[../2深化/02_内存模型与布局|内存模型与布局]] 中学到的虚拟内存概念扩展到内核层面。
...
**前置要求**：
- 已学完 [[../2深化/02_内存模型与布局|内存模型与布局]]（虚拟地址空间、页机制）
- 已学完 [[../2深化/03_动态内存管理|动态内存管理]]（用户态内存管理概念）
- 已学完 [[../2深化/07_面向对象C编程|面向对象C编程]]（理解结构体作为"对象"的思想）
- 建议阅读 [[../ASM教程/06_内存与IO|ASM: 内存与IO]]（MMU、页表硬件层理解）
...
- **返回**: [[../目录|目录]]
- **相关**: [[../2深化/02_内存模型与布局|内存模型与布局]] | [[../2深化/03_动态内存管理|动态内存管理]] | [[../2深化/07_面向对象C编程|面向对象C编程]]
- **汇编参考**: [[../../ASM教程/06_内存与IO|ASM: 内存与IO]] | [[../../ASM教程/01_体系结构与寄存器|ASM: 体系结构与寄存器]]
- **Rust 延伸**: [[../../Rust教程/6内核/02-内核Rust抽象层|Rust: 内核Rust抽象层]]
