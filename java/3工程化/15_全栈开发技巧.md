...
很多公司选择 Java 全栈（前端 Vue3 + 后端 Spring Boot），而不是 Java + Go + React 三语言栈。这不是"将就"，而是有清晰的工程理由：

**原因一：降低招聘和协作成本。** 一个团队统一 Java 技术栈，前后端联调时后端能看懂前端报错（NPE 是 NPE，类型错误是类型错误），前端能读懂后端日志（SQL 报错、事务回滚），而不是互相丢截图说"你这边看一下"。小团队（5 人以下）时这种协作效率差距是致命的。

**原因二：共享类型定义。** 后端用 record 或 DTO 定义的接口契约，前端用 TypeScript 定义的接口类型，可以由 OpenAPI 自动生成两端代码。如果前后端各用不同语言，类型同步需要额外工具链（gRPC + protobuf 等），复杂度陡增。

**原因三：Spring 生态的全栈能力。** Spring 不只是后端框架——Spring MVC 处理 HTTP、Spring WebSocket 处理实时通信、Spring Security 处理认证授权、Spring Session 管理会话状态。一套框架覆盖全栈需求，不需要引入 Nginx 之外的中间件（Redis 除外）。

### Thymeleaf 与前后端分离的取舍

Thymeleaf（服务端模板渲染）适合什么场景？

| 维度 | Thymeleaf | 前后端分离 |
|------|-----------|-----------|
| SEO | 天然友好（服务端渲染完整 HTML） | 需 SSR（Next.js/Nuxt） |
| 首屏速度 | 快（一次 HTTP 返回完整页面） | 慢（先加载空壳再请求数据） |
| 开发效率 | 前后端耦合，改页面要重启后端 | 并行开发，前端用 Mock |
| 部署复杂度 | 单应用打包即可 | 需要 Nginx + 前后端分别部署 |
| 复杂交互 | 困难（JSP 式的模板表达力有限） | 灵活（React/Vue 生态） |

**结论：** 管理后台、内部工具、SEO 不敏感的 SaaS → 前后端分离；营销落地页、博客、内容型网站 → Thymeleaf 或 SSR。Java 全栈的价值在于你可以**两种都选**，而不是被技术栈绑架。

### REST API 设计的深层考量

REST 不只是"用 HTTP 动词表示 CRUD"。好的 REST 设计背后是资源模型的思考：
...
// 正例：资源式 API，URL 本身就是领域模型
@GetMapping("/users/{userId}/orders")
public List<OrderDto> listOrders(@PathVariable Long userId,
                                  @RequestParam(defaultValue = "1") int page) { ... }
```

**为什么资源式更好？** 因为 URL 是**稳定的契约**，而实现细节（SQL 怎么查、缓存怎么用）是**易变的实现**。`/users/{id}/orders` 无论后端从 MySQL 迁移到 MongoDB 还是加了 Redis 缓存，URL 不变，客户端不受影响。过程式 API 把 `getUserOrders` 改成 `fetchUserOrders` 就是破坏性变更。

**HATEOAS 的实际价值。** REST 的最高成熟度是 HATEOAS（响应里带下一步操作的链接）。实践中很少完整实现，但核心思想——**服务端告诉客户端"你现在能做什么"**——在分页场景很有用：
...
```json
{
  ...
  "pagination": {
    "page": 2,
    "total": 10,
    "next": "/users/123/orders?page=3",
    "prev": "/users/123/orders?page=1"
  }
}
```

客户端不需要知道 URL 拼接规则，直接用 `next` 链接翻页。

### WebSocket 与 REST 的互补关系

REST 是请求-响应模型（客户端主动问，服务端被动答），WebSocket 是全双工模型（服务端也能主动推）。两者不是替代关系：

| 场景 | 选择 | 理由 |
|------|------|------|
| 查询订单列表 | REST | 客户端一次性获取，不需要实时推送 |
| 聊天消息 | WebSocket | 双方都需要实时收发 |
| 订单状态变更通知 | WebSocket | 服务端主动推，客户端不需要轮询 |
| 文件上传 | REST | multipart 是 HTTP 原生能力 |
| 仪表盘实时数据 | WebSocket | 数据源持续变化，推送比轮询高效 |

**多实例下的 WebSocket 广播。** Spring 默认的 `SimpMessagingTemplate` 只能广播到连在本机的 WebSocket 连接。两台实例部署时，用户 A 连实例 1，用户 B 连实例 2，A 发消息 B 收不到。解法：用 Redis 的 Pub/Sub 做消息中转——应用收到消息后 publish 到 Redis channel，所有实例订阅该 channel 并广播给自己管理的 WebSocket 连接。

### GraphQL 与 REST 的选择

GraphQL 解决的是 REST 在复杂前端场景下的问题：**过度获取**（返回了前端不需要的字段）和**不足获取**（一次请求拿不到所有需要的数据，需要多次请求）。

```graphql
# 一次查询拿到用户、订单、商品详情——REST 需要 3 次请求
query {
  user(id: 123) {
    name
    orders {
...
**什么时候选 GraphQL？** 前端页面数据结构复杂（电商详情页、社交 Feed 流）、多端（Web/App/小程序）数据需求差异大、BFF 层需要灵活裁剪字段。**什么时候 REST 够用？** 管理后台、CRUD 为主的应用、对外公开的 API（GraphQL 的查询复杂度可能被恶意利用做 DDoS）。

### 微服务模式下的全栈架构

当系统从单体演进到微服务，全栈架构需要新增几个组件：

```
浏览器 → Nginx → API Gateway → 微服务 A
                              → 微服务 B
                              → 微服务 C
```

**API Gateway（网关）：** 统一入口，负责路由、认证、限流。Spring Cloud Gateway 基于 WebFlux（响应式），吞吐量是传统 Servlet 模型的数倍。路由规则写在配置里，前端无感知后端拆分成了几个服务。

**Config Server（配置中心）：** 微服务的配置统一管理，热更新不重启。Spring Cloud Config 用 Git 仓库做配置源，配合 `@RefreshScope` 实现运行时刷新。

**Discovery（服务发现）：** 服务实例动态注册和发现。Nacos 同时做配置中心和服务发现，一个组件解决两个问题。服务间调用用 OpenFeign（声明式 HTTP 客户端），写接口注解就能远程调用，像调本地方法一样。

```java
@FeignClient(name = "user-service", fallback = UserServiceFallback.class)
public interface UserServiceClient {
    @GetMapping("/users/{id}")
    ApiResult<UserDto> getUser(@PathVariable Long id);
}
```

**为什么微服务对全栈开发有影响？** 因为前端调用的 `/api/v1/users` 现在可能经过 Gateway → 路由到 user-service。如果 Gateway 没有做好超时配置、熔断降级，前端感受到的就是"接口超时"。全栈开发者需要理解这条链路上每个环节的超时和重试策略，才能定位"为什么页面卡住了"。

### 微服务间的分布式事务

微服务拆分后，一个业务操作可能跨多个服务的数据库。传统的 `@Transactional` 只能管单个数据库事务，跨服务的一致性需要额外方案：

| 方案 | 原理 | 适用场景 |
|------|------|----------|
| Saga | 每个服务执行本地事务并发事件，失败时按反向顺序补偿 | 长事务、跨服务 |
| TCC（Try-Confirm-Cancel） | 业务层面的两阶段提交 | 资金类强一致 |
| 本地消息表 | 本地事务 + 消息队列，保证最终一致 | 大多数业务场景 |
| Seata AT 模式 | 框架自动管理 undo log，对业务透明 | 已有 Spring Cloud 生态 |

**实际选型：** 90% 的场景用**本地消息表 + MQ** 实现最终一致性就够了（订单创建后发 MQ 消息，库存服务消费并扣减）。只有金融级场景才需要 TCC 或 Seata。分布式事务的核心认知：**强一致性的代价是性能和可用性**，CAP 定理决定了你不能三者兼得。
...
- Java 全栈的优势：统一技术栈降低协作成本，Spring 生态覆盖全栈需求，REST 与 GraphQL 按场景选择；
- 微服务架构下全栈开发者需要理解 Gateway → Config → Discovery 的链路，以及分布式事务的最终一致性方案。
