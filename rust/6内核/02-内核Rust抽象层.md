...
> 📌 **C语言作为参照**：Rust的内核抽象层（如Pin、Arc、锁机制等）的设计，很大程度上是对C语言内核模式的封装。要理解这些抽象的背后逻辑，请参阅 [[../../C语言深化教程/4内核/06_并发与同步|C语言教程: 并发与同步（C视角）]] 和 [[../../C语言深化教程/2深化/07_面向对象C编程|C语言教程: C语言中的面向对象编程]]。
