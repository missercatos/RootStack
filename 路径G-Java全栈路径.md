...
>
> 本路径覆盖 **Java 全栈**（语言入门 → 深入特性 → 工程化与 Spring 全家桶 → 前端闭环）。前端部分已由 [[前端开发/引导阅读|前端开发教程]] 承接（Phase 11）；服务器运维目录仍在规划中。
...
> **检查点**: 独立完成一个含前端页面的部署上线项目（前端部分见 Phase 11）；能画出自己项目的架构图并回答"为什么这么拆"；向任一开源项目提交过至少一个被合并的 PR。

---

### Phase 11: 前端全栈闭环 (约 6-8 周)

> Java 全栈的最后一块拼图。不要求成为前端专家——按"够用且扎实"的原则，从 [[前端开发/引导阅读|前端开发引导阅读]] 中选取"Java 后端接前端"路线推进：

| 序号 | 文件 | 核心内容 | 建议学时 |
|:----:|------|---------|:-------:|
| 1 | [[前端开发/01-基础/HTML/01-HTML基础语法\|HTML 基础语法]] → [[前端开发/01-基础/HTML/02-HTML表单与语义化\|表单与语义化]] | 页面骨架与表单（对照 Bean Validation） | 6h |
| 2 | [[前端开发/01-基础/CSS/01-CSS基础语法\|CSS 基础]] → [[前端开发/01-基础/CSS/03-Flexbox布局\|Flexbox]] → [[前端开发/01-基础/CSS/06-CSS实战：响应式设计\|响应式实战]] | 样式三件套，够用即可 | 10h |
| 3 | [[前端开发/01-基础/JavaScript/01-JS基础语法\|JS 基础]] → [[前端开发/01-基础/JavaScript/02-函数与作用域\|函数与闭包]] → [[前端开发/01-基础/JavaScript/05-异步编程\|异步编程]] → [[前端开发/01-基础/JavaScript/07-JS实战：交互式页面\|Todo 实战]] | 语言核心，重点异步模型 | 16h |
| 4 | [[前端开发/04-DOM与交互/AJAX/03-axios与拦截器\|axios 与拦截器]] → [[前端开发/09-融会贯通/02-Java全栈前后端联调\|前后端联调]] | CORS/JWT 打通/错误归一——全栈关键章 | 10h |
| 5 | [[前端开发/03-JS框架/Vue3/01-Vue3基础\|Vue3 基础]] → [[前端开发/03-JS框架/Vue3/02-组合式API\|组合式 API]] → [[前端开发/03-JS框架/Vue3/04-Pinia与VueRouter4\|Pinia+Router]] → [[前端开发/03-JS框架/Vue3/05-Vue3实战\|Vue3 实战]] | 框架选型默认 Vue3（模板思维对后端友好；团队用 React 则换读 React 01-04） | 24h |
| 6 | [[前端开发/08-项目实战/02-后台管理系统\|后台管理系统实战]] | 综合作品集项目，mock 可切真实 Spring Boot API | 20h |

> **检查点**: 用 Vue3(或 React) + axios 独立完成对接自己 Spring Boot 后端的完整 CRUD 前端；理解 CORS 预检请求并能独立排障联调问题；部署一套前后端分离应用（nginx + jar 同机或 compose）。
>
> 深入方向按需自取：TypeScript（[[前端开发/01-基础/TypeScript/01-TS基础与类型系统|TS 基础]]）、React 技术栈（[[前端开发/03-JS框架/React/01-React基础|React 基础]] 起步）、SSR 全栈（[[前端开发/03-JS框架/Next.js/01-Next.js基础|Next.js]]）——见 [[前端开发/引导阅读|引导阅读]] 的技术栈组合路线。
...
- [[前端开发/引导阅读|前端开发教程]] — Phase 11 前端部分的入口，按需选取路线
...
| 目录 | 内容 | 状态 |
|------|------|------|
| 前端目录 | HTML/CSS/JS/TS、Vue/React/Next/Angular、可视化、工程化与联调 (~91篇) | 已完成，入口 [[前端开发/引导阅读\|引导阅读]] |
| 服务器运维目录 | Linux 进阶、Nginx、Docker/Kubernetes、监控告警 | 规划中 |

运维目录发布后，将并入本路径形成 Phase 12+ 的部署运维闭环。
