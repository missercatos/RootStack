...
> > [!success]- 点击查看答案
> > 正确答案: C
> > **解析**: Intel 规定复位后 CS=0xF000, IP=0xFFF0，芯片组将其重映射到 `0xFFFF0`（或 `0xFFFFFFF0`），这是 BIOS 的入口地址。

> [!question] 判断题 1
> BIOS POST 完成后直接跳转到操作系统内核。 （ ）
> - [ ] ✅ 正确
> - [ ] ❌ 错误
>
> > [!success]- 点击查看答案
> > 答案: 错误
> > **解析**: BIOS 加载 MBR（主引导记录）的前 512 字节到 `0x7C00`，然后跳转到那里执行。MBR 是由操作系统安装的引导加载器，由它完成内核的加载。
...
> > [!success]- 点击查看答案
> > 正确答案: B
> > **解析**: `dw 0xAA55` 在 x86 小端序下写入的两个字节是 `0x55` 然后 `0xAA`。BIOS 检查这两个字节来确认扇区是否是有效的引导扇区。

> [!question] 判断题 1
> `int 0x13, AH=0x02` 使用 LBA（逻辑块地址）寻址模式。 （ ）
> - [ ] ✅ 正确
> - [ ] ❌ 错误
>
> > [!success]- 点击查看答案
> > 答案: 错误
> > **解析**: `AH=0x02` 使用传统的 CHS（柱面/磁头/扇区）寻址，受限于约 8GB 硬盘容量。LBA 模式使用 `AH=0x42` 扩展读，支持大硬盘。
...
```
┌────────────────────────────────────────────────────────────┐
│ Base[31:24] │G│D/B│L│AVL│ Limit[19:16] │P│DPL│S│ Type │Base[23:0]│
│             │  │   │  │   │              │ │   │ │      │          │
...
> [!question] 选择题 1
> 切换到保护模式后，为什么必须执行一条远跳转指令？
> - [ ] A. 清空内存
> - [ ] B. 刷新 CPU 的指令预取队列，使 CS 加载新的段选择子
> - [ ] C. 重置 A20 门
> - [ ] D. 初始化 GDT
>
> > [!success]- 点击查看答案
> > 正确答案: B
> > **解析**: 设置 CR0.PE=1 后，CPU 进入保护模式，但指令预取队列中的指令仍是在实模式下取的。远跳转清空预取队列，同时加载 CS 为保护模式的段选择子。

> [!question] 判断题 1
> GDT 的第一个条目（索引 0）必须设为 0。 （ ）
> - [ ] ✅ 正确
> - [ ] ❌ 错误
>
> > [!success]- 点击查看答案
> > 答案: 正确
> > **解析**: GDT[0] 是空描述符（null descriptor），处理器规定它必须为 0。如果加载 null selector 会导致 #GP（通用保护异常）。
...
逻辑地址 (selector:offset)
    │
    ▼
从 GDTR 获取 GDT 基址 → GDT[selector.index] → 获取段描述符
    │
    ▼
   Base + offset → 线性地址
    │
...
> > [!success]- 点击查看答案
> > 正确答案: B
> > **解析**: `0x10` 是 GDT 索引 2 (0x10 >> 3 = 2)，TI=0 (GDT)，RPL=0。通常用于数据段选择子。
...
> > [!success]- 点击查看答案
> > 正确答案: C
> > **解析**: `iretq`（interrupt return quadword）专用于 64 位长模式，弹出 40 字节的栈帧（RIP, CS, RFLAGS, RSP, SS）。

> [!question] 判断题 1
> 64 位长模式下，GDT 中代码段描述符的 L 位应为 0。 （ ）
> - [ ] ✅ 正确
> - [ ] ❌ 错误
>
> > [!success]- 点击查看答案
> > 答案: 错误
> > **解析**: 64 位代码段要求 L=1 和 D=0。L=1 表示 64 位代码段，L=0 且 D=1 表示 32 位兼容模式代码段。
...
> [!question] 选择题 1
> 两阶段引导器中，MBR 加载 stage2 到内存的哪个地址？
> - [ ] A. `0x7C00`
> - [ ] B. `0x10000`
> - [ ] C. `0x100000`
> - [ ] D. `0x00000`
...
> > [!success]- 点击查看答案
> > 正确答案: B
> > **解析**: 传统约定中 MBR 加载第二阶段加载器到 `0x10000`（64KB 处），该地址避开了 BIOS 数据区和中断向量表，且足够空间容纳大型引导器。
...
> > [!success]- 点击查看答案
> > 正确答案: B
> > **解析**: `lgdt` 加载全局描述符表寄存器（GDTR）。GDTR 包含 16 位 limit 和 32/64 位基址。与之对应，`lidt` 加载 IDTR。
...
> > [!success]- 点击查看答案
> > 正确答案: B
> > **解析**: `int 0x13` 是 BIOS 磁盘 I/O 服务。`int 0x10` 是视频，`int 0x15` 是系统服务，`int 0x21` 是 DOS 功能调用（非 BIOS）。

> [!question] 选择题 3
> 传统 MBR 引导扇区被 BIOS 加载到的物理内存地址是？
> - [ ] A. `0x0000`
> - [ ] B. `0x1000`
> - [ ] C. `0x7C00`
> - [ ] D. `0xFFFF`
...
> > [!success]- 点击查看答案
> > 正确答案: C
> > **解析**: IBM PC 兼容机标准规定 BIOS 将 MBR 加载到 `0x7C00`。这个地址已成为所有 x86 引导器的惯例入口。

> [!question] 选择题 4
> CR0 寄存器的 bit 0 叫什么名字？
> - [ ] A. PE (Protection Enable)
> - [ ] B. PG (Paging)
> - [ ] C. WP (Write Protect)
> - [ ] D. TS (Task Switched)
>
> > [!success]- 点击查看答案
> > 正确答案: A
> > **解析**: CR0.PE（bit 0）控制保护模式开关。0 = 实模式，1 = 保护模式。CR0.PG（bit 31）控制分页。

> [!question] 选择题 5
> A20 门开启前，CPU 最多能寻址多少内存？
> - [ ] A. 640KB
> - [ ] B. 1MB
> - [ ] C. 4GB
> - [ ] D. 16MB
...
> > [!success]- 点击查看答案
> > 正确答案: B
> > **解析**: 实模式有 20 根地址线（A0-A19），最多 1MB。但开启 A20 后，实模式下可利用第 21 根地址线访问 (0xFFFF*16 + 0xFFFF) ≈ 1MB+64KB 的高端内存区。

> [!question] 选择题 6
> 长模式下 EFER MSR 的 `LME` 位位于第几位？
> - [ ] A. bit 0
> - [ ] B. bit 5
> - [ ] C. bit 8
> - [ ] D. bit 31
...
> > [!success]- 点击查看答案
> > 正确答案: C
> > **解析**: EFER (Extended Feature Enable Register) MSR 地址 `0xC0000080`，bit 8 为 LME (Long Mode Enable)。仅当 CR0.PG=0 时才能写入 LME，然后在开启分页时同时激活长模式。

> [!question] 选择题 7
> `cpuid` 指令的输入和输出分别在哪些寄存器中？
> - [ ] A. EAX 输入, EBX+ECX+EDX 输出
> - [ ] B. ECX 输入, EAX 输出
> - [ ] C. EAX(+ECX) 输入, EAX+EBX+ECX+EDX 输出
> - [ ] D. EDX 输入, EAX 输出
>
> > [!success]- 点击查看答案
> > 正确答案: C
> > **解析**: `cpuid` 以 EAX（叶子号）和 ECX（子叶号）为输入。结果在 EAX, EBX, ECX, EDX 中返回。例如检查长模式支持用 `eax=0x80000001` 后检查 `edx bit 29`。
