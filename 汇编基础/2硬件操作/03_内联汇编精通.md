...
> > [!success]- 点击查看答案
> > 正确答案: C
> > **解析**: `"r"` = register — 编译器从通用寄存器池中任选一个。`"m"` = memory, `"i"` = immediate, `"a"` = eax/rax 特定寄存器。

> [!question] 选择题 2
> `asm volatile("mfence" ::: "memory")` 中，`"memory"` clobber 的作用是？
> - [ ] A. 将 asm 的输出写入内存文件
> - [ ] B. 告诉编译器"内存的内容可能被 asm 修改了"
> - [ ] C. 释放所有之前分配的内存
> - [ ] D. 声明 asm 需要使用内存分配
>
> > [!success]- 点击查看答案
> > 正确答案: B
> > **解析**: `"memory"` clobber 通知编译器：任何可在 asm 外部间接访问的变量（指针、全局变量等）都可能被该 asm 块修改。编译器会在 asm 前后刷新/重读所有寄存器中缓存的此类值。

> [!question] 判断题 1
> GCC 内联汇编使用 Intel 语法（与 NASM 相同）。 （ ）
> - [ ] ✅ 正确
> - [ ] ❌ 错误
>
> > [!success]- 点击查看答案
> > 答案: 错误
> > **解析**: GCC 内联汇编默认使用 **AT&T 语法**（操作数顺序相反、寄存器带 `%` 前缀、立即数带 `$` 前缀）。可通过 `.intel_syntax noprefix` 伪指令切换到 Intel 语法，但并不常见。
...
> [!question] 选择题 1
> 以下哪条内联汇编正确实现了 `inb`（从端口读 1 字节）？
> - [ ] A. `asm("inb %0, %1" : "=a"(val) : "r"(port));`
> - [ ] B. `asm volatile("inb %1, %0" : "=a"(val) : "Nd"(port));`
> - [ ] C. `asm volatile("in %0, %1" : "=r"(val) : "r"(port));`
> - [ ] D. `asm volatile("inb %0, %1" : "=a"(val) : "d"(port));`
>
> > [!success]- 点击查看答案
> > 正确答案: B
> > **解析**: `inb` 的 AT&T 语法是 `inb 端口, %al`——操作数顺序与 Intel 相反。`"=a"` 绑定 val 到 al 寄存器。`"Nd"` 是 x86 专用约束：N = 0~255 无符号立即数，d = DX 寄存器——compiler 会根据端口号大小自动选择。

> [!question] 选择题 2
> `asm goto` 的标签在汇编模板中如何引用？
> - [ ] A. `jmp %label`
> - [ ] B. `jmp %[label]`
> - [ ] C. `jmp %l[label]`
> - [ ] D. `jmp %%label`
...
> > [!success]- 点击查看答案
> > 正确答案: C
> > **解析**: 在 asm goto 模板中，标签通过 `%l[label_name]` 引用（`l` = label）。普通变量通过 `%0`/`%1` 或 `%[var_name]` 引用。

> [!question] 选择题 3
> 以下哪种约束类型让 GCC 知道变量在内联汇编中被"读写"（既输入又输出同样的值）？
> - [ ] A. `"=r"`（等号约束）
> - [ ] B. `"+r"`（加号约束）
> - [ ] C. `"&r"`（earlyclobber 约束）
> - [ ] D. `"r"`（输入约束）
...
> > [!success]- 点击查看答案
> > 正确答案: B
> > **解析**: `"+r"` 约束声明操作数是"Read-Write"——在 asm 执行前读入，执行后写回。`"=r"` 是纯输出，`"r"` 是纯输入，`"&r"` 标记输出在输入被消费前就写入。

> [!question] 选择题 4
> Linux 内核中的 `likely()`/`unlikely()` 宏背后的机制是？
> - [ ] A. GCC 的 `__builtin_expect` + 内联汇编分支提示
> - [ ] B. 内联汇编 `asm goto` 直接完成条件跳转
> - [ ] C. 硬件预测器（CPU branch predictor）
> - [ ] D. 静态分支预测（编译器生成 hint prefix）
>
> > [!success]- 点击查看答案
> > 正确答案: A
> > **解析**: `likely(x)` = `__builtin_expect(!!(x), 1)` —— GCC 内建函数，影响编译器对分支的预测策略（生成倾向性的分支代码布局）。不涉及 `asm goto`。但 `asm goto` 可以用于更复杂的自定义分支实现。

> [!question] 选择题 5
> 在 GCC 内联 asm 中，`%%rax` 的含义是？
> - [ ] A. 引用操作数 %rax
> - [ ] B. 字面意思——寄存器 %rax
> - [ ] C. 表示 rax 的地址
> - [ ] D. 编译错误——不应使用双百分号
...
> > [!success]- 点击查看答案
> > 正确答案: B
> > **解析**: GCC 内联 asm 的模板字符串中 `%` 有特殊含义（引用操作数）。要表示字面寄存器 `%rax`，必须写 `%%rax`——第一个 `%` 是转义字符。但最佳实践是使用约束让编译器管理寄存器。
