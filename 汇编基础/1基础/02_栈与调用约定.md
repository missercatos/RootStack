...
graph TD
    subgraph "栈 (高地址 → 低地址)"
        A["argv, envp<br/>← 程序启动时的栈"]
        B["函数参数 n<br/>← 第7+个参数（超出入寄存器范围的）"]
        C["返回地址<br/>← call 指令压入"]
        D["旧的 rbp<br/>← 被调用者保存"]
        E["局部变量<br/>← 函数内部分配"]
        F["..."]
...
| 题号 | 题目 | 链接 | 涉及知识点 |
|------|------|------|-----------|
| 20 | 有效的括号 | https://leetcode.cn/problems/valid-parentheses/ | 栈结构（push/pop对应） |
| 155 | 最小栈 | https://leetcode.cn/problems/min-stack/ | 栈帧设计 |
| 232 | 用栈实现队列 | https://leetcode.cn/problems/implement-queue-using-stacks/ | 双栈、调用约定思想 |
| 94 | 二叉树的中序遍历 | https://leetcode.cn/problems/binary-tree-inorder-traversal/ | 递归调用的栈帧展开 |
