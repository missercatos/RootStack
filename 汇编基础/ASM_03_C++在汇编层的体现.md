...
> 普通函数：`call 固定地址`。虚函数：`先取 vptr → 再取函数指针 → 间接 call`。多一次内存访问，这就是虚函数有额外开销的汇编根源。详见 [[../cpp教程/cpp深化教程/07_面向对象(三)多态与虚函数]]。
...
初始化列表生成更高效的汇编（少一条指令），对于 `std::string` 等非平凡类型差异更大。这也是 C++ 教程 [[../cpp教程/cpp深化教程/05_面向对象(一)类与对象基础]] 中强调初始化列表的原因。
