...
> C++ 非静态成员函数的隐式第一参数是 **this 指针**，因此在 Linux 上 this 通过 `rdi` 传递。这是 C++ 教程 [[../05_面向对象(一)类与对象基础]] 中讲解 this 指针底层机制的基础。
...
x86-64 ABI 要求 `call` 指令执行前 `rsp` 必须 16 字节对齐。`push`/`pop` 操作 8 字节，编译器自动插入 `sub rsp, 8` 等调整。在 C++ 教程 [[../05_面向对象(一)类与对象基础]] 的内存对齐章节中，对象大小必须是最大对齐值的整数倍，与此要求一脉相承。
