##   C++ 深化教程：第七章 面向对象 (四) —— 运算符重载与友元
## ==========================================================================

【🎯 本章目标】
  1. 掌握运算符重载的基本语法和底层原理
  2. 区分成员函数重载和友元函数重载的选择原则
  3. 深入理解编译器匹配运算符重载的过程
  4. 掌握类型转换运算符和C++20太空船运算符
  5. 通过5个生活场景案例综合运用前7章所有知识
...
【原理2-2】成员函数 vs 友元函数的决策树

选择成员函数重载还是友元/全局函数重载，取决于以下原则：

成员函数重载的场景：
  - 运算符的语义与类紧密相关，且左操作数必须是类对象
  - 赋值运算符(=, +=, -=, ...)
  - 下标运算符([])
  - 函数调用运算符(())
  - 成员访问运算符(->)
...
友元/全局函数重载的场景：
  - 需要对称性（如 `operator`+ 要支持 5 + obj 和 obj + 5）
  - 输出运算符(<<)需要返回ostream&
  - 需要访问私有成员但不是该类的成员（友元）
  - 左操作数不是类类型（如上面 10 + a 的例子）

必须用成员函数重载的运算符（C++标准规定）：
  =   []   ()   ->
...
public:
    ...
    // ====== 全局函数（友元）重载（支持对称性）======
    friend Number operator*(int scalar, const Number& n) {
        return Number(scalar * n.value);
    }

    friend Number operator*(const Number& n, int scalar) {
        return scalar * n;  // 委托给上面的重载
    }
...
// 使用演示：
// Number n(5);
// Number m = n + 3;     // OK，成员函数重载
// Number p = 3 * n;     // OK，友元函数重载
// Number q = n * 3;     // OK，友元函数重载
// std::cout << n;       // OK，友元函数重载
```

【原理2-3】运算符重载不会改变优先级和结合性

```cpp
class Vec {
    int x, y;
public:
    Vec(int xv, int yv) : x(xv), y(yv) { }
    Vec operator+(const Vec& other) const {
        return Vec(x + other.x, y + other.y);
    }
    Vec operator*(int scale) const {
...
int main() {
    ...
    // 以下表达式等价于 a + (b * 5)，而不是 (a + b) * 5
    // 因为 * 的优先级高于 +，这是语言规定的，无法改变
    Vec c = a + b * 5;  // * 先计算，然后 +

    // 你也不能改变运算符的结合性
    // a + b + c 总是从左到右结合：(a + b) + c
...
友元函数/类在编译器层面被授予了特殊权限。编译器在符号表中
将友元函数标记为有权访问声明它的类的私有/protected成员。
...
private:
    ...
    // 友元函数声明
    friend void auditorAccess(const BankAccount& account);
    friend class GovernmentAuditor;

public:
    BankAccount(double bal, const std::string& code)
        : balance(bal), secretCode(code) { }
};

// 友元函数可以访问私有成员
void auditorAccess(const BankAccount& account) {
    // 可以直接访问私有成员
    std::cout << "Audit: balance=" << account.balance
              << " code=" << account.secretCode << std::endl;
}

// 友元类——其所有成员函数都可以访问私有成员
class GovernmentAuditor {
public:
    void examine(const BankAccount& account) {
        std::cout << "Gov Audit: " << account.balance
                  << " " << account.secretCode << std::endl;
    }

    void freeze(BankAccount& account) {
        // account.balance = 0;  // 可以修改！
        std::cout << "Account frozen" << std::endl;
    }
};

int main() {
    BankAccount acc(9999.99, "SECRET123");
    auditorAccess(acc);
...
底层实现：
  编译器在类定义的符号条目中维护一个友元列表。当友元函数
  尝试访问类的私有成员时，编译器检查调用者是否在该友元列表中。
  这纯粹是编译期检查，运行时不产生额外开销。
...
private:
    int width, height;
    friend class RectanglePainter;  // RectanglePainter可以访问私有成员
...
public:
    void paint(const Rectangle& rect) {
        std::cout << "Painting rectangle " << rect.width
...
class Car {
public:
    void startEngine(Engine& engine);  // 声明但暂不定义
};

class Engine {
private:
    int rpm;
    friend void Car::startEngine(Engine& engine);
    // 只把Car::startEngine设为友元，而非整个Car类

public:
    Engine() : rpm(0) { }
};

void Car::startEngine(Engine& engine) {
    engine.rpm = 1000;  // 访问Engine的私有成员
    std::cout << "Engine started at " << engine.rpm << " RPM" << std::endl;
}
```
