...
到了这里可以把「查找函数怎么选」归纳成三步，照着走基本不会错：

- **先看要不要比较内容。** 要比较内容（字符串、列表、向量）就用 `member`、`assoc`、`rassoc` 这一族，它们内部用 `equal`；只比较对象标识（符号、buffer、marker、小整数）就用 `memq`、`assq`、`rassq`，它们用 `eq`，快得多。
- **再看数据是列表还是键值对。** 判断「某个元素在不在列表里」用 `member` 或 `memq`；判断「某个键对应什么值」用 `assoc` 或 `assq`；只知道值要反查键，用 `rassoc` 或 `rassq`。
- **最后看要不要把值取出来。** 已经确定是 alist 而且只想要值，直接 `alist-get` 比先 `assoc` 再 `cdr` 更简洁；需要区分「键不存在」和「值是 nil」，就退回 `assq` 判断键是否存在。

`memq` 和 `member` 返回的是从匹配处开始的整个尾部而不是布尔值，这一点既是便利也是陷阱：直接当条件用没问题（非空列表为真），但如果把返回值当成 `t` 传给别人，就会得到一个列表而不是布尔真值。想要严格的布尔值，用 `(and (member x lst) t)`，或者用 `seq-contains-p`。
...
`aref` 与 `elt` 的关系可以这样理解：`elt` 是「通用但慢」的版本，`aref` 是「专用但快」的版本。`elt` 的内部实现要对参数做类型分派，如果是列表还要走 `nth`；而 `elt` 作用在向量或字符串上时，最终也是调 `aref`。所以选择标准很清楚：

- 已经确定是向量或字符串，用 `aref`，省掉类型判断。
- 函数要同时接受列表和向量，用 `elt` 或 `seq-elt`，把分派交给运行时。
- 在循环里对**列表**按下标访问，两个都不要用——那是 O(n²)，应该先把列表转成向量再 `aref`，或者干脆改写成顺序遍历。

配套的写入接口 `aset` 只对数组有效，列表没有对应的原地赋值原语。要改列表中第 N 个元素，得用 `setcar` 配合 `nthcdr`：
...
这个写法很常见，但要记住它是**破坏性**的：如果 `l` 还和别处共享结构，改动会一起显示出来。
...
### 8.1 map.el：三种结构的统一接口

既然三种结构都在表达「映射」，Emacs 27 起内置的 `map.el` 提供了同一套读写接口，不必在调用点判断类型：

```elisp
(require 'map)

(map-elt '((a . 1) (b . 2)) 'b)          ; => 2      alist
(map-elt (list 'a 1 'b 2) 'b)            ; => 2      plist
(map-elt (let ((h (make-hash-table)))    ; => 1      hash-table
           (puthash 'a 1 h) h)
         'a)

(map-keys '((a . 1) (b . 2)))            ; => (a b)
(map-contains-key '((a . 1)) 'a)         ; => t
...
`map-put` 这个宏从 27.1 起已废弃，改用 `map-put!` 或者 `(setf (map-elt ...) ...)`。

什么时候用 `map-elt` 而不是具体的 `alist-get` / `plist-get` / `gethash`？判据是「数据结构的类型是不是这个函数的参数」。如果你写的函数签名是 `(defun my-fn (config) ...)` 而 `config` 可能是 alist 也可能是哈希表（用户配置里两种情况都常见），那就该用 `map-elt`，让分派发生在运行时。如果函数内部已经明确知道自己在处理 alist，`alist-get` 更直接，也少一层间接。

需要留意 `map-elt` 有一个和 `alist-get` 相同的弱点：它无法区分「键不存在」和「值就是 nil」。要区分，还得回到 `map-contains-key` 或者对应结构的存在性判断函数。
...
### 11.9 遍历链表时用下标而不是顺着 cdr 走

从 C 过来的人写链表遍历时，习惯写成按下标访问：

```elisp
;; 不好：每次 nth 都从头数一遍，整体是 O(n^2)
(let ((l (list 'a 'b 'c 'd)))
  (dotimes (i (length l))
    (message "%S" (nth i l))))
```

`length` 本身是 O(n)，`nth` 每次又是 O(n)，循环 n 次就是 O(n²)，而且完全不必要。Elisp 里链表遍历的正确姿势有三种，按场景选：

```elisp
;; 一、只需要元素本身：dolist 最清楚
(dolist (x (list 'a 'b 'c 'd))
  (message "%S" x))

;; 二、顺着 cdr 走：需要"剩余列表"时用这个
(let ((l (list 'a 'b 'c 'd)))
  (while l
    (message "当前 %S，剩余 %S" (car l) (cdr l))
    (setq l (cdr l))))

;; 三、需要下标：用 cl-loop，它内部维护计数器，不重复遍历
(cl-loop for x in (list 'a 'b 'c 'd)
         for i from 0
         do (message "%d: %S" i x))
```

第二种写法里的 `(while l ...)` 依赖「非空列表为真」这条性质：空列表就是 `nil`，`nil` 为假，循环自然结束。这是 Elisp 里非常惯用的模式，看到 `(while lst ... (setq lst (cdr lst)))` 就应该理解成「沿着链表走到底」。

顺带一提，`(if (= (length lst) 0) ...)` 这样的写法既慢又啰嗦，判断列表是否为空直接用 `(null lst)` 或者 `(if lst ...)`；如果列表可能很长又只想知道「有没有元素」，`consp` 是 O(1) 的。
