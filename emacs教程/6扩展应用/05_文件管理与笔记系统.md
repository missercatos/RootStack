...
;; 不询问，直接插入；调试阶段可以设为 t 观察它挑了哪个模板
(setq auto-insert-query nil)

;; 模板文件存放目录；模板文件本身是普通文本文件，
;; 里面可以用 @ 开头的占位符，auto-insert 会替换成对应内容
...
;; 为 Elisp 指定模板
(define-auto-insert "\\.el\\'" "elisp-header.el")
...
`~/.emacs.d/templates/rootstack-doc.md` 里可以写：
...
auto-insert 在模板里识别的占位符包括 `@FILE@`（文件名）、`@DATE@`（日期）、`@AUTHOR@`（`user-full-name`）等，具体清单见 `auto-insert` 的文档字符串。
