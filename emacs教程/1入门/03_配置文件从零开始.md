...
```elisp
;; 1. 让 C-k 在行尾时不删除换行符之外的东西（默认行为已符合预期，仅作说明）
;; 2. 关闭"滚动到文件末尾以外的部分"
(setq scroll-error-top-bottom t)

;; 3. 搜索时忽略大小写：全小写搜索时自动忽略大小写
(setq case-fold-search t)          ; 默认即为 t，此处显式声明以便理解
(setq search-upper-case 'not-yanks)

;; 4. 输入时不要自动把选中文本替换掉的场景（与 delete-selection-mode 配合）
(delete-selection-mode 1)          ; 现代编辑器习惯：输入即替换选中内容

;; 5. 大文件与长行保护：超过阈值的行自动切换到简化显示
(setq so-long-threshold 1000)
(global-so-long-mode 1)            ; Emacs 27 起内置，打开日志文件时的救命设置
...
```elisp
;; 11. 补全列表不自动弹窗（等你按 TAB 再看，避免干扰）
;;     这里只展示变量名，具体配置在补全章节展开
(setq completion-auto-help 'lazy)

;; 12. 确认类提问用简答形式
(setq use-short-answers t)

;; 13. 命令执行慢时不要立刻弹"仍在运行"提示
(setq max-mini-window-height 0.25)

;; 14. 让 minibuffer 记住历史（已在最小配置中）
(savehist-mode 1)
```
...
```elisp
;; 15. 用 M-o 之外的更顺手方式在窗口间跳转（windmove，内置）
(when (fboundp 'windmove-default-keybindings)
  (windmove-default-keybindings))   ; S-<left> 等方向键切窗口

;; 16. C-x C-j 打开当前文件所在目录（dired-x 提供）
(require 'dired-x)
