...
```elisp
;; 保存桌面到 ~/.emacs.d/ 下（默认位置由 desktop-path 决定）
(require 'desktop)
...
- 桌面文件默认写在 `desktop-dirname`（通常是 `~/.emacs.d/`），它可能包含你正在编辑的文件路径。多机同步配置时要把它排除（见 [[emacs教程/3配置实践/06_多机同步与配置分发|多机同步与配置分发]]）。
...
- 环境与工具：Emacs Wiki <https://www.emacswiki.org/>；Emacs 中文社区论坛 <https://emacs-china.org/>
