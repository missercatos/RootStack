...
realgud 的安装没有特殊步骤，用包管理器即可。它的官方 README 建议的加载方式是用 `M-x load-library RET realgud RET`，这条命令等价于 `(require 'realgud)`。

```elisp
(use-package realgud
  :ensure t
  :commands (realgud:gdb realgud:pdb realgud:bashdb realgud:zshdb
             realgud:rdebug realgud:trepan3k realgud:remake)
  :bind
  ;; realgud 的入口命令是 realgud:<调试器名>，直接绑到自己的前缀下
  (("C-c d g" . realgud:gdb)
   ("C-c d p" . realgud:pdb)
   ...
   ("C-c d m" . realgud:remake)))
```

`:commands` 的作用是延迟加载：realgud 只在第一次调用上面这些命令时才真正被载入，不会拖慢启动。由于 realgud 的入口命令很多，逐个列出来比用 `:defer t` 更精确。

realgud 的入口函数命名规则是 `realgud:<调试器名>`，例如 `realgud:gdb`、`realgud:pdb`、`realgud:bashdb`。它们都是交互式命令，用 `M-x` 调用即可，不需要预先启动什么。

### 4.4 源码窗口的短命令

realgud 最好用的部分是它的**源码窗口短命令**：命令窗口（调试器 REPL）之外的那个显示源码的窗口里，大多数单键都直接对应一个调试动作，不需要按前缀。

| 键位 | 动作 |
| --- | --- |
| `n` 或 `F10` | 单步跳过（next） |
| `s` 或 `SPC` 或 `F11` | 单步进入（step） |
| `f` 或 `S-F11` | 执行到当前函数返回（finish） |
| `c` 或 `F5` | 继续运行到下一个断点 |
| `b` 或 `F9` | 设置断点 |
| `D` | 按编号清除断点 |
| `e` | 求值表达式 |
...
| `q` 或 `S-F5` | 退出调试 |
| `S` | 跳到命令窗口 |
| 鼠标中键 | 在 tooltip 里查看光标下的变量 |

这套短命令是 realgud 区别于内置 GUD 最明显的地方：内置 GUD 的键位是 `C-c C-x` 形式的前缀组合，realgud 则是单键。代价是这些单键只在 realgud 的源码窗口里生效，不会污染其他缓冲区。

realgud 打开两个窗口：**命令窗口**（调试器 REPL）与**源码窗口**。源码窗口里的当前位置用箭头指示，走过的位置用灰色箭头标注。有些调试器（例如 `realgud:gdb`）在你在命令窗口里输入 `start` 之前不会弹出源码窗口，这是正常行为。

### 4.5 与内置 GUD 的关系

两者的关系需要说准确，避免混淆：

- **不是替代关系，是并列关系**。realgud 没有替换内置的 GUD，它是另一套实现。安装 realgud 之后，`M-x gdb` 仍然走内置的 gdb-mi，`M-x realgud:gdb` 才走 realgud。两条路线的缓冲区、键位、命令行界面都是独立的。
- **不要同时用两套调试同一个程序**。realgud 与 GUD 各自维护自己的调试器进程与会话状态，同时开会得到两个互相不知情的调试器，容易造成混乱。
- **调试器本身要另行安装**。realgud 只是前端，`bashdb`、`zshdb`、`trepan3k`、`remake` 这些调试器需要你自己装（`sudo apt install bashdb`、`pipx install trepan3k` 等），而 `gdb` 与 `pdb` 一般系统里已经有。
- **它也在 GNU ELPA 上**。realgud 除了 MELPA 之外还发布在 GNU ELPA，如果你的配置里只启用了 GNU ELPA，也能装上。

什么时候用 realgud：**你要调试 shell 脚本、Makefile、或者 Ruby 与 Perl**。这些场景内置 GUD 要么不支持，要么支持得很弱，realgud 是目前最成熟的选择。纯 C/C++ 与 Python 场景下，内置 GUD 或 dap-mode 已经够好，不必额外引入 realgud。
