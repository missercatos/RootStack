...
eshell 有两个可选的启动文件：`eshell-rc-script` 对每次新开的 eshell 生效，`eshell-login-script` 只在登录 shell 场景下生效（判断依据是 `eshell-login-script` 提供的机制，即第一个 eshell 缓冲区）。它们的内容就是普通的 eshell 命令，适合放别名与 PATH 调整；如果想让 eshell 的行为和 bash 尽量靠近，也可以直接 `source ~/.bashrc`，但要注意 bash 里的语法（例如函数定义、数组）eshell 不一定认。

目录相关的快捷写法值得单独记住，它们能显著减少敲键盘的次数：

- `cd ..` 或者别名 `..` 回到上级目录；`cd -` 回到上一个访问过的目录。
- `cd =/` 跳到当前项目的根目录（`=` 对应当前缓冲区的项目根，具体依据 `eshell-directory-name` 与项目识别规则）。
- `cd =` 之后的路径补全会以项目根为基准，因此在深层目录里跳来跳去很快。
- `~` 与 `~/` 与 bash 一致；`$HOME` 也直接可用。
- 在 eshell 里 `cd` 到 `dired` 正在浏览的目录是很自然的事：先用 `C-x d` 浏览，找到目录后记下路径，再在 eshell 里 `cd` 进去。两个缓冲区共享同一套 TRAMP 连接，所以在远程目录上也能这样配合。

eshell 的提示符可以完全自定义，`eshell-prompt-function` 决定提示符内容，`eshell-highlight-prompt` 决定是否给提示符加高亮。默认提示符是「路径加 `$`」，如果你习惯看 Git 分支，可以在提示符里插入分支名，代价是每次渲染提示符都要跑一次 Git 命令，在大仓库上会拖慢每一条命令的响应，不建议这么做。
...
一个经验判断：**如果一条命令的输出超过一屏并且你不会去点击其中的文件名，它更适合放在 eshell；如果输出会持续刷新（进度条、监控面板），它必须放进真终端。** 有了这条规则，绝大多数选择都不需要犹豫。
...
遇到「某个键到底被谁处理」的疑问时，不要猜，直接在终端缓冲区里按 `C-h k` 再按那个键：回显会告诉你这个键在当前缓冲区里解析到哪个命令、来自哪张键映射。如果解析结果是 `vterm--self-insert` 或 `eat-self-input`，说明它会被送给终端程序；如果是别的 Emacs 命令，说明被 Emacs 截获了。想看整张键位表，按 `C-h m`（`describe-mode`）会列出当前主模式与启用的小模式的完整说明与键位。

粘贴也是一个容易被忽视的冲突点。在 vterm 与 eat 里，向终端粘贴大量文本时，正规做法是使用「括号粘贴」（bracketed paste）模式，让终端程序知道这是粘贴而不是逐字符输入，否则 readline 之类的程序会把粘贴内容里的换行当成多次回车，把多行命令逐行执行掉。vterm 与 eat 都实现了 `xterm-paste` 的桥接（键映射里能看到 `[remap xterm-paste]` 与 `xterm-paste` 的绑定），因此在图形界面下用 `C-y` 或鼠标中键粘贴通常能正确走括号粘贴通道；如果发现粘贴多行命令时行为异常，先检查终端程序自己是否支持括号粘贴。

复制方向的习惯也值得建立：在 vterm 里首选 `C-c C-t` 进入复制模式，用 Emacs 的选区与 `M-w` 复制，理由是终端里用鼠标框选得到的是带换行与折行的画面文本，往往需要手工清理，而复制模式里的文本就是缓冲区里的真实内容。需要复制全部回滚历史时，`C-x h`（全选）再 `M-w` 更省事。
