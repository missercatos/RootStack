...
```
虚拟地址 (48-bit effective):
┌──────────┬──────────┬──────────┬──────────┬────────────┐
│  PML4    │  PDPT    │   PDT    │   PTE    │   Offset   │
│  (9 bit) │  (9 bit) │  (9 bit) │  (9 bit) │  (12 bit)  │
└─────┬────┴─────┬────┴─────┬────┴─────┬────┴──────┬─────┘
      │          │          │          │           │
      ▼          ▼          ▼          ▼           ▼
   ┌──────┐  ┌──────┐  ┌──────┐  ┌──────┐    ┌─────────┐
   │ PML4 │→│ PDPT │→│  PD  │→│  PT  │    │ 物理页帧  │
   │ 表   │  │ 表   │  │ 表   │  │ 表   │    │ 4KB 页   │
   └──────┘  └──────┘  └──────┘  └──────┘    └─────────┘

每次转换需要 4 次内存访问，因此 CPU 使用 TLB (Translation Lookaside Buffer)
缓存最近的转换结果。
```
...
│    0x48 0x89 0xF8... │
│                      │
│  subtract:           │ ← subtract 函数的机器码
│    0x48 0x89 0xD8... │
│                      │
└──────────────────────┘

函数指针 fp = add 存储的值 = add 函数第一条指令的地址 = 0x400520 (示例)
