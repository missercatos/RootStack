...
**408 补充**：WAN 的核心协议是 IP 和 BGP；LAN 的核心协议是以太网和 ARP；MAN 在 408 中较少考察，但城域网的概念需知。
...
**Nyquist 与 Shannon 定理 (物理层考点，详见 [[B_物理层]])**：
...
> **408 辨析核心**：发送时延与传播时延是**独立**的概念。发送时延取决于分组长度和发送速率，发生在主机/路由器内部；传播时延取决于物理距离和信号速度，发生在传输介质上。两者可以同时发生 (流水线效应)。
...
中国考研 408 (计算机学科专业基础综合) 采用**五层模型**作为教学标准，在 OSI 七层基础上合并了表示层和会话层到应用层。
...
| 层 | 408 五层模型 (推荐) | OSI 七层 | TCP/IP 四层 | 本教程对应章节 |
|:--:|-------------------|---------|------------|:------------:|
...
**408 统一采用五层模型的原因**：教学上，五层模型既保留了 OSI 的逻辑清晰性 (物理层和数据链路层分别处理 bit 和 frame)，又避免了对表示层/会话层的过度抽象 (这两层在现代互联网中几乎不独立存在)。实践中根据协议查找对应层次即可。
...
> **408 对"分层原则"的总结**：`分层 = 越高层离用户越近、越低层离硬件越近。N 层为 N+1 层提供服务，使用 N-1 层提供的服务。`
...
> **详细内容**参见：[[../red_team/网安基础知识/01-计算机网络基础]] — 红队视角的 OSI 七层攻击与防御详解。本节侧重 408 理论框架与实战的映射关系。
...
CNI (Container Network Interface) 定义了容器运行时与网络插件之间的接口规范，本质上就是 408 中**接口 (Interface)** 概念的工程实现。
