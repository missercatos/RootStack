...
auditd 是内核级审计系统，可精细记录系统调用、文件访问、用户操作等事件。详见 [[36-Linux安全执行环境]]。
...
dm-verity 是 Linux 内核的 device-mapper 目标，提供只读块设备的透明完整性验证。详见 [[40-dm-verity与完整性保护]]。
...
- [[22-防火墙与安全]] — 防火墙配置（iptables/nftables/firewalld/ufw）
- [[21-SSH远程管理]] — SSH 服务安全配置（密钥认证、端口安全）
- [[36-Linux安全执行环境]] — seccomp、capabilities、Landlock 等深入讲解
- [[40-dm-verity与完整性保护]] — dm-verity 完整性验证、Secure Boot、IMA/EVM 详解
- [[07-用户与权限管理]] — 用户、组与文件权限基础
- [[08-进程管理]] — 进程管理与安全上下文
