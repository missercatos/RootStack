...
> - A. 将 BPF 字节码编译为机器码
> - B. 确保 BPF 程序安全运行，不会崩溃内核
> - C. 管理 BPF Map 的内存分配
> - D. 将 C 代码编译为 BPF 字节码
>
> > [!success]- 点击查看答案
> > **B**
> > 验证器负责在加载 BPF 程序时进行静态分析，确保所有路径终止（无无限循环）、内存访问边界合法、无未初始化变量使用，防止 BPF 程序导致内核崩溃。

> [!question]- 选择题 2：tcpdump 使用 BPF 过滤的主要优势是什么？
> - A. 可以修改网络数据包
> - B. 在内核中直接过滤，减少内核到用户态的数据复制
> - C. 可以解密 HTTPS 流量
> - D. 支持远程抓包
>
> > [!success]- 点击查看答案
...
> > [!success]- 点击查看答案
> > **B**
> > `-g` 参数表示在采样时同时记录调用栈信息，这对于生成火焰图和分析函数调用链关系至关重要。

> [!question]- 选择题 5：bpftrace 中 `@[comm] = count()` 语句的含义是什么？
> - A. 统计名为 comm 的进程的 CPU 使用率
> - B. 以进程名为 key，统计触发次数
> - C. 打印当前进程名
> - D. 设置 comm 变量的值为当前计数
>
...
> - A. tracepoint 只能追踪用户态函数
> - B. tracepoint 是内核中的稳定追踪点，kprobe 是动态函数追踪
> - C. kprobe 性能更好
> - D. tracepoint 需要编译内核模块
>
> > [!success]- 点击查看答案
> > **B**
> > tracepoint 是内核源码中定义的稳定追踪点，接口不随内核版本变化；kprobe 是动态追踪，可以挂载到任意内核函数入口/出口，但可能随内核版本变化而失效。

> [!question]- 选择题 8：生成火焰图（Flame Graph）的标准流程第一步是什么？
> - A. 安装 Graphviz
> - B. 使用 `perf record -F 99 -g` 进行采样
> - C. 运行 `flamegraph.pl` 脚本
> - D. 使用 `perf report` 分析结果
>
> > [!success]- 点击查看答案
> > **B**
...
> > [!success]- 点击查看答案
> > **C**
> > bpftrace 专为快速诊断设计，支持简洁的一行命令语法来追踪内核/用户空间事件，学习曲线低，非常适合即时问题诊断。
