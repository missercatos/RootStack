...
本教程按知识领域划分为八个部分，共 **72 章**（主教程，含第 0 章）+ 发行版专项内容 + WSL 专项目录。
...
从分区表到文件系统，从逻辑卷到虚拟化底座——存储与虚拟化专题。

| 章节 | 标题 | 核心内容 |
|------|------|---------|
| [[66-Linux分区结构全解]] | Linux 分区结构全解 | 分区表、MBR/GPT、引导分区、对齐、分区方案 |
| [[67-分区工具实战]] | 分区工具实战 | fdisk/gdisk/parted、分区创建与救援、备份恢复分区表 |
| [[68-LVM逻辑卷管理]] | LVM 逻辑卷管理 | PV/VG/LV、在线扩容缩容、快照、thin provisioning |
| [[69-Btrfs从入门到生产]] | Btrfs 从入门到生产 | 子卷、快照、RAID、压缩、scrub、生产实践 |
| [[70-ZFS实战指南]] | ZFS 实战指南 | 存储池、数据集、快照、发送接收、ARC 调优 |
| [[71-libvirt与QEMU-KVM]] | libvirt 与 QEMU/KVM | 三层架构、分发行版安装、virsh/存储池/网络/快照/迁移、裸 QEMU 对比 |
