...
`/proc/sys/` 子目录下的文件可以**读写**，用于动态调整内核参数。详见 [[50-BPF与系统追踪]] 中关于 sysctl 的内容。
...
NixOS 的结构与 FHS 差异极大（参见 [[48-不可变系统]]）：
...
- [[36-文件系统设计]] — 深入理解文件系统设计
- [[42-文件系统深入]] — 文件系统底层原理
- [[48-不可变系统]] — NixOS 等不可变系统的文件结构
- [[12-存储管理与磁盘操作]] — 分区、挂载与存储管理
- [[44-压缩与归档工具大全]] — 文件打包与压缩
