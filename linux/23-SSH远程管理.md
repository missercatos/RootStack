...
上面看到了连接需要知道用户名，ip地址，以及开放端口。这里讲解一下如何获取客户端（你要连接的机器）的IP地址。

首先用户名是不需要考虑的，服务器（客户端）主人会设置并告知你，在合法的情况下，用户端（你使用的电脑）是没法得知服务器用户名的和公网IP地址的，只能通过主人告知。

但是假如你是服务器主人，你需要用下面的方法去得知自己服务器的IP地址。
（*注意是linux环境，本教程的所有操作都是基于linux环境）

查看本机局域网IP（注意只有当客户端和用户端处在同一wifi或者内网的情况下才有效）
```
# 方法1（最经典常用）
hostname -I  #老式服务器，各别发行版已经不再自主安装，需要自行下载

# 方法2（查看详细信息，通常网卡叫 eth0 或 ens33）
ip addr show  #或者简写 ip a

# 方法3（传统命令，需安装net-tools）
ifconfig
```

查看公网IP（当你的主机和你要连接的主机不在同一个网络情况下，比如在家里想要连接公司服务器）（需要在服务器执行，在你的电脑上执行得到的是你自己设备的公网ip)

```
curl ifconfig.me 
curl ip.sb  （返回速度最快，但是有时候解析会不正常）
curl icanhazip.com （一般最常用）

假如服务器没装curl，可以用下面的方法
wget -qO- ifconfig.me
wget -qO- ip.sh
wget -qO- icanhazip.com
```
成功之后会看到类似123.45.67.89的一串数字
但是有以下几个注意点。

1.  假如你看到的是100.64.0.0/10 网段，比如100.64.xxx.xxx,或者100.127.xxx.xxx
>说明你处于 运营商级 NAT（CGNAT） 之下，也就是你的宽带根本没有独立的公网 IP，而是和几百人共享一个出口。这种情况下，在你家路由器上做端口转发也无效，因为运营商的上级路由器不给你转发。
>
2.   假如说你看到的是10.x.x.x、172.16.x.x 或 192.168.x.x
>说明 curl 结果被拦截了，可能你公司网络有透明代理，真正的外网 IP 被藏起来了。
>
3. 假如说看到的是其他任意数字（如 39.144.xx.xx、218.xx.xx.xx 等
>那么就是有效的公网IP地址，可以进行接下来的步骤了。

在服务器上查看网卡IP 
```
`ip route | grep default`
```
查看网关 IP 是否和你的公网 IP 在同一个网段。如果网关是 10.0.0.1，而公网是 39.144.xx.xx，那就正常（说明网关在内网，出口在运营商那）

### 如果发现不是真公网 IP（或无法做端口转发），怎么办？
方案 A：Tailscale 或 ZeroTier。双方安装后，两台电脑会获得一个虚拟的 100.x.x.x 内网 IP，你们直接 ssh 用户名@100.x.x.x 就能连，无视任何防火墙和 NAT。

方案 B：Sshwifty 或 ngrok 的 TCP 隧道。在 Arch 上运行一条命令，将 SSH 端口暴露到一个临时生成的公网域名上，朋友访问那个域名就能连你。

### NAT和端口转发
*拿到公网 IP 并不代表对方一定能连上！*

现在绝大多数家庭宽带和公司网络都在路由器后面。你查到的公网 IP 实际上是路由器的 IP，而不是你 Linux 电脑的。外网请求到达路由器后，路由器不知道该转发给谁，就会拒绝连接
必须登录路由器管理后台（通常是 192.168.1.1），找到 “端口映射” 或 “虚拟服务器”，添加规则：
1. 外部端口：22（22是默认端口，建议改成 10000+ 的高位端口，防暴力破解，比如10086“开个玩笑，只是举个例子，不会真有人设置成10086吧？？？”）
2. 内部 IP：填你刚才查到的局域网 IP（如 192.168.1.100）
3. 内部端口：22
4. 协议选择：TCP
设置好后，告诉对方连接地址：ssh 用户名@公网IP -p 映射的外部端口 （-p写在用户名前面也可以）
...
```
然后你在设置好端口之后要放行端口
如果是 firewalld（CentOS/RHEL 系列）：
```
sudo firewall-cmd --add-port=xxxx/tcp --permanent   # 把xxxx换成你改的端口
sudo firewall-cmd --reload
sudo firewall-cmd --list-ports  # 查看是否添加成功
```
如果是 ufw（Ubuntu/Debian 系列）：
```
sudo ufw allow 2222/tcp          # 把2222换成你改的端口
sudo ufw reload
sudo ufw status                  # 查看是否放行
```



*检查一次，防止一些找不出来的莫名其妙小问题：*
1. 确认服务器有没有启动ssh（一般linux服务器系统是默认启动的，这里检查一遍）
```
systemctl status sshd   # 或 systemctl status ssh

确保状态是 active (running)。没开的话执行 sudo systemctl start sshd。
sudo systemctl enable sshd 设置为开机自启
```
2. 防火墙是否放行端口，根据你设置开放的端口进行检查
```
作为服务端，你可以随时查看 SSH 到底在监听哪个端口，以及防火墙有没有拦路：
sudo ss -tlnp | grep ssh

输出 0.0.0.0:22 表示正在监听所有 IP 的 22 端口
```
...
如果 xx 端口被拦截，需要执行放行命令（如 `sudo ufw allow xx`）。
```
...
Q:我在连接服务器的时候卡住不动，没反应，最后提示 Connection timed out（连接超时）怎么办

原因：你的 SSH 请求包发到了 125.45.126.44，但对方的路由器没有把 10086 端口的数据转给他电脑，或者运营商拦截了。
排查（请对方操作）：

1. 检查路由器 WAN 口 IP 是否变化：对方服务器联网的路由器后台（192.168.1.1），看“WAN口状态”里的 IP。必须和你现在敲的 125.45.126.44 完全一致。如果路由器重启过，公网 IP 可能变了，需要用 curl ifconfig.me 重新查新的 IP。    
2. 检查端口映射是否生效（最易错）：登录服务器的路由器，确认“端口映射”里的内部 IP，必须和当前服务器的局域网 IP（用 ip a 查出来的 192.168.x.x）一模一样。如果电脑重启后 IP 变了（比如从 .108 变成 .109），映射就失效了，必须改回新的 IP。
3. 检查防火墙：在服务器终端执行（临时关闭防火墙测试）：
```
sudo iptables -F   # 清空防火墙规则（重启后会恢复，仅测试用）
```

Q:连接别人服务器的时候瞬间报错 Connection refused（连接被拒绝）

原因：数据包成功到达了对方电脑，但对方电脑的 10086 端口没有程序在监听（SSH 服务没开，或者没改对端口）。
排查（请对方操作）：
1. 确认 SSH 服务是否在 对你开放端口的 监听：
```
sudo ss -tlnp | grep xxxxxx #改成你的端口号
```
如果没有任何输出，说明 SSH 没在 端口跑

2. 确认 `/etc/ssh/sshd_config` 里的端口号：
   ```
...
Q:弹出了 Are you sure you want to continue connecting (yes/no)?
说明网络完全通了。你直接输入 yes 回车

Q:提示 `Permission denied (publickey,password)`
用户名 不对，或者对方没给 你登录的用户设置密码，或者 SSH 配置禁止了密码登录。
让对方给你的账户设置密码：
```
sudo passwd user 
```
