...
1. 为什么删除大量文件后 vhdx 不会变小？compact 前为什么必须 `wsl --shutdown`？
2. 你的 WSL IP 是 `172.28.100.5`，同事的电脑能直接 curl 这个地址吗？给出两种让它可达的方案。
3. 连接 VPN 后 WSL 中 `ping 223.5.5.5` 通但 `curl baidu.com` 失败，问题在哪一层？最小修复动作是什么？
4. mirrored 模式下还需要 portproxy 吗？为什么它从机制上消灭了"IP 漂移"问题？
