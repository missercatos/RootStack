...
rsyslog 是大多数 Linux 发行版的默认 syslog 实现，功能强大且模块化。[[11-systemd服务管理|systemd]] 的 journald 可以接管日志，但 rsyslog 仍广泛用于 `/var/log/` 传统文本日志。
...
[[11-systemd服务管理|systemd]] 自带的 `journald` 是一个现代化的二进制日志系统，与传统文本日志有根本性差异。
