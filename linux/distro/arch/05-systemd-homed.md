...
> [!question]- 选择题 1：systemd-homed 的核心理念是什么？
> - A. 将所有用户信息集中存储在 LDAP 中
> - B. 将用户身份信息和数据自包含在 home 目录中
> - C. 使用 NFS 实现网络用户漫游
> - D. 用容器隔离每个用户
>
> > [!success]- 点击查看答案
> > **B**
> > systemd-homed 的核心创新是将用户的身份信息（JSON 记录）封装在 home 目录内，使之可加密、可迁移、可漫游。

> [!question]- 选择题 2：systemd-homed 推荐使用的存储后端是？
> - A. directory
> - B. fscrypt
> - C. luks
> - D. cifs
...
> > [!success]- 点击查看答案
> > **C**
> > LUKS 后端是推荐选择，它提供 LUKS2 加密磁盘镜像，登录时自动挂载解密，登出后自动锁定，安全性最高。

> [!question]- 选择题 3：homectl 创建用户时，哪个参数用于指定磁盘镜像大小？
> - A. --home-size
> - B. --disk-size
> - C. --luks-size
> - D. --volume-size
...
> > [!success]- 点击查看答案
> > **B**
> > `--disk-size` 参数用于指定 LUKS 磁盘镜像的大小，例如 `--disk-size=50G`。

> [!question]- 判断题 4：systemd-homed 管理的用户在登出所有会话后，LUKS 后端的 home 目录会自动锁定（卸载并加密锁定）
> - A. 正确
> - B. 错误
>
> > [!success]- 点击查看答案
> > **A. 正确**
> > LUKS 后端在用户登出所有会话时自动卸载并锁定加密镜像，这是其安全性的关键特性。

> [!question]- 选择题 5：在 PAM 配置中，pam_systemd_home.so 应该放在 pam_unix.so 的什么位置？
> - A. 之后，使用 required
> - B. 之前，使用 sufficient
> - C. 之后，使用 optional
> - D. 之前，使用 required
...
> > [!success]- 点击查看答案
> > **B**
> > pam_systemd_home.so 必须在 pam_unix.so 之前，且使用 sufficient 关键字，这样 homed 用户认证成功后可以跳过传统 pam_unix 认证。

> [!question]- 选择题 6：systemd-homed 的用户记录以什么格式存储在 ~/.identity 文件中？
> - A. INI 格式
> - B. YAML 格式
> - C. JSON 格式
> - D. TOML 格式
...
> > [!success]- 点击查看答案
> > **C**
> > systemd-homed 使用 JSON 格式存储用户记录，包含用户名、存储后端、组成员、资源限制等全部信息。

> [!question]- 判断题 7：systemd-homed 适合用于管理系统服务账户（如 http、mysql 用户）
> - A. 正确
> - B. 错误
>
> > [!success]- 点击查看答案
> > **B. 错误**
> > 系统服务账户不适合使用 homed，因为服务需要在用户未登录时就能访问其工作目录。系统服务账户应使用 systemd-sysusers 管理。

> [!question]- 选择题 8：要为 homed 用户添加 FIDO2 硬件令牌认证，应使用什么命令？
> - A. homectl update alice --fido2-device=auto
> - B. homectl auth alice --add-fido2
> - C. homectl create alice --security=fido2
> - D. passwd --fido2 alice
>
> > [!success]- 点击查看答案
> > **A**
> > `homectl update alice --fido2-device=auto` 可以为已有用户添加 FIDO2 认证方式，自动检测连接的 FIDO2 设备。

> [!question]- 选择题 9：解决 SSH 登录 homed 用户时公钥无法读取的问题，应在 sshd_config 中配置什么？
> - A. PubkeyAuthentication yes
> - B. AuthorizedKeysCommand /usr/bin/userdbctl ssh-authorized-keys %u
> - C. PermitRootLogin yes
> - D. ChallengeResponseAuthentication yes
>
> > [!success]- 点击查看答案
> > **B**
> > 由于 homed 用户未登录时 home 目录被锁定，无法读取 ~/.ssh/authorized_keys，需使用 AuthorizedKeysCommand 从用户记录中提取公钥。

> [!question]- 判断题 10：systemd-homed 支持将用户的 LUKS 加密 home 镜像放在 USB 设备上，插入其他运行 homed 的机器即可登录
> - A. 正确
> - B. 错误
>
> > [!success]- 点击查看答案
> > **A. 正确**
> > 这是 systemd-homed 的"可移植用户"功能。homed 会自动扫描可移动设备上的 .home 镜像，读取内嵌的 JSON 记录并动态分配 UID，实现即插即用的用户漫游。
