...
> [!question]- 选择题 1：partner 分配器中，order=3 表示分配多少页？
> - A. 3 页
> - B. 6 页
> - C. 8 页
> - D. 12 页
...
> > 答案
> > **C**
> > order=3 表示 2^3 = 8 页。标准页为 4KB，所以是 32KB。

> [!question]- 选择题 2：`malloc()` 与 `kmalloc()` 的主要区别是什么？
> - A. 没有任何区别
> - B. malloc 返回虚拟连续内存（用户空间），kmalloc 返回物理连续内存（内核空间）
> - C. malloc 速度更快
> - D. kmalloc 返回的内存更大
>
> > 答案
> > **B**
> > `malloc` 是 C 库函数，为用户空间分配虚拟连续内存；`kmalloc` 是内核函数，分配物理连续内存。

> [!question]- 选择题 3：`/proc/sys/vm/swappiness` 设为 10 的含义是什么？
> - A. 使用 10% 的 swap 空间
> - B. 系统只有在内存严重不足时才使用 swap
> - C. 倾向于使用 swap 而非回收页面缓存
> - D. 禁用 swap
>
> > 答案
> > **B**
> > swappiness 较低时，内核优先回收文件缓存而不将匿名页换出到 swap。

> [!question]- 选择题 4：TLB 的全称是什么？它缓存什么？
> - A. Transaction Lookaside Buffer，缓存文件数据
> - B. Translation Lookaside Buffer，缓存页表项（虚拟→物理地址映射）
> - C. Translation Latency Buffer，缓存磁盘 I/O
> - D. Temporal Load Balancer，缓存 CPU 负载
>
> > 答案
> > **B**
> > TLB 是 CPU 内部的硬件缓存，存储最近使用的虚拟地址到物理地址的映射，避免每次访存都查询页表。

> [!question]- 选择题 5：THP（Transparent Huge Pages）是什么？
> - A. 一种新型文件系统
> - B. 自动将连续普通页合并为 2MB 大页的内核特性
> - C. 一种内核调试工具
> - D. 一种 swap 优化策略
>
> > 答案
> > **B**
> > THP 自动将 512 个连续的 4KB 页合并为 2MB 大页，减少 TLB 压力。但可能增加内存碎片和分配延迟。

> [!question]- 判断题 6：Linux 中 `free -h` 显示的 "available" 列通常大于 "free" 列。
> - A. 正确
> - B. 错误
>
> > 答案
> > **A. 正确**
> > "available" 包含了可回收的缓存（buff/cache），所以通常大于 "free" 列。这是判断内存是否真正不足的正确指标。

> [!question]- 选择题 7：`oom_score_adj` 设为 -1000 表示什么？
> - A. 进程有最高优先级被杀
> - B. 进程完全免疫 OOM Killer
> - C. 进程可以被安全杀死
> - D. 没有特殊含义
...
> > 答案
> > **B**
> > `oom_score_adj` 范围 -1000 到 1000，-1000 表示进程完全免疫 OOM Killer。

> [!question]- 选择题 8：NUMA 架构中，"node distance" 表示什么？
> - A. 节点间的物理距离（厘米）
> - B. CPU 访问该节点内存的相对延迟代价
> - C. 节点间网络延迟
> - D. 节点上运行的进程数
>
> > 答案
> > **B**
> > node distance 表示 CPU 访问不同 NUMA 节点内存的相对成本，本地访问通常为 10，远程访问为 21。

> [!question]- 判断题 9：`/proc/sys/vm/overcommit_memory=2` 表示允许无限制的内存过量分配。
> - A. 正确
> - B. 错误
>
> > 答案
> > **B. 错误**
> > 0 是启发式过量分配，2 表示禁止过量分配，commit 总量受 `(物理内存 * overcommit_ratio / 100) + Swap` 约束。

> [!question]- 选择题 10：伙伴系统中，两个相邻的 order=0 空闲块可以合并为？
> - A. order=0
> - B. order=1
> - C. order=2
> - D. 不能合并
...
> > 答案
> > **B**
> > 两个 order=0 的空闲页（4KB）合并为 order=1 的一个 8KB 空闲块。合并要求它们是"伙伴"（地址连续且出自同一上级块）。

---

> **交叉链接**：内存管理与进程调度紧密相连，可延伸阅读 [[32-存储管理]]、[[33-虚拟存储与页面置换]]。性能优化视角参考 [[37-系统调优与性能分析]]。OOM Killer 与 cgroups 策略在 [[09-systemd服务管理]] 中也有涉及。
