...
> 拥有 AES-NI 指令集的 CPU 下，AES-XTS 加密的吞吐损失通常在 5-10% 以内。不具备硬件加速的系统加密性能下降可能超过 50%。详见 [[28-系统安全加固与审计]] 中的 dm-verity 部分。
...
> **dm-verity 的详细配置**（包括 veritysetup、Systemd 集成、与 Secure Boot 的配合）将在 [[28-系统安全加固与审计]] 中深入讲解，其中涵盖了从内核命令行签名到 IMA/EVM 的完整验证链路。
...
> 详细的 LVM 实战操作请参见 [[12-存储管理与磁盘操作]]，其中包括分区表管理、文件系统创建和挂载的完整流程。
...
- [[12-存储管理与磁盘操作]] — 分区表、文件系统创建、挂载管理与 LVM 基础
- [[42-文件系统深入]] — VFS 层架构、ext4/XFS/Btrfs/ZFS/F2FS 内部设计
- [[28-系统安全加固与审计]] — dm-verity 完整性保护、Secure Boot、磁盘加密安全策略
- [[40-引导流程与GRUB]] — LUKS 加密的根文件系统引导流程
- [[38-Linux内核基础与模块]] — 内核模块加载与 Device Mapper 驱动管理
- [[45-系统错误排查与日志分析]] — I/O 错误日志分析与存储故障诊断
