...
> [!question]- 选择题 1：USE 方法论中的三个字母分别代表什么？
> - A. Use, System, Error
> - B. Utilization, Saturation, Errors
> - C. User, Space, Environment
> - D. Unit, Standard, Execution
>
> > 答案
> > **B**
> > USE 方法由 Brendan Gregg 提出：Utilization（利用率）、Saturation（饱和度）、Errors（错误）。

> [!question]- 选择题 2：top 输出中，`wa` 列持续较高说明什么？
> - A. CPU 使用率过高
> - B. 缺少可用内存
> - C. 存在磁盘 I/O 瓶颈
> - D. 网络连接过多
...
> > 答案
> > **C**
> > `wa`（iowait）是 CPU 在等待 I/O 操作完成的时间占比，持续高 wa 是磁盘瓶颈的典型信号。

> [!question]- 选择题 3：`vmstat` 输出中 `si` 和 `so` 列持续大于 0 说明什么？
> - A. 系统正在活跃地使用 swap，存在内存压力
> - B. CPU 调度器工作正常
> - C. 网络流量很高
> - D. 系统空闲
>
> > 答案
> > **A**
> > `si`（swap in）和 `so`（swap out）持续非零表示系统频繁在内存和 swap 间交换页面，内存不足。

> [!question]- 选择题 4：`ss` 命令的主要作用是什么？
> - A. 查看文件系统使用量
> - B. 查看 socket 连接状态
> - C. 监控 CPU 温度
> - D. 显示系统服务
...
> > 答案
> > **B**
> > `ss`（Socket Statistics）是现代 Linux 上查看 socket 连接的标准工具，已取代 netstat。

> [!question]- 选择题 5：`perf record -g` 中 `-g` 参数的作用是什么？
> - A. 只追踪图形界面进程
> - B. 记录调用栈（call graph）信息
> - C. 以 GDB 模式运行
> - D. 使用 GPU 加速
>
> > 答案
...
> [!question]- 选择题 6：`sar -r -f /var/log/sysstat/sa28` 命令查看的是什么？
> - A. 实时内存使用
> - B. 本月 28 号的历史内存数据
> - C. 最近 28 分钟的 I/O 数据
> - D. 28 小时内的网络数据
...
> > 答案
> > **B**
> > `-f /var/log/sysstat/sa28` 指定读取 28 号的历史数据文件，`-r` 是内存报告。

> [!question]- 判断题 7：`strace` 用于跟踪库函数调用，`ltrace` 用于跟踪系统调用。
> - A. 正确
> - B. 错误
>
> > 答案
> > **B. 错误**
> > 恰恰相反：`strace` 跟踪系统调用，`ltrace` 跟踪动态库（libc 等）函数调用。

> [!question]- 选择题 8：`/etc/security/limits.conf` 中，`nofile` 限制控制的是什么？
> - A. 文件系统最大容量
> - B. 进程能打开的最大文件描述符数量
> - C. 目录下允许的最大文件数
> - D. 单个文件最大大小
...
> > 答案
> > **B**
> > `nofile` 控制进程最大可打开的文件描述符数，对应 `ulimit -n`。

> [!question]- 选择题 9：以下哪个工具最适合查看"过去某天某个时段的系统性能数据"？
> - A. top
> - B. htop
> - C. sar
> - D. vmstat
...
> > 答案
> > **C**
> > `sar` 通过 sysstat 定期采集数据到 `/var/log/sysstat/`，可以回溯查询任意历史时段。

> [!question]- 判断题 10：`lsof /path/to/file` 可以查看当前哪个进程正在使用该文件。
> - A. 正确
> - B. 错误
>
> > 答案
> > **A. 正确**
> > `lsof` 列出打开的文件，指定文件路径可以反向查找占用该文件的进程，在 `umount` 失败时非常有用。

---

> **交叉链接**：性能分析涉及多个子系统，可结合 [[08-进程管理]]、[[31-处理机调度]]、[[39-内存管理深入]]、[[40-文件系统深入]]、[[43-系统错误排查与日志分析]] 深入学习各个维度。持续性监控方案参考 [[58-监控系统]]。
