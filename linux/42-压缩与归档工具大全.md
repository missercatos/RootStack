...
> > 答案
> > **B**
> > `-c` 代表 Create（创建新的归档文件）。tar 本身不做压缩，压缩由 `-z`、`-j`、`-J` 等选项完成。

> [!question]- 选择题 2：创建 zstd 压缩的 tar 归档，正确的命令是？
...
> > 答案
> > **B**
> > zstd 使用 `--zstd` 长选项（无短选项）。gzip 为 `-z`，bzip2 为 `-j`，xz 为 `-J`。

> [!question]- 选择题 3：在压缩速度和压缩率之间取得最佳平衡的工具是？
> - A. gzip
> - B. bzip2
> - C. xz
> - D. zstd
...
> > 答案
> > **D**
> > zstd 设计目标即为此：默认级别下比 gzip 更快且压缩率更高。
...
> > 答案
> > **B**
> > `--listed-incremental=SNAPSHOT_FILE` 创建并维护快照文件来记录文件状态以实现增量备份。

> [!question]- 选择题 5：7z 加密归档时加密文件名的选项是？
> - A. `-mhe=on`
> - B. `--encrypt-names`
> - C. `-ef`
> - D. `-h`
...
> > 答案
> > **A**
> > `-mhe=on`（encrypt header）加密文件列表。完整命令：`7z a -p -mhe=on archive.7z files`。

> [!question]- 判断题 6：`xz -9` 的压缩率通常高于 `zstd -19`。
> - A. 正确
> - B. 错误
>
> > 答案
> > **A. 正确**
> > 在大多数数据集上，xz -9 的压缩率高于 zstd -19。但 zstd 的解压速度远快于 xz。

> [!question]- 判断题 7：tar 本身既能归档也能压缩。
> - A. 正确
> - B. 错误
>
> > 答案
> > **B. 错误**
> > tar 仅归档（合并多个文件为单个文件），压缩通过外部程序（gzip/bzip2/xz/zstd）完成。

> [!question]- 选择题 8：通过 SSH 将本地目录直接传输到远程并解压，正确的命令是？
...
> > 答案
> > **B**
> > `-czf -` 中的 `-` 表示输出到标准输出，通过管道传给 ssh；远程端 `tar -xzf -` 从标准输入读入。

> [!question]- 判断题 9：zstd 的字典压缩模式适合压缩大量结构相似的小文件。
> - A. 正确
> - B. 错误
>
> > 答案
> > **A. 正确**
> > zstd 通过 `--train` 学习数据模式构建字典，对大量彼此相似的小文件压缩效果极佳。

> [!question]- 选择题 10：lz4 的典型使用场景是？
> - A. 长期存档
> - B. 实时数据流和低延迟传输
> - C. 密码加密
> - D. 增量备份
...
> > 答案
> > **B**
> > lz4 压缩速度极快（数 GB/s），解压速度更快，适合需要近乎无延迟压缩传输的实时场景。

---

> **交叉链接**：归档与备份紧密相连，备份策略参考 [[15-备份与恢复]]。文件目录管理基础参考 [[04-文件与目录管理]]。磁盘存储和文件系统参考 [[10-存储管理与磁盘操作]] 和 [[40-文件系统深入]]。
