...
> [!question]- 选择题 1：获取设备属性信息以编写 udev 规则，最核心的命令是？
> - A. `lspci -v`
> - B. `udevadm info -a`
> - C. `dmesg | grep device`
> - D. `cat /proc/devices`
...
> > 答案
> > **B**
> > `udevadm info -a -n /dev/设备名` 是编写 udev 规则前获取设备匹配键的核心命令。

> [!question]- 选择题 2：`/dev/sda` 的主设备号标识什么？
> - A. 分区编号
> - B. 驱动类型（如 SCSI 磁盘驱动）
> - C. 磁盘序列号
> - D. 文件系统类型
...
> > 答案
> > **B**
> > 主设备号（major number）标识设备驱动类型，次设备号（minor number）标识该驱动下的具体设备实例。

> [!question]- 选择题 3：Linux 上 AMD 显卡的推荐开源驱动是？
> - A. radeon
> - B. fglrx
> - C. amdgpu
> - D. nvidia
...
> > 答案
> > **C**
> > amdgpu 是 AMD GCN 及更新架构的现代开源驱动，radeon 用于较老的显卡，fglrx 已是历史。

> [!question]- 选择题 4：libgpiod 中监控 GPIO 引脚电平变化的命令是？
> - A. gpioget
> - B. gpioset
> - C. gpiomon
> - D. gpiodetect
...
> > 答案
> > **C**
> > `gpiomon` 用于监控 GPIO 引脚的电平变化事件，支持上升沿/下降沿过滤。

> [!question]- 选择题 5：`udevadm monitor` 的作用是什么？
> - A. 显示历史所有设备事件
> - B. 实时监控设备热插拔事件
> - C. 编辑 udev 规则
> - D. 备份 udev 配置
...
> > 答案
> > **B**
> > `udevadm monitor` 实时显示内核通过 uevent 发送的设备事件（add/remove/change），是调试 udev 规则的利器。

> [!question]- 判断题 6：Linux 设备驱动必须是内核模块（.ko）形式。
> - A. 正确
> - B. 错误
>
> > 答案
> > **B. 错误**
> > 驱动可以编译进内核（built-in），也可以是内核模块（.ko），还可以通过用户空间接口实现（如 FUSE、VFIO、UIO）。

> [!question]- 选择题 7：`tags` 在 udev 规则中的作用是什么？
> - A. 设置设备文件内容
> - B. 给设备打上标签，`uaccess` 标签可让登录用户自动获得访问权限
> - C. 管理设备的中断
> - D. 指定设备的 DMA 地址
>
> > 答案
> > **B**
> > `TAG+="uaccess"` 配合 `systemd-logind`，让当前活动 seat 上的用户自动获得设备访问权限。

> [!question]- 选择题 8：PipeWire 在 Linux 音频栈中的位置是？
> - A. 内核驱动层
> - B. 用户空间音频服务器（替代 PulseAudio/JACK）
> - C. 固件层
> - D. 硬件抽象层
>
> > 答案
> > **B**
> > PipeWire 是用户空间音频/视频流处理服务器，旨在统一替代 PulseAudio 和 JACK。

> [!question]- 判断题 9：I2C 协议需要四根信号线：SDA、SCL、MOSI、MISO。
> - A. 正确
> - B. 错误
>
> > 答案
> > **B. 错误**
> > I2C 仅需两根线：SDA（数据线）和 SCL（时钟线）。MOSI/MISO 是 SPI 协议的信号线。

> [!question]- 选择题 10：`dmidecode -t memory` 显示什么信息？
> - A. 硬盘空间
> - B. 物理内存条的容量、速度、类型等 DMI 信息
> - C. 系统内存使用率
> - D. 文件系统内存缓存
>
> > 答案
> > **B**
> > `dmidecode` 读取 SMBIOS/DMI 表，`-t memory` 显示物理内存插槽的详细信息。

---

> **交叉链接**：硬件驱动是内核模块的主要应用场景，延伸阅读 [[36-Linux内核基础与模块]]。深入 I/O 设备管理参考 [[35-I-O设备管理]]。音频系统深度配置参考 [[50-PipeWire与音频系统]]。发行版特定的显卡驱动安装参考 [[distro/arch/desktop/06-显卡驱动配置]]。
