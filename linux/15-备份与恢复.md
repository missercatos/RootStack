...
`tar` 是 Linux 最基础的备份工具。[[42-压缩与归档工具|完整 tar 指南见 42 章]]，这里聚焦备份场景。
...
> 数据库备份的详细策略、WAL 归档、流复制等内容见 [[56-数据库运维]]。
...
将此脚本配合 systemd timer 或 cron 定时执行即可实现自动化备份。详见 [[13-计划任务与自动化]]。
...
> [!question]- 选择题 1：3-2-1 备份法则的含义是？
> - A. 3 份备份、2 个副本、1 个源文件
> - B. 3 份副本、2 种介质、1 份异地
> - C. 3 种工具、2 份本地、1 份远程
> - D. 3 天一备份、2 种压缩、1 次验证
>
> > [!success]- 点击查看答案
> > **B**。经典 3-2-1 原则：3 份数据副本，至少 2 种不同存储介质，至少 1 份存放在异地。

> [!question]- 选择题 2：rsync 中 `--link-dest` 选项的作用是？
> - A. 创建目标的符号链接
> - B. 对未变化文件创建硬链接而非复制（去重）
> - C. 删除目标中的旧文件
> - D. 限制传输速度
>
> > [!success]- 点击查看答案
> > **B**。`--link-dest=DIR` 使 rsync 对 DIR 中已存在且内容相同的文件创建硬链接，实现高效去重，每次备份看起来完整但相同文件只占一份空间。

> [!question]- 判断题 3：BorgBackup 的 `borg prune` 用于删除指定时间之前的旧存档

> > [!success]- 点击查看答案
> > 正确。`borg prune` 根据 `--keep-daily/weekly/monthly` 策略删除旧存档，之后需运行 `borg compact` 回收空间。

> [!question]- 选择题 4：生产环境中直接 cp 正在运行的 MySQL 数据库文件制作备份是？
> - A. 推荐做法
> - B. 不可靠——可能产生不一致数据
> - C. 比 mysqldump 更安全
> - D. 只能用于 MyISAM
>
> > [!success]- 点击查看答案
> > **B**。直接复制正在运行的数据库文件会得到不一致的快照（崩溃一致而非事务一致），应使用 `mysqldump --single-transaction` 或先创建 LVM/Btrfs 快照再复制。

> [!question]- 选择题 5：dd 命令中 bs 参数的作用是？
> - A. 指定备份大小限制
> - B. 指定每次读写的块大小
> - C. 校验数据完整性
> - D. 压缩输出
>
> > [!success]- 点击查看答案
> > **B**。`bs=4M` 指定每次读写的块大小，合适的块大小可以显著提高 dd 的传输速度。

> [!question]- 判断题 6：Borg 和 Restic 都内置去重和加密功能

> > [!success]- 点击查看答案
> > 正确。两者都是现代化备份工具，内置块级去重、客户端加密、压缩和完整性校验。

> [!question]- 选择题 7：tar 增量备份中，`--listed-incremental` 的快照文件的用途是？
> - A. 存储备份密码
> - B. 记录文件的时间戳和状态，供下次增量对比
> - C. 压缩备份文件
> - D. 列出所有差异文件
>
> > [!success]- 点击查看答案
> > **B**。快照文件（.snar）记录上次备份时每个文件的时间戳和 inode 变化计数器，下次增量备份时通过对比决定哪些文件需要重新备份。

> [!question]- 选择题 8：Timeshift 的主要适用场景是？
> - A. 数据库热备份
> - B. 系统级快照回滚（系统配置升级前）
> - C. 远程云备份
> - D. 用户文件去重备份
>
> > [!success]- 点击查看答案
> > **B**。Timeshift 专注于系统文件快照，适合在系统更新、软件安装或配置修改前创建回滚点。用户数据（`/home`）不在其默认备份范围内。

> [!question]- 判断题 9：dd 的 if（input file）和 of（output file）写反会导致目标磁盘数据永久覆盖

> > [!success]- 点击查看答案
> > 正确。`if` 是输入（读取源），`of` 是输出（写入目标），写反会直接覆盖目标设备，且不可逆。

> [!question]- 选择题 10：验证备份是否可恢复的最佳做法是？
> - A. 仅检查备份文件大小
> - B. 检查备份日志是否包含"成功"
> - C. 定期在测试环境完整执行恢复流程
> - D. 信任备份工具的校验功能
>
> > [!success]- 点击查看答案
> > **C**。真正的验证是在测试环境（或临时目录）实际执行一次完整恢复，确认文件完整性、权限正确、服务可启动。其他方式只能检测部分问题。
