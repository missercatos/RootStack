...
> > 答案
> > **B**
> > `journalctl -b -1` 查看上一次启动的日志。`-b 0` 或 `-b` 是本次，`-b -2` 是倒数第三次。
...
> [!question]- 选择题 3：默认情况下，journald 日志存储在哪里？
...
> > 答案
> > **B**
> > 默认存储在 `/run/log/journal/`（内存），重启后丢失。需创建 `/var/log/journal/` 实现持久化。

> [!question]- 选择题 4：进入紧急模式后，第一步应该做什么？
> - A. 直接编辑配置文件
...
> > 答案
> > **B**
> > 紧急模式下根分区以只读方式挂载，必须先 `mount -o remount,rw /` 才能编辑配置文件。

> [!question]- 选择题 5：SMART 属性中，哪个数值大于 0 表示硬盘可能存在潜在故障？
> - A. Power-On Hours
...
> > 答案
> > **C**
> > 重分配扇区计数（ID 5）大于 0 说明已有坏扇区被重新映射，是硬盘健康状况下降的早期信号。

> [!question]- 判断题 6：`strace` 用于跟踪库函数调用，`ltrace` 用于跟踪系统调用。
> - A. 正确
> - B. 错误
>
> > 答案
> > **B. 错误**
> > 恰恰相反：`strace` 跟踪系统调用，`ltrace` 跟踪动态库函数调用。

> [!question]- 选择题 7：`coredumpctl gdb firefox` 的作用是什么？
> - A. 删除 firefox 的 coredump
> - B. 使用 GDB 分析 firefox 最近一次崩溃的 coredump
> - C. 阻塞 firefox 的所有网络连接
> - D. 重启 firefox
>
> > 答案
> > **B**
> > `coredumpctl gdb <程序名>` 直接启动 GDB 加载该程序最近的 coredump，无需手动查找转储文件。

> [!question]- 选择题 8：`lsof -i :80` 的作用是什么？
> - A. 列出 80 天内的日志文件
> - B. 列出正在使用 80 端口的进程
> - C. 限制文件描述符为 80
> - D. 归档 80 个文件
...
> > 答案
> > **B**
> > `-i :80` 按端口过滤，显示所有打开了 80 端口（TCP/UDP, 监听或连接中）的进程。

> [!question]- 判断题 9：`journalctl --vacuum-size=500M` 会删除所有超过 500M 的日志文件。
> - A. 正确
> - B. 错误
>
> > 答案
> > **B. 错误**
> > `--vacuum-size=500M` 保留总大小不超过 500M 的最近日志，删除更旧的日志。`--vacuum-time` 按时间而非大小清理。

> [!question]- 选择题 10：从 Live USB 启动后进入已安装系统进行修复，应该怎么做？
> - A. 直接执行安装好的系统的程序
> - B. 挂载根分区 → chroot 进入 → 在 chroot 内修复
> - C. 从 Live USB 直接执行修复命令而无需 chroot
> - D. 重装系统
>
> > 答案
> > **B**
> > 挂载根分区和必要虚拟文件系统后 chroot 进入已安装系统，在此环境中执行修复操作（重装内核、重建 initramfs、修复 GRUB 等）。

---

> **交叉链接**：系统性能深入分析参考 [[37-系统调优与性能分析]]。服务管理参考 [[09-systemd服务管理]]。引导流程问题参考 [[38-引导流程与GRUB]]。备份与恢复策略参考 [[15-备份与恢复]]。
