...
> - A. 不支持读写操作
> - B. 每次文件操作涉及额外的用户/内核态切换，有性能开销
> - C. 不支持多用户访问
> - D. 只能在 root 权限下运行
>
> > [!success]- 点击查看答案
> > **B**
> > FUSE 文件系统每次操作需要经过 VFS → FUSE 内核模块 → /dev/fuse → 用户空间守护进程 → 返回，至少涉及 2 次用户/内核态切换，相比内核文件系统有额外性能开销。

> [!question]- 选择题 2：fuse-overlayfs 存在的主要原因是什么？
> - A. 内核 overlayfs 不稳定
> - B. 内核 overlayfs 需要 CAP_SYS_ADMIN，rootless 容器无法使用
> - C. fuse-overlayfs 性能更好
> - D. 内核 overlayfs 不支持多层
>
> > [!success]- 点击查看答案
> > **B**
...
> > [!success]- 点击查看答案
> > **B**
> > EROFS 使用固定大小的输出块，对页面缓存友好，随机读取性能远超 SquashFS（变长块）。这使得 EROFS 特别适合需要频繁随机访问的场景如 Android 系统分区。

> [!question]- 选择题 5：overlayfs 中删除一个 lower 层的文件时，upper 层会如何处理？
> - A. 直接删除 lower 层文件
> - B. 在 upper 层创建一个 whiteout 字符设备标记
> - C. 将文件移动到回收站
> - D. 什么都不做，只标记不可见
>
> > [!success]- 点击查看答案
...
> > [!success]- 点击查看答案
> > **B**
> > composefs 使用 EROFS 存储元数据（文件名、权限、目录结构），配合 overlayfs 机制和内容寻址对象存储实现高效的文件去重和完整性验证。

> [!question]- 选择题 8：以下哪个 FUSE 文件系统最适合将多个不同大小的磁盘合并为单一存储池？
> - A. sshfs
> - B. gocryptfs
> - C. mergerfs
> - D. archivemount
...
> - A. 支持更多文件系统格式
> - B. 性能接近原生、强一致性、完整 POSIX 支持
> - C. 不需要宿主机守护进程
> - D. 支持 Windows 客户机
>
> > [!success]- 点击查看答案
> > **B**
> > virtiofs 专为虚拟机设计，性能接近原生（尤其 DAX 模式下），提供强一致性和完整 POSIX 语义，而 9p 只有基本缓存、弱一致性和有限 POSIX 支持。
