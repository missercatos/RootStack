...
循环缓冲是 [[F_队列_Queue|队列]] 的一种物理实现，也是管道（pipe）和套接字缓冲区（sk_buff）的基础。
...
`[[../数据结构/N_B树_BTree|N 树]]` 和 `[[../数据结构/P_哈希表_HashTable|哈希表]]` 的磁盘表现形式与 I/O 调度器直接相关。B 树的节点大小通常为一个或数个扇区（例如 InnoDB 的 16KB 页 = 4 个 4KB 扇区），分配到相邻 LBA 的叶子节点每次磁头扫过时可被连续读出。
...
`[[../数据结构/R_跳表_SkipList|跳表]]` 和 `[[../数据结构/M_红黑树_RedBlackTree|红黑树]]` 在内存中维护 MemTable 的常见选择。跳表在此期间因 concurrency 友好性胜出 -- 其分层结构允许无锁读取。
...
- B 树的磁盘存储布局 → [[../数据结构/N_B树_BTree|N 树]]
- 队列与环形缓冲在磁盘调度中的应用 → [[../数据结构/F_队列_Queue|队列 Queue]]
- 设备驱动的并发问题 → [[../数据结构/L_树_Tree_BST_AVL|并发数据结构]]
