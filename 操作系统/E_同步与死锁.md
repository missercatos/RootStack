...
**对数据结构的应用**：无锁数据结构（如无锁队列、无锁栈）用 CAS 指令替代自旋锁，减少忙等。实现的复杂度远高于加锁，但高竞争场景下性能更优。有兴趣深挖见 [[../数据结构/P_跳表_SkipList|跳表]] 和现代 C++ 中的 `std::atomic`。
...
- 信号量在生产——消费模式中的应用 → [[../数据结构/G_队列_Queue|队列 Queue]]
- 死锁在并发数据结构中的实例 → [[../数据结构/J_树_Tree_BST_AVL|并发树的锁策略]]
