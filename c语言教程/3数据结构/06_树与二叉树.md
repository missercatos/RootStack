...
本节侧重底层实现与内存理解，与 [[../../数据结构/L_树_Tree_BST_AVL|CPP教程对应章节]] 形成互补——CPP教程侧重 `std::set`/`std::map`（基于红黑树）的STL使用、迭代器语义和算法，本教程侧重手动实现二叉搜索树和AVL树的插入/删除/旋转，深刻理解树节点间的指针关系。
...
- **CPP对照**: [[../../数据结构/L_树_Tree_BST_AVL|CPP: 树/BST/AVL]] | [[../../数据结构/M_红黑树_RedBlackTree|CPP: 红黑树]]
