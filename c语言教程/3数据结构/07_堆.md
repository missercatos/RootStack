...
本节侧重底层实现与内存理解，与 [[../../数据结构/J_堆_Heap|CPP教程对应章节]] 形成互补——CPP教程侧重 `std::priority_queue` 的 STL 使用、自定义比较器与算法，本教程侧重手动实现二叉堆的数组存储、sift-up/sift-down 操作、以及堆在优先队列和排序中的应用。
...
- **CPP对照**: [[../../数据结构/J_堆_Heap|CPP: 堆]] | [[../../cpp教程/容器库/适配器/priority_queue|CPP: priority_queue]]
