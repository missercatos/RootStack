...
本节侧重底层实现与内存理解，与 [[../cpp深化教程/数据结构/B_栈_Stack|CPP教程对应章节]] 形成互补——CPP教程侧重 `std::stack` 的 STL 使用与适配器模式，本教程侧重用 C 手动实现两种底层结构（数组栈和链式栈），深入理解栈帧的内存本质。
...
> 关于尾递归优化的汇编原理，参见 [[../ASM教程/03_栈与函数|ASM: 栈与函数]]。
...
> > **正确答案: B**
> >
> > **解析**: x86 栈向低地址增长，`push` 先将 `rsp` 减 8（腾出空间），再将值写入 `[rsp]`。详见 [[../ASM教程/03_栈与函数|ASM: 栈与函数]]。
...
- **上一章**: [[02_链表|链表]] | **下一章**: [[04_队列|队列]] | **返回**: [[../目录|目录]]
- **CPP对照**: [[../cpp深化教程/数据结构/B_栈_Stack|CPP: 栈]] | [[../cpp深化教程/容器库/04_stack|CPP: stack]]
- **相关**: [[2深化/01_指针深度剖析|指针深度剖析]] | [[2深化/03_动态内存管理|动态内存管理]]
- **ASM**: [[../ASM教程/03_栈与函数|ASM: 栈与函数]] | [[../ASM教程/02_寄存器与寻址|ASM: 寄存器与寻址]]
