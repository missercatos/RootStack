...
本节侧重底层实现与内存理解，与 [[../../数据结构/E_栈_Stack|CPP教程对应章节]] 形成互补——CPP教程侧重 `std::stack` 的 STL 使用与适配器模式，本教程侧重用 C 手动实现两种底层结构（数组栈和链式栈），深入理解栈帧的内存本质。
...
- **CPP对照**: [[../../数据结构/E_栈_Stack|CPP: 栈]] | [[../../cpp教程/容器库/适配器/stack|CPP: stack]]
