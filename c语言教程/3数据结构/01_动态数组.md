...
本节侧重底层实现与内存理解，与 [[../../cpp教程/容器库/序列容器/vector.md|CPP教程对应章节]] 形成互补——CPP教程侧重STL使用与算法优化，本教程侧重手动实现与内存本质。你将亲手用C语言实现一个完整的动态数组，深入理解 `realloc()` 的工作原理、扩容策略对性能的影响、以及数组在内存中的真实布局。
...
> 关于栈与堆的区别，参见 [[2深化/03_动态内存管理|动态内存管理]] 和 [[../ASM教程/03_栈与函数|ASM: 栈与函数]]。
...
> 关于 `realloc` 的底层汇编实现，参见 [[../ASM教程/05_内存分配|ASM: 内存分配]]。
...
> 更高效的排序需要 `qsort`（C标准库），用法见 [[2深化/02_函数指针与回调|函数指针与回调]]。
...
> 关于 vector 的 STL 用法，详见 [[../../cpp教程/容器库/序列容器/vector.md|CPP: vector]]。
...
> **练习 4.2.1**: 为什么 `operator[]` 是 O(1)？从汇编角度解释：已知基址和元素大小，通过 `[base + index * sizeof(int)]` 计算地址，详见 [[../ASM教程/02_寄存器与寻址|ASM: 寄存器与寻址]]。
...
| 题号 | 题目 | 难度 | 知识点 |
|------|------|------|--------|
| [P1046](https://www.luogu.com.cn/problem/P1046) | 陶陶摘苹果 | 入门 | 数组操作 |
| [P1427](https://www.luogu.com.cn/problem/P1427) | 小鱼的数字游戏 | 入门 | 数组反转 |
| [P1428](https://www.luogu.com.cn/problem/P1428) | 小鱼比可爱 | 入门 | 数组遍历 |
| [P1554](https://www.luogu.com.cn/problem/P1554) | 梦中的统计 | 入门 | 数组计数 |
| [P5728](https://www.luogu.com.cn/problem/P5728) | 旗鼓相当的对手 | 入门 | 数组比较 |
...
- **下一章**: [[02_链表|链表]] | **返回**: [[../目录|目录]]
- **CPP对照**: [[../../cpp教程/容器库/序列容器/vector.md|CPP: vector]] | [[../../数据结构/A_容器_Container|CPP: 容器]]
- **相关**: [[2深化/03_动态内存管理|动态内存管理]] | [[2深化/01_指针深度剖析|指针深度剖析]]
- **ASM**: [[../ASM教程/02_寄存器与寻址|ASM: 寄存器与寻址]] | [[../ASM教程/05_内存分配|ASM: 内存分配]]
