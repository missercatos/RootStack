...
本节侧重底层实现与内存理解，与 [[../../数据结构/U_图_Graph|CPP教程对应章节]] 形成互补——CPP教程侧重图的 STL 表示（`vector<vector<int>>`）、算法模板与竞赛优化，本教程侧重手动实现邻接矩阵和邻接表、BFS/DFS 的底层细节、以及大型图的内存管理策略。
...
- **CPP对照**: [[../../数据结构/U_图_Graph|CPP: 图]] | [[../../数据结构/V_图的高级算法_AdvancedGraph|CPP: 图的高级算法]]
