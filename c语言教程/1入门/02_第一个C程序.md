...
> 编译器是 C 语言的"翻译官"，也是理解 C 语言底层行为的关键。汇编语言的详细内容见 [[../ASM教程/07_编译原理|ASM教程: 编译原理]] 和 [[../ASM教程/02_指令集与寻址|ASM教程: 指令集与寻址]]。
...
> 编译原理的深入讲解见 [[../ASM教程/07_编译原理|ASM教程: 编译原理]]。C++ 的编译过程与 C 完全相同，只是多出了名称修饰（name mangling）等 C++ 特有步骤。
...
> C++ 的输入输出体系比 C 更类型安全，但 C 的 `printf` 在小规模输出场景下更简洁。详见 [[../cpp深化教程/基础教程/04_第一个程序与输入输出|CPP教程: 第一个程序与输入输出]]。
...
> 参数传递约定详见 [[../ASM教程/03_栈与函数|ASM教程: 栈与函数]]。
...
> - 带 `for` 循环的函数汇编
> 
> 用注释在汇编代码中标注每条 C 语句对应的汇编指令。参考 [[../ASM教程/02_指令集与寻址|ASM教程: 指令集与寻址]]。
