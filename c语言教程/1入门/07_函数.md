...
> 函数调用的完整栈帧机制，特别是 x86-64 System V ABI 的寄存器传参约定，详见 [[../ASM教程/03_栈与函数|ASM教程: 栈与函数]]。C++ 的成员函数调用唯一的区别是多传递了一个 `this` 指针作为隐式第一参数。
...
> C++ 提供了"引用"（`&`）作为参数传递的语法糖，底层同样是传递地址。详见 [[../cpp深化教程/03_指针与引用|CPP教程: 指针与引用]]。
...
> `printf` 和 `scanf` 就是典型的可变参数函数。详细的 x86-64 可变参数调用约定请参考 [[../ASM教程/03_栈与函数|ASM教程: 栈与函数]]。
...
> > [!success]- 点击查看答案
> > 正确答案: C
> > **解析**: x86-64 System V ABI 规定整数返回值通过 `rax`/`eax` 寄存器返回。参数通过 `rdi`、`rsi`、`rdx`、`rcx`、`r8`、`r9` 依次传递。详见 [[../ASM教程/03_栈与函数|ASM教程: 栈与函数]]。
...
> 
> 洛谷练习：https://www.luogu.com.cn/problem/P5727 （汉诺塔问题）
...
> 3. 树的深度优先遍历（用栈模拟递归）
> 
> 洛谷练习：https://www.luogu.com.cn/problem/P5728 （递归与递推）
>
> 相关：https://www.luogu.com.cn/problem/P5729 （数楼梯）
