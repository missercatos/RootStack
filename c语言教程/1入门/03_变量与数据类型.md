...
> 汇编层面：有符号比较使用 `jg`/`jl`（检查 SF 和 OF），无符号比较使用 `ja`/`jb`（检查 CF）。详见 [[../ASM教程/02_指令集与寻址|ASM教程: 指令集与寻址]]。
...
> C++ 使用了更安全的 `static_cast` / `reinterpret_cast`，参见 [[../cpp深化教程/03_指针与引用|CPP教程: 指针与引用]]。
...
> 
> 洛谷练习：https://www.luogu.com.cn/problem/P1143 （进制转换）
...
> **难度**: 
> 
> 洛谷练习：https://www.luogu.com.cn/problem/P1042 （乒乓球）
> 
> 完成后额外实验：
