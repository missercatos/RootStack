...
本章的核心思想是：**OOP 是一种设计范式，不是语言特性**。Linux 内核、GTK+、CPython 等大型 C 项目都使用本章描述的模式。建议同时阅读 [[2深化/04_函数指针与回调|函数指针与回调]] 理解函数指针，以及 [[../cpp深化教程/05_面向对象类与对象基础|CPP: 类与对象基础]]、[[../cpp深化教程/06_面向对象继承|CPP: 继承]] 和 [[../cpp深化教程/07_面向对象多态与虚函数|CPP: 多态与虚函数]] 对比 C++ 的等价特性。
...
>  **对应 C++ 特性**: 不透明指针 + 访问器函数 = C++ 的 `private` 成员 + `public` 成员函数。区别在于 C 完全靠约定和编译期的类型遮蔽。详见 [[../cpp深化教程/05_面向对象类与对象基础|CPP: 类与对象基础]]。
...
>  **核心洞察**: C++ 的虚函数机制并没有硬件层面的新特性——就是函数指针表 + 间接调用。所有"魔法"都是编译器的语法糖。这也是为什么理解 C 的 vtable 实现后，C++ 的面向对象变得透明。详见 [[../cpp深化教程/07_面向对象多态与虚函数|CPP: 多态与虚函数]]。
...
- **前置章节**: [[2深化/04_函数指针与回调|函数指针与回调]] — 函数指针是 vtable 的基础
- **前置章节**: [[2深化/01_指针深度剖析|指针深度剖析]] — 结构体指针与 -> 运算符
- **C++ 对比**: [[../cpp深化教程/05_面向对象类与对象基础|CPP: 类与对象基础]] — C++ 类的语法糖
- **C++ 对比**: [[../cpp深化教程/06_面向对象继承|CPP: 继承]] — 继承的内存布局对应
- **C++ 对比**: [[../cpp深化教程/07_面向对象多态与虚函数|CPP: 多态与虚函数]] — 虚函数表机制对比
- **汇编参考**: [[../ASM教程/03_栈与函数|ASM: 栈与函数]] — call 指令与间接调用
- **Rust 参考**: [[../Rust教程/README|Rust教程]] — Rust 的 trait 系统对比 C 的 vtable
