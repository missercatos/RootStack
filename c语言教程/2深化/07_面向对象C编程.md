...
本章的核心思想是：**OOP 是一种设计范式，不是语言特性**。Linux 内核、GTK+、CPython 等大型 C 项目都使用本章描述的模式。建议同时阅读 [[04_函数指针与回调|函数指针与回调]] 理解函数指针，以及 [[../cpp深化教程/05_面向对象(一)类与对象基础|CPP: 类与对象基础]]、[[../cpp深化教程/06_面向对象继承|CPP: 继承]] 和 [[../cpp深化教程/07_面向对象多态与虚函数|CPP: 多态与虚函数]] 对比 C++ 的等价特性。
...
>  **对应 C++ 特性**: 不透明指针 + 访问器函数 = C++ 的 `private` 成员 + `public` 成员函数。区别在于 C 完全靠约定和编译期的类型遮蔽。详见 [[../cpp深化教程/05_面向对象(一)类与对象基础|CPP: 类与对象基础]]。
...
- **C++ 对比**: [[../cpp深化教程/05_面向对象(一)类与对象基础|CPP: 类与对象基础]] — C++ 类的语法糖
