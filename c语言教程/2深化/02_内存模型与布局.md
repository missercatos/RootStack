...
>  本章从"程序运行时，内存里到底长什么样"这个根本问题出发，深入剖析 C 语言程序的内存布局。理解内存布局是理解指针行为、段错误原因、栈溢出、堆碎片化等一切底层问题的前提。建议同时阅读 [[../ASM教程/06_内存与IO|ASM: 内存与IO]] 和 [[../ASM教程/03_栈与函数|ASM: 栈与函数]]。
...
> 每个函数的机器码按顺序排列在 `.text` 段中。函数指针的值就是 `.text` 段中的地址。详见 [[2深化/04_函数指针与回调|函数指针与回调]]。
...
>  **注意**: 大块内存（通常 >128KB）的 malloc 使用 `mmap` 而非 `brk`，释放时会立即归还给操作系统。详见 [[2深化/03_动态内存管理|动态内存管理]]。
...
碎片化的详细分析和解决方法见 [[2深化/03_动态内存管理|动态内存管理]]。
...
> 详细栈帧分析参见 [[../ASM教程/03_栈与函数|ASM: 栈与函数]]。
...
> ️ **重要**: 字节序问题在跨平台开发、网络编程、嵌入式系统中是常见的 bug 来源。永远不要假设目标平台是小端——即使 x86 占据了绝大部分市场。
...
完整 gdb 调试指南参考 [[2深化/03_动态内存管理|动态内存管理]] 中的调试部分。
...
- **前置章节**: [[2深化/01_指针深度剖析|指针深度剖析]] — 理解指针才能理解内存地址
- **同系列后续**: [[2深化/03_动态内存管理|动态内存管理]] — 堆内存管理的深入讨论
- **汇编参考**: [[../ASM教程/06_内存与IO|ASM: 内存与IO]] — MMU、虚拟内存物理实现
- **汇编参考**: [[../ASM教程/03_栈与函数|ASM: 栈与函数]] — 栈帧的汇编细节
- **汇编参考**: [[../ASM教程/07_编译原理|ASM: 编译原理]] — ELF 格式的段与节
- **汇编参考**: [[../ASM教程/01_体系结构与寄存器|ASM: 体系结构与寄存器]] — CPU 架构与寄存器
- **C++ 对比**: [[../cpp深化教程/03_指针与引用|CPP: 指针与引用]] — C++ 的对象内存布局 vtable
- **C++ 对比**: [[../cpp深化教程/05_面向对象类与对象基础|CPP: 类与对象基础]] — 类对象的内存布局
