...
>  函数指针是 C 语言中实现高阶编程模式的基石。通过函数指针，C 可以在运行时选择要执行的代码，实现回调（callback）、策略模式、插件架构等。理解函数指针也是理解 C++ 虚函数表（vtable）的前提——虚函数的底层实现就是函数指针表。建议对照阅读 [[2深化/01_指针深度剖析|指针深度剖析]] 中的指针声明规则和 [[../ASM教程/03_栈与函数|ASM: 栈与函数]] 中的函数调用机制。
...
>  **关键指令 `call *%rax`**: 星号 `*` 表示间接调用——CPU 读取 rax 寄存器的值作为目标地址，将返回地址压栈，然后跳转。相比之下，`call add` 是直接调用，目标地址编译时就确定。这是动态多态的硬件基础。详见 [[../ASM教程/03_栈与函数|ASM: 栈与函数]]。
...
> 与 C++ 对比：C++ 的 `std::sort` 使用模板（编译期多态）实现同样效果，通常比 qsort 快（比较函数可内联）。详见 [[../cpp深化教程/05_面向对象类与对象基础|CPP: 类与对象基础]]。
...
> 分派表将 O(1) 查找性能与代码清晰性结合。这是 Linux 内核中系统调用表、文件操作表等核心机制的基础。详见 [[2深化/07_面向对象C编程|面向对象C编程]]。
...
> 注意 `s->append(s, ...)` 的参数重复——第一个参数 `s` 传了两次。这就是为什么 C++ 发明了隐式 `this` 指针：将 `s->append(s, "World")` 简化为 `s->append("World")`。详见 [[../cpp深化教程/05_面向对象类与对象基础|CPP: 类与对象基础]]。
...
// 完整的 OOP 实现见 [[2深化/07_面向对象C编程|面向对象C编程]]
```
...
> > 正确答案: B
> >
> > **解析**: C++ 编译器自动为有虚函数的类生成 vtable（虚函数指针数组），并通过 vptr 实现动态分派。C 语言需要程序员手动创建函数指针表、在每个对象中存储 vtable 指针、手动传递 this 指针。详见 [[2深化/07_面向对象C编程|面向对象C编程]]。
...
- **前置章节**: [[2深化/01_指针深度剖析|指针深度剖析]] — 指针声明规则（右左法则）
- **同系列相关**: [[2深化/07_面向对象C编程|面向对象C编程]] — 函数指针构造虚函数表
- **汇编参考**: [[../ASM教程/03_栈与函数|ASM: 栈与函数]] — call/ret 指令和间接调用
- **汇编参考**: [[../ASM教程/02_指令集与寻址|ASM: 指令集与寻址]] — 间接寻址模式
- **C++ 对比**: [[../cpp深化教程/05_面向对象类与对象基础|CPP: 类与对象基础]] — 成员函数与 this 指针
- **C++ 对比**: [[../cpp深化教程/07_面向对象多态与虚函数|CPP: 多态与虚函数]] — vtable 的 C 语言等价实现
