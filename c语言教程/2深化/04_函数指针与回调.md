...
> 与 C++ 对比：C++ 的 `std::sort` 使用模板（编译期多态）实现同样效果，通常比 qsort 快（比较函数可内联）。详见 [[../cpp深化教程/05_面向对象(一)类与对象基础|CPP: 类与对象基础]]。
...
> 注意 `s->append(s, ...)` 的参数重复——第一个参数 `s` 传了两次。这就是为什么 C++ 发明了隐式 `this` 指针：将 `s->append(s, "World")` 简化为 `s->append("World")`。详见 [[../cpp深化教程/05_面向对象(一)类与对象基础|CPP: 类与对象基础]]。
...
- **汇编参考**:  — 间接寻址模式
- **C++ 对比**: [[../cpp深化教程/05_面向对象(一)类与对象基础|CPP: 类与对象基础]] — 成员函数与 this 指针
