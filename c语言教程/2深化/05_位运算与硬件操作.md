...
>  位运算是 C 语言最接近硬件的特性之一。所有高级语言的布尔运算、乘除法优化、加密算法，最终都翻译为 CPU 的 AND/OR/XOR/SHIFT 指令。本章从 CPU 的逻辑门出发，深入位运算符的底层实现，然后延伸到 `volatile` 关键字、内存映射 I/O（MMIO）和 GCC 内联汇编——这些是嵌入式开发和操作系统内核的必备技能。建议同时阅读 [[../ASM教程/02_指令集与寻址|ASM: 指令集与寻址]] 中的位操作指令和 [[../ASM教程/06_内存与IO|ASM: 内存与IO]] 中的 MMIO。
...
> 完整的内联汇编教程参见 [[../ASM教程/10_GCC内联汇编|ASM: GCC内联汇编]]。
...
- **汇编参考**: [[../ASM教程/02_指令集与寻址|ASM: 指令集与寻址]] — AND/OR/XOR/SHL/SHR/TEST 指令
- **汇编参考**: [[../ASM教程/06_内存与IO|ASM: 内存与IO]] — MMIO 的硬件实现
- **汇编参考**: [[../ASM教程/10_GCC内联汇编|ASM: GCC内联汇编]] — 完整的内联汇编教程
- **同系列相关**: [[2深化/02_内存模型与布局|内存模型与布局]] — 地址空间与 MMIO 定位
- **同系列相关**: [[2深化/06_编译链接与ELF|编译链接与ELF]] — 链接器脚本中的内存布局
