...
// 注意: memcmp 比较包含 padding 字节，其值未定义
// 结构体比较应逐个字段比较
```
...
> setjmp/longjmp 是底层机制，使用不当容易产生难以调试的 bug。C++ 的异常处理在底层也使用类似机制（但编译器自动管理栈展开和析构函数调用）。详见 中关于非局部跳转的更多细节。
